...
from tenancy.exceptions import BrandingLogoUploadRejectedError
...
# Reserved slug (also listed in `tenancy.slug_validation._RESERVED_ROUTE_SLUGS`,
...
#
# Both writers of the ``logo`` field (``tenancy.serializers.
...
# The delivery route (``tenancy.views.OrganizationLogoDeliveryView.
...
def signed_logo_url(logo: FieldFile | str | None) -> str | None:
    ...
    (``tenancy.views.OrganizationLogoDeliveryView._resolve_logo_key``):
...
def build_logo_display_url(branding: OrganizationBranding | None, request=None) -> str:
    ...
    Not used by the invitation email (``tenancy.notification_contexts``):
...
def sign_branding_logo_upload(
    ...
    not this function's -- see ``tenancy.views.
    ...
    delivery route (``tenancy.views.OrganizationLogoDeliveryView``) treats
...
def branding_diff_state(branding: OrganizationBranding | None) -> dict[str, str]:
    ...
    Shared by both write surfaces -- ``tenancy.views.OrganizationBrandingView``
