...
1b. **Declare the service subclasses.** `0.4.0` replaces `vinta_orgs.helpers` with generics bound once per project: `Organizations(OrganizationService[Organization])` and `Memberships(MembershipService[Organization, OrganizationMembership])`, each with `model_class`. `MembershipService` derives the organization model from its membership model's foreign key, so it needs no organization-service parameter. The call sites to move are `resolve_membership_for_user` → `memberships.resolve_for_user` and `resolve_organization_for_user` → `memberships.resolve_organization_for_user`. (Corrected 2026-08-15: this said "three files under `accounts/`". **There are none on this branch** — `vinta_orgs.helpers` is first imported on **Phase 3.5**, by **nine** files: `accounts/account_adapters.py`, `accounts/post_auth_destination.py`, `accounts/tests/test_social_gated_onboarding.py`, `legal/services.py`, `common/utils/view_utils.py`, and four modules under `organizations/tests/`. The three `accounts/` files named here import our own `organizations.models.get_active_organization_membership` at this point in the stack, not the package helper. Phase 3 therefore only *declares* the subclasses; Phase 3.5 does the moving, against the nine-file list.) Our own `OrganizationService` in `organizations/services.py` is a *different, domain* class and is not what this replaces — do not conflate them; if the names collide confusingly, alias the package one at import.
