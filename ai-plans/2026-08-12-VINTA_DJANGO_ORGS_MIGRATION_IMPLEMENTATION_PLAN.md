...
1. **Content-type / permission data migration** (`tenancy/migrations/0023_move_content_types_to_tenancy.py`): update `django_content_type.app_label` from `organizations` to `tenancy` for the app's four models (`Organization`, `OrganizationMembership`, `OrganizationInvitation`, `OrganizationBranding`), and repoint the `auth_permission` rows that reference them. Idempotent both directions; the *original* content-type row always survives (and keeps its id), in both cases: **no collision** (no `tenancy`-labelled row yet for a model) relabels the existing row in place, so every FK stays valid at the same id with zero further work; **collision** (both an `organizations`- and a `tenancy`-labelled row already exist — the shape produced by a `migrate` run against a database while the app was resolvable under both labels) merges the fresh `tenancy` row into the surviving *original* one: a duplicate-codename permission is merged (its group/user grants re-pointed, the fresh permission row deleted), a permission with no codename match is simply re-pointed, any `django_admin_log` entry pointing at the fresh row is re-pointed defensively, and the fresh, now-empty content type row is deleted last. Keeping the original id (rather than the fresh row's) in the collision branch matters beyond that one run: it makes the branch converge with the no-collision one, so a forward → reverse → forward loop (`post_migrate` recreates a fresh `tenancy` row after every reverse) never loses the original id, its permissions, or its admin-log linkage. `organizationtier` / `subscriptionplan` (models deleted in `0015_remove_subscriptionplan_tier_and_more.py`) and `organizationsite` (the package's own model, not installed until Phase 1c) are explicitly out of scope. Reverse relabels the single surviving `tenancy` row back to `organizations`; because the original id always survives forward (both branches), this is an exact, lossless undo, not merely a best-effort one — consistent with, but stronger than required by, the "Pre-launch posture" Guiding Decision.
...
6. Delete the dead-with-reason `BrandingWriteGateReason.NO_SLUG` (and its `BRANDING_GATE_EXCEPTIONS` entry and the `if not organization.slug` check in `evaluate_branding_write_gate`) from `@tenancy/permissions.py` — retired in the Phase 1c review (see the plan's Guiding Decisions "Slug precondition for branding writes is retired" row) once `organization_slug_not_blank` made it permanently unreachable through any supported write path. `OrganizationSlugRequiredForBrandingError` in `@tenancy/exceptions.py` goes with it once nothing references it.
