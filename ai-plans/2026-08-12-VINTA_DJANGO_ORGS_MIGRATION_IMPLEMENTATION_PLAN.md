...
| Decision | Resolution |
|---|---|
| **Package owns the authorization substrate** (amended, package `0.3.0`) | **Anything `vinta-django-orgs` `0.3.0` ships, we consume rather than reimplement.** Four pieces this plan had hand-written move to the package: the membership `is_active` gate (now on `AbstractOrganizationMembership`, filtered *in* `OrganizationModelBackend._get_membership` rather than on its result, so the per-organization cache never holds an unusable row); `vinta_orgs.authorization.has_organization_permission(user, permission, organization)` with `include_global` / `allow_superuser` both defaulting off (the two escalation paths Phase 4's review found — a global `user_permissions` grant, and membership of the seeded `organization_admin` group via the Django user-admin picker — are refused by default rather than by our own narrowing); `vinta_orgs.drf.OrganizationScopedAPIViewMixin` plus `resolve_membership_for_user` (the resolution seam between authentication and `check_permissions`, and the full 0/1/2+ × named/absent/non-member table with `AmbiguousOrganizationError` for the 400); and `vinta_orgs.testing` (the reseed-at-setup fixture for the transactional-flush defect, driven by `ORGANIZATION_GROUP_SEEDERS`). **What stays ours** is domain, not infrastructure: the capability catalog and its codenames, the `X-Organization-Id` header name and its integer-pk retriever, the `_active_membership` stash the ~60 legacy `get_active_organization_membership` call sites read, the reseller-root subtree walk, the branding entitlement gates, and `membership_role_label` with the two published role strings. The test that keeps the line honest is the one already established by the prune chore: if a test would still pass against a stock package install, it belongs upstream, not here. |
...
| **Scoping semantics flip** | Our `BaseOrganizationModelManager` requires an explicit `filter_by_organization(org_id)`; the package's `objects` scopes implicitly and returns `.none()` when nothing is bound. This is the single most dangerous delta in the migration — an unbound query in a Celery task reads as "no data" rather than as a bug. Mitigated two ways: a behavior-neutral audit phase binds every call site *before* any model flips, and `STRICT_ORGANIZATION_FILTER = True` from the moment the first one does, so the failure is an exception rather than an empty list. (Amended, package `0.3.0`: strict mode is now the package **default**, and `0.3.0` additionally stops `create()` / `bulk_create()` scoping the queryset they are built from — both insert without reading, so the scoping could only ever refuse a valid call, including `create(organization=organization)` and every `instance.related_set.create(...)`. We keep the setting spelled out explicitly in `SHARED_SCHEMA_ORGANIZATIONS` rather than inheriting it: this plan's central safety argument rests on it, and a default is a weaker guarantee than a declaration. `get_or_create()` / `update_or_create()` still scope, which is correct — they look a row up first, and that lookup is exactly the one that must not span tenants. That is the Phase 2a blocker, now fixed upstream.) |
| **Organization resolution** (amended, package `0.3.0`) | A custom retriever reads `X-Organization-Id` (integer PK) and is the only entry in `ORGANIZATION_RETRIEVERS`. `TenantScopedViewMixin` is retained rather than replaced by `OrganizationMiddleware` — that part of the original decision stands, and `0.3.0` vindicates it by shipping the same conclusion: the middleware runs before DRF authentication has populated `request.user`, so a user-dependent organization cannot be resolved there at all. What changes is that the mixin no longer *implements* the seam. It composes `vinta_orgs.drf.OrganizationScopedAPIViewMixin`, which owns the `perform_authentication` override and the release in a `finally` around `dispatch`, and delegates the table to `resolve_membership_for_user`. Our mixin keeps exactly what is ours: the header name, the integer-pk lookup, and the `_active_membership` stash that makes ~60 existing `get_active_organization_membership` call sites header-aware without touching them. The package raises `AmbiguousOrganizationError` where we raised a DRF `ValidationError`; the 400 status and the `{"detail": ...}` body are contract and must survive the swap — pin them, do not re-derive them. |
...
1. **Bump `vinta-django-orgs` to `>=0.3,<0.4`.** This is the phase that first consumes `0.3.0` API, which is why the bump lands here rather than in Phase 1: verified before choosing this point, and each of the three candidate forcing conditions was checked and rejected. `is_active` predates this plan on our concrete model, and a concrete model may legally override a field inherited from an *abstract* base, so our declaration (which carries `db_index` / `db_default` the package's does not) wins and the model state is unchanged — `makemigrations --check` must stay clean, and it failing is the signal that this reasoning was wrong. We use neither `IsOrganizationOwner` nor `DjangoOrganizationModelPermissions`, so their new "no organization selected → `False`" behaviour reaches nothing. And the `create()` / `bulk_create()` change only *removes* raises, so nothing written against `0.2.0` breaks. Phases 0, 1, 2a and 2b are therefore untouched by the bump.
...
2. **Delegate the resolution table to `resolve_membership_for_user`**, keeping what is ours: the `X-Organization-Id` header name, the integer-pk lookup, and the `_active_membership` stash. **The status codes are contract, not implementation.** The package raises `AmbiguousOrganizationError` where we raised a DRF `ValidationError`, and `OrganizationAccessDeniedError` where we raised `PermissionDenied`; both must still render 400 and 403 with the existing `{"detail": ...}` bodies. Map them explicitly rather than assuming DRF's default handler agrees.
...
**Alpha dependency.** `vinta-django-orgs` is `0.3.0`, Development Status `Alpha`, first published to PyPI on 2026-08-11. Pinned `>=0.3,<0.4`. Three minor releases have each carried a breaking change — `0.1.0` to `OrganizationMembership.objects` scoping, `0.2.0` renaming both app labels, and `0.3.0` defaulting `STRICT_ORGANIZATION_FILTER` to `True` — so the minor pin is doing real work. Being the package's author is what makes this acceptable: a breaking upstream change is a decision, not a surprise. **This plan has now been amended twice for exactly such a bump** (see **Amendments**), which is the empirical case for keeping the pin tight and re-reading the diff on every bump rather than trusting semver alone.
