...
0. **Rewrite `common/organization_context.py` as a class-specialized state object.** `0.4.0` deletes the module-level `get_current_organization` / `set_current_organization` / `clear_current_organization` / `reset_current_organization` / `organization_context` functions that Phase 2a made this module re-export. Declare one `ProjectOrganizationState(OrganizationState[Organization])` with `model_class = Organization`, and keep the module's existing seven public names as thin shims over `organization_state.get()` / `.set()` / `.clear()` / `.reset()` / `.context()`. **The shims are the point**: **162 references across 25 modules** import these names from this module, and only two test modules import them from the package directly — repoint those two. Rewriting one module keeps those 162 untouched, which is why this is a Phase 3 change and not a stack-wide sweep. (Corrected 2026-08-16: this said "roughly 260 call sites across 33 files", which was never measured. **Unit, so it cannot drift again:** a *reference* is one AST `Name` node resolving to one of this module's nine public names — `get_current_organization`, `set_current_organization`, `clear_current_organization`, `reset_current_organization`, `organization_context`, `organization_state`, `ProjectOrganizationState`, `OrganizationOrSlug`, `OrganizationToken` — counted outside `common/organization_context.py` itself, with the `from common.organization_context import ...` statements excluded. A *module* is a `.py` file with at least one such import. Two further files mention the module in prose only, `conftest.py` and `common/organization_services.py`, and are not counted. Measured on this branch at commit `621c44f0`.) `OrganizationOrSlug` is no longer public upstream; restate it here in terms of *our* `Organization`, as this module already does for the other signatures.
...
1b. **Declare the service subclasses.** `0.4.0` replaces `vinta_orgs.helpers` with generics bound once per project: `Organizations(OrganizationService[Organization])` and `Memberships(MembershipService[Organization, OrganizationMembership])`, each with `model_class`. `MembershipService` derives the organization model from its membership model's foreign key, so it needs no organization-service parameter. The call sites to move are `resolve_membership_for_user` → `memberships.resolve_for_user` and `resolve_organization_for_user` → `memberships.resolve_organization_for_user`. (Corrected 2026-08-15, re-corrected and verified 2026-08-16: this said "three files under `accounts/`", then "nine files". **There are none on this branch** — `vinta_orgs.helpers` is first imported on **Phase 3.5**. Verified there by `git grep -n 'vinta_orgs.helpers' plan/vinta-django-orgs-migration/phase-3.5`, with **imports and prose recorded separately** so nobody hunts a file that does not exist:

   **Seven real imports** (these are the edits Phase 3.5 owns):

   | File | Line | Form |
   | --- | --- | --- |
   | `accounts/account_adapters.py` | 20 | module-level `from vinta_orgs.helpers import resolve_membership_for_user` |
   | `accounts/post_auth_destination.py` | 23 | module-level, same |
   | `accounts/tests/test_social_gated_onboarding.py` | 148 | function-local, same |
   | `legal/services.py` | 18 | module-level, same |
   | `organizations/tests/test_models.py` | 12 | module-level, same |
   | `organizations/tests/test_org_resolution.py` | 32 | module-level, same |
   | `organizations/tests/test_permissions.py` | 6 | module-level, same |

   That is **three** modules under `organizations/tests/`, not four, and **two** application modules plus **one** test under `accounts/`.

   **Three prose-only mentions**, in comments and docstrings — no import, nothing to repoint, but they name a module that will not exist and should be reworded: `common/utils/view_utils.py:24`, `organizations/models.py:247`, `common/tests/test_resolution_table_after_reorder.py:7`. (`common/utils/view_utils.py` was previously listed as an importer; it is not one.) The plan's own L145 and the tracking file's L406 also name the deleted module in prose.

   The `accounts/` files import our own `organizations.models.get_active_organization_membership` at *this* point in the stack, not the package helper. Phase 3 therefore only *declares* the subclasses; Phase 3.5 does the moving, against the seven-import list.) Our own `OrganizationService` in `organizations/services.py` is a *different, domain* class and is not what this replaces — do not conflate them; if the names collide confusingly, alias the package one at import.
...
Tests:
- **Unit**: `organizations/tests/test_permission_backend.py` — an admin's membership resolves the four permissions under a bound organization and **none** under a different bound organization; an unbound context yields no organization permissions. Scope this to **our** wiring — that our `AUTHENTICATION_BACKENDS` entry is the package's class, in the right order, against our concrete membership model. A test that would pass against a stock package install belongs upstream (the standard the prune chore sets).

  (Corrected 2026-08-13: this list originally also required "the union with global permissions works", which contradicts the ownership standard stated in the same sentence — the union is unconditional package code with no repo-owned hook, and Phase 4's `has_organization_permission(..., include_global=False)` is deliberately built *not* to read it. The prune chore resolved the contradiction in favour of the standard and deleted `TestTheUnionWithGlobalPermissions`; `has_perm` itself keeps stock `ModelBackend` semantics, untested here on purpose.)
...
- **Integration**: `payments/tests/test_reseller_root_billing.py` — preserves the low-level reseller-root subtree rule and its explicit-organization permission check. Under the package header resolver, the descendant-bound request shape that would make this branch decisive is currently unreachable; this test does not claim endpoint behavior.
...
1. `@organizations/serializers.py`: `MyMembershipSerializer` and the `mine` serializer swap `role` for `permissions`; the member-list serializer drops `role` and `is_billing_owner`. `can_manage_branding` stays a distinct field for the reason given in **API Design**. **Deviation**: `OrganizationMembershipSerializer` also gained a `permissions` field, which the plan's API Design bullet did not call for (it says only *drop* `role` / `is_billing_owner`). Added because without it an admin datatable has no way to tell who the admins are once `role` is gone; not an any-member leak since the endpoint is `IsOrganizationAdmin`-gated.
...
**This plan has now been amended three times for exactly such a bump**, which is no longer just an argument for a tight pin — it is the dominant maintenance cost of the migration. Each bump has been cheaper than the last **only because the coupling is concentrated**: `common/organization_context.py` re-exports the package's binding API for **162 references across 25 modules** (unit and measurement defined in Phase 3, change 0), so `0.4.0`'s deletion of that API is one module's rewrite rather than a 25-module sweep. Keep it that way. The rule that follows from three bumps: **wrap the package's surface in one project-owned module per concern, and let call sites import ours.** Where that discipline held, `0.4.0` cost a shim; where it did not — the **seven** files importing `vinta_orgs.helpers` directly, none of them on this branch and all of them on Phase 3.5 (list in Phase 3, change 1b) — it costs a per-file edit.
...
- **2026-08-15** — Retargeted from `vinta-django-orgs` `0.3.0` to `0.4.0`, which **deletes the function-based API** in favour of class-specialized generics.
  ...
  **Verified before acting.** Three conditions could have dragged the bump earlier and taken the whole stack with it; each was checked and rejected. No migration in this repo writes `organization` on an existing row, so the new immutability rule needs no opt-in anywhere — an audit, and audits go stale, so `common/tests/test_organization_immutability.py` (added 2026-08-16) pins the rule itself: a persisted row re-stamped with another organization raises, an ordinary save of the same row does not, and `unsafe_organization_update=True` is reachable at the call site if a data migration ever does need it. `ORGANIZATION_GROUP_SEEDERS` already points at our own callable, not the deleted package helper. And Phase 2a's re-export of the function API keeps working on its own branch, because Phases 0–2b pin `>=0.2,<0.3` — the first place their code runs under `0.4.0` is Phase 3's full-suite run, which is where the bump lands.

  **The write-path cost this bump buys, recorded as a decision rather than discovered later.** The immutability rule is not free, and its price is a locking read on the write path. `SingleOrganizationModelMixin.save()` opens a `transaction.atomic()` and issues `_base_manager.select_for_update().filter(pk=…).values_list('pk', 'organization_id')` on **every** save of an already-persisted scoped row whose write includes the organization column — across all 34 scoped models. Measured on this branch, one such save is **four statements where an unscoped model issues one**: `SAVEPOINT`, `SELECT … FOR UPDATE`, `UPDATE`, `RELEASE SAVEPOINT`.

  *When it applies* is wider than it first looks. The obvious case is `update_fields is None` — a plain `instance.save()`. But it is **equally true when `update_fields` names an `OrganizationSafeForeignKey`**, because `expand_safe_relation_field_names` expands that name to `<name>_fk` *and* `organization` (assigning the relation sets both, so writing only the key would leave the row pointing across organizations). So `sync.save(update_fields=['calendar'])` takes the lock too. Only an `update_fields` list naming ordinary columns escapes it, and inserts never take it.

  *Why it is accepted.* Under `ATOMIC_REQUESTS = True` the mixin's `atomic()` nests inside the request transaction rather than replacing it, so each such save costs a savepoint pair plus a locking read, and **holds the row lock until the request transaction commits** — not until the save returns. Two writers touching the same row in the same request cycle serialize on it. This is accepted **because we are pre-launch**: there is no production write concurrency to regress, and the alternative (carrying our own weaker check, or pinning to `0.3.0`) costs more than the lock does. It is written down here so that if write latency or lock contention shows up after launch, this is the first place to look rather than a surprise.

  *What makes a future upstream change to it visible.* `common/tests/test_organization_immutability.py` pins both halves — the refusal itself and its query shape (`TestWhatTheRefusalCostsOnTheWritePath`). It is the suite's only **write-path** query-count assertion; every other one is read-path, which is why nothing caught this. If upstream changes the check's cost, that file goes red.
  ...
  **The lesson recorded rather than re-learned.** Three bumps in, the cost of each is set by how concentrated the coupling is. `0.4.0` deleted the binding API that **162 references across 25 modules** use (unit defined in change 0 above), and it cost one module's rewrite because those references import `common/organization_context.py` rather than `vinta_orgs.state`. The **seven** files that import `vinta_orgs.helpers` directly — all on Phase 3.5, none here; list in change 1b — are the counter-example and the per-file cost. See the **Alpha dependency** note in **Risk & Rollout Notes**.
