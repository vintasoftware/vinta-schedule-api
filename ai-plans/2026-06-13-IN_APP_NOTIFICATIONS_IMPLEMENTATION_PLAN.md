...
| **Zero ORM in the view** | `NotificationViewSet` is a plain DRF `ViewSet` (not `GenericViewSet`) with **no `queryset` / `get_queryset`**. Every endpoint reads/writes solely through the native vintasend service; the serializer is instantiated directly. `serializer_class` is kept only as a drf-spectacular schema hint. Native `get_in_app_notifications` already returns exactly `status in (SENT, READ)`, so there is nothing for an ORM query to add. |
...
| **Ownership enforcement on mark-read** | Both single and bulk go through native `mark_read_bulk(ids, user_id=request.user.id)`, which scopes the update + read-back to the user server-side. Single mark-read passes `[pk]` and 404s when the result is empty (foreign / unknown / non-SENT-non-READ id); bulk passes the id list and silently skips foreign ids. Both prevent IDOR natively — no ORM ownership query. |
...
- Calls `NotificationService.mark_read_bulk([id], user_id=request.user.id)` (native, ownership-scoped); `404` when the result is empty (foreign / unknown / non-SENT-non-READ id). No ORM lookup.
...
Changes:
1. `notifications/views.py`: add a detail action `mark_read` (`POST /notifications/{id}/mark-read/`). Delegate to native `self.notification_service.mark_read_bulk([pk], user_id=request.user.id)` (ownership-scoped + idempotent); `raise NotFound` (404) when the returned iterable is empty (foreign / unknown / non-SENT-non-READ id), else serialize the single returned notification. No ORM `get_object`/`get_queryset`.
