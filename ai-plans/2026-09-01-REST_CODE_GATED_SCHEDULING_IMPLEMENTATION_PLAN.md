...
| **Duration lives on the bookable resource** | `CalendarGroup.duration`, a nullable `DurationField`. **Amended 2026-09-03** — the plan originally put this on `CalendarManagementToken` as a per-code pin. That was wrong in one decisive way: a codeless public-group booking presents no code, so it inherited no pin and could be booked at any length — the one path reachable with no credential at all was also the one path with no length constraint. Duration is a property of the thing being booked, not of the invitation to book it. Since nothing had merged, `0051` was rewritten in place rather than adding and later dropping a column inside one stack. |
...
a per-code pin. Because nothing in this stack had merged, migration `0051` was rewritten in place to
add `CalendarGroup.duration` instead — the token column is never created at any point in the
history. See the **Duration lives on the bookable resource** row in **Guiding Decisions** for why.
...
### Phase 0 — Booking-code REST scaffolding, mint attribution, and group duration
...
3. No duration is carried on the issued codes — the constraint lives on the group, so a re-issued reschedule code inherits it automatically through `can_perform_group_scheduling` / `can_perform_update`. A 30-minute group appointment therefore cannot be rescheduled into a 60-minute one even though the code itself pins nothing.
...
4. **The slot search takes its duration from `group.duration`, not from the client.** That is the only length a codeless booking can have, so `duration_seconds` is not a query parameter on these endpoints at all — accepting one would let a caller browse slots at a length the write side would then refuse. A group with a null `duration` cannot accept public scheduling (Phase 0's invariant), so these reads return `403` for it rather than guessing a length.
...
in a late phase and dropping `CalendarManagementToken.duration` in the same phase would have left a
column created only to be deleted inside one unmerged stack. Migration `0051` was rewritten in place
instead, and the nine phase branches were rebased and force-pushed. No production data was at risk —
the token column never shipped. The cost was re-anchoring the inline review comments on nine PRs,
which was accepted deliberately.
...
- **2026-09-03** — Moved the event-duration constraint from `CalendarManagementToken` to
  `CalendarGroup`. The per-code pin left the codeless public-group path — the only path reachable with
  no credential at all — with no length constraint, because it presents no code to carry a pin.
  Duration is a property of the thing being booked, not of the invitation. Single-calendar pinning is
  dropped rather than relocated (explicit decision; there is no `Calendar.duration`), so Phases 1 and 4
  lose per-code pinning. A public group with a null duration now refuses bookings rather than accepting
  any length — fail closed — which requires a pre-deploy audit of existing public groups; see
  **Risk & Rollout Notes**. The invariant that `accepts_public_scheduling=True` requires a duration is
  enforced in the service layer, so the deployed GraphQL group mutations inherit it.

  **Resolved by rewriting history, not by migrating forward.** Nothing had merged, so migration `0051`
  was rewritten in place to add `CalendarGroup.duration` and never create the token column; Phase 0 was
  squashed; and phases 1, 2, 3, 3b, 4, 5, 6 and 7 were rebased and force-pushed. The separate duration
  phase drafted for this change was folded into Phase 0 and deleted, and the phases drafted after it
  renumbered to 8 (patient management codes) and 9 (public-group flag and codeless discovery).
  Affected phases: 0 (group duration added), 1 and 4 (per-code pin removed), 2 (pin re-pointed at the
  group), 3 (codeless bookings now constrained), 5 (read override derives from the group), 6
  (`duration_seconds` removed from the mint contract), 3b and 7 (rebase only). Branches force-pushed:
  all nine. Inline review comments on the open PRs were left to go stale rather than re-posted.
