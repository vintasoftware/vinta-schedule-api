...
- **Status**: reviewed clean (no BLOCKER; 1 correctness fix + 3 test-gap fixes; 2 activation preconditions + 1 NIT recorded), gates green, ready to integrate. **Final executable phase — the plan is code-complete through Phase 13.**
- **Models**: implementer Tier 4; reviewer Tier 4 (money leaves the building); fixer Tier 4
- **Branch**: `plan/billing-plans-and-limits/phase-13` · **Base**: `plan/billing-plans-and-limits/phase-12` (unmerged — #203 open)
- **Commits**: `04e858e` implementation, `bfc2bd2` review fixes · **No migration** (the durable "period closed" marker is the rolled `current_period_start`, not a new model)

**Gates** (implementer, independent re-run, then fixer): suite **4622 passed / 0 failed**; `mypy` **305 / 57** at ceiling; `ruff` clean (530); `makemigrations --check` clean.

`close_billing_periods` beat task, per subscription whose period has ended: reconcile → charge overage → roll → (counters reset via the roll). **Exactly-once by construction** (reviewer- and orchestrator-verified): the whole reconcile→charge→roll runs in one `transaction.atomic()` under `SELECT ... FOR UPDATE`, the idempotency key is `overage-{pk}-{period_start}` derived from the pre-roll `current_period_start`, and after the roll `current_period_end` is in the future so the `while` guard makes a re-run a no-op. Concurrent sweeps serialize on the lock; a crash before commit unrolls and retries with the identical key (provider dedupes). **Same-derivation**: the charged sum (`overage_total()` = `Sum(unit_price) where not is_within_allowance`) and `reconcile_period` both read one `for_billing_period` query; the boundary is the stored period the meter stamped. **Inert real-money gate**: `_charge_overage` short-circuits on a NULL effective limit, and every org is `unlimited` today, so no charge fires — activation is gated to Phase 14 + the recurrence fix.

The two deferred period-boundary actions were wired into the one close pass: Phase 9's `pending_plan` downgrade flip and Phase 10's `CANCELLED → FREE` (routed through `transition_billing_state`). A finance management command re-runs reconciliation for a named closed period (read-only).

**Fixes this round:** the finance command reconstructed an *annual* sub's past period with a 12-month stride when periods roll monthly — made monthly-correct via a shared `resolve_settlement_period`, and the same bug in `reconcile_period`'s internal bounds was fixed with it (else the report would disagree with the command's displayed bounds). Three money-path test gaps closed: catch-up now proves N distinct keys / one charge per elapsed period; the CANCELLED case proves the final period's overage is charged before the FREE flip; and a new `transaction=True` concurrency test proves two racing closes settle exactly one charge (6/6 green in isolation).

**Carried forward from 13 (Phase 14 activation preconditions):** (two structural items recorded, not fixed — both are inert today because every org is on `unlimited` and both would restructure the money path, which belongs with Phase 14's activation work rather than this inert final-phase fix round. No money-path code changed for either.)

- **C — the overage gate keys on the CURRENT effective limit, not the stamped rows.** `CycleCloseService._charge_overage` (`cycle_close_service.py:237-247`) short-circuits and returns `Decimal("0")` when the *current* effective `event_occurrences` limit is NULL. A mid-period move to `unlimited` (limit set to NULL) *between* metering and close would make the gate drop legitimately-stamped overage rows silently — `MeteredOccurrence.for_billing_period(...).overage_total()` would be > 0 but is never summed. The direction is safe (undercharge, never overcharge) and it is inert today, but **Phase 14's activation must re-key this gate on the stamped rows / `overage_total() > 0`, not the live limit.** The stamped `is_within_allowance` was frozen at meter time (Phase 7) precisely so a later limit change can't retroactively reprice a closed period; the gate must honor the same principle. Not fixed now because re-keying the gate is a money-path change that should ship with the activation tests that actually exercise a real charge.

- **D — catch-up settles up to `MAX_CLOSE_PERIODS_PER_RUN` (24) periods in ONE transaction holding the row lock across up to 24 provider round-trips.** `close_subscription` (`cycle_close_service.py:169-178`) wraps the whole catch-up `while` loop in a single `transaction.atomic()` + `select_for_update`. If period N's charge fails, periods 1..N-1's rolls **and** their `Payment` rows unwind and are re-attempted on the next beat (provider dedup saves the *money* via the stable `(subscription, period_start)` key, but it re-reconciles/re-charges-dedup N-1 periods, and the subscription row lock is held across up to 24 network calls). **Phase 14 should give each period its own `transaction.atomic()`** so an earlier period's successful roll survives a later period's failure and the lock is not held across two dozen provider calls. Not fixed now because it restructures the money path's crash-safety reasoning, which should be done alongside the activation tests that exercise a real charge.

- **NIT — `_apply_cancelled_to_free_if_due` re-logs forever for a subscription stuck non-FREE/non-CANCELLED.** `cycle_close_service.py:367-377` swallows `IllegalBillingStateTransitionError` and warns every beat for a subscription that is CANCELLED-adjacent but not on the `CANCELLED → FREE` edge (e.g. GRACE-at-cancel). Consider a one-shot marker or rate-limit at activation. Inert today; not fixed now.
