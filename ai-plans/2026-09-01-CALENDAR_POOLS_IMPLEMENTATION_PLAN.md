...
| **Uniqueness under projection** | `UNIQUE(slot_fk, calendar_fk, source_pool_fk)` cannot be a plain constraint: Postgres treats NULLs as distinct, so two inline rows for the same pair would both be accepted, which silently corrupts `required_count` satisfaction counting. **Corrected during Phase 3:** this originally said to use `NULLS NOT DISTINCT` *or* a partial index. `NULLS NOT DISTINCT` requires **Postgres 15+**, and local development runs 14.23 while CI runs `postgres:15` and docker-compose runs `postgres:alpine` — so it would have passed CI and failed on a developer machine. What shipped is a **pair of partial unique indexes**, provably equivalent and portable across all three: `UNIQUE (slot_fk, calendar_fk) WHERE source_pool_fk IS NULL` and `UNIQUE (slot_fk, calendar_fk, source_pool_fk) WHERE source_pool_fk IS NOT NULL`. Do not reach for `nulls_distinct=False`. |
...
`CalendarGroupSlotInputData` in [calendar_integration/services/dataclasses.py:329-336](calendar_integration/services/dataclasses.py#L329-L336) gains `pool_ids: list[int] | None = None` alongside its existing `calendar_ids`. **Corrected during Phase 3:** this originally said `list[int]` "defaulting to empty", which contradicts the omit-means-unchanged rule in the Phase 3 body — an empty list cannot encode "unchanged". `None` means leave attachments untouched, `[]` means detach all. Existing callers omit the field entirely and so behave identically.
