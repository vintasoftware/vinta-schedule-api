...
| **Bundle events** | **Amended 2026-06-23.** Bundle events **ARE** create-able, reschedulable, and cancellable through the Public API. (1) **Reschedule/cancel**: updating/deleting an existing bundle **primary** event routes to `_update_bundle_event` / `_delete_bundle_event`; gated only by `_public_token_may_write` (owner-scoped must own the bundle calendar; **org-wide acts org-wide**). (2) **Create** (`create_event` / `scheduleEvent`): a bundle calendar routes to `_create_bundle_event` (the designed fan-out); permitted for an **owner-scoped** token that owns the bundle calendar. **Org-wide create stays blocked** — the plan keeps `create_event` owner-scoped-only (see Non-goals). Non-primary bundle *child* events remain rejected by the existing `is_bundle_event` `ValueError`. Do **not** re-introduce a bundle-calendar `PermissionDenied` in `create_event` / `update_event` / `delete_event` / `_load_recurring_master_for_occurrence`. |
...
- **2026-06-23 (follow-up)** — Bundle event **creation** also enabled through the Public API. Removed the scoped-token bundle-calendar `PermissionDenied` from `create_event` (it now routes a bundle calendar to `_create_bundle_event` as designed) for an **owner-scoped** token that owns the bundle calendar; **org-wide create stays blocked**. Updated the pre-existing `scheduleEvent` bundle test (`test_schedule_event_bundle_calendar_proceeds_to_fanout`) to assert the create reaches the fan-out, and added a service-layer allowance test. Done on the **phase-0b** branch (at the user's request); phase-1/2/3 re-rebased + force-pushed.
