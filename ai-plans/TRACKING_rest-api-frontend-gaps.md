...
## Current Phase
Phase 1 — Add `OrganizationMembership.is_active` (next).

## Remaining Phases
2 (list members), 3 (deactivate/reactivate member), 4 (request own import), 5 (request own sync), 6 (admin sync other), 7 (rooms sync trigger), 8 (transfer event), 9 (calendar soft-disable), 10 (bundle update), 11 (bundle disable), 12 (token create), 13 (token list), 14 (token revoke), 15 (token edit perms), 16 (events expanded).
