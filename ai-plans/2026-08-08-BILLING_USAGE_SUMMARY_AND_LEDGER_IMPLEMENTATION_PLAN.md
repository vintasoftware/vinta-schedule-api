...
3. **Idempotency**: `get_or_create` on `(subscription, billing_period_start)`, inside the existing period transaction. Cycle close is already idempotent on that key and catch-up runs re-enter it, so a re-run must be a no-op at the database level rather than a second statement. Persisting must never be able to fail a close that already charged — the write is ordered after the charge and any unexpected error is logged and swallowed.
   ...
   **Correction, found during Phase 2's Tier 4 review**: an earlier draft of this plan said a missing statement was "recoverable by re-running close". It is not. `_roll_period` commits immediately after the write, moving `current_period_end` into the future, and `close_subscription`'s `while locked.current_period_end <= now` guard then never re-enters that period. A swallowed failure loses that statement permanently; recovery requires a targeted backfill. Say that, and not the reassuring version, in the `ERROR` log an on-call engineer will read.

   The statement write must also read **exactly** the row set the charge summed. `_charge_overage` and `reconcile_period` filter `MeteredOccurrence` by period only, with no organization filter, so the statement must not add one — pool membership is computed at close time and can differ from what it was at meter time (a child promoted to its own billing root mid-period leaves the pool, and its already-billed rows would silently vanish from the statement while still appearing on the invoice).
...
**Phase 2 writes inside the transaction that charges customers.** The statement write is ordered *after* `_charge_overage` and *before* `_roll_period`, and is failure-isolated: an error persisting a statement is logged and swallowed rather than allowed to roll back a committed charge or block the period from rolling. The `(subscription, billing_period_start)` unique constraint makes a re-entry a no-op — the same reasoning `MeteredOccurrence` and `ProviderWebhookEvent` already encode, where the constraint is the correctness mechanism rather than the code path.

Two corrections from that phase's Tier 4 review, both worth carrying:

- **A swallowed statement failure is permanent, not recoverable.** The period rolls immediately afterwards and the sweep guard never revisits it. The `ERROR` log says so plainly; recovery is a targeted backfill.
- **Assigning the raw `payment_id` column instead of the `payment` instance defeats the isolation entirely.** Django creates FK constraints on PostgreSQL as `DEFERRABLE INITIALLY DEFERRED`, so a dangling id is checked at *outer commit* — after the savepoint closed, outside the `try/except`, after the roll — and would roll back the whole close including a charge the provider already took. Assign the instance, so the error lands inside the savepoint where the handler can absorb it. The isolation guarantee is also not unconditional: if `savepoint_rollback()` itself fails, Django marks the outer atomic for rollback regardless.
...
**Phase 1 shifts every usage count from `COUNT(*)` to a hash aggregate.** Grouping by `organization_id` means the aggregation can no longer be satisfied straight from an index — `metered_occ_sub_period_idx` is `(subscription, billing_period_start)` and does not carry `organization_id`, so `check_postpaid_allowance`, on the hottest guarded write path, now scans the same rows and then aggregates. The result set is bounded by the pool size (almost always 1). Flagged during Phase 1's Tier 4 review as a note rather than a change; if a large reseller subtree ever makes this measurable, the fix is to add `organization_id` to that index, not to reintroduce a second scalar counter.
