...
2. Add a **policy filter** to the engine: given candidates, an `EffectivePolicy`, and request `now`, drop candidates with `start < now + lead_time`, `start > now + max_horizon`, or whose bare window `[start, end]` overlaps any blocking span **expanded to `[span_start - buffer_before, span_end + buffer_after]`** (the dead-zone-around-the-event model — matches SPEC **Acceptance scenarios** #4; an earlier candidate-envelope phrasing was inverted and is corrected). **When a buffer applies, fetch blocking spans for managed calendars too** (see **Guiding Decisions**), widening the fetch window by `buffer_after` on the start side and `buffer_before` on the end side; when the policy is unconstrained, skip all of this so output is unchanged.
