...
- **2026-06-04** — Phase 3 DI cleanup: `AccountAdapter.confirm_email` no longer resolves `OrganizationService` from the `di_core` container global at the call site. It now arrives via **constructor injection** (`__init__` `Provide["organization_service"]`), mirroring `notification_service`. No `di_core/apps.py` change — `accounts` is already wired in `INTERNAL_INSTALLED_APPS`. Affected phases: 3 (production rewrite). Branches force-pushed: `plan/auth-tenant-provisioning/phase-3` and rebase-only `phase-4`–`phase-9`.
- **2026-06-04** — Phase 6 DI cleanup (symmetry with Phase 3): `SocialAccountAdapter._provision_org_membership` no longer resolves `OrganizationService` from the `di_core` container global. It now arrives via **constructor injection** — `@inject` on `__init__` (decorating `__init__`, not the class, so `SocialAccountAdapter` stays a real class and `__bases__`-based test patching keeps working); the helper became an instance method using `self.organization_service`. Affected phases: 6 (production rewrite). Branches force-pushed: `plan/auth-tenant-provisioning/phase-6` and rebase-only `phase-7`–`phase-9`. Both adapters now consume `OrganizationService` the same way.
