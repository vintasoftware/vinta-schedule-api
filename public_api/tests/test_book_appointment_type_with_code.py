"""Integration tests for createAppointmentTypeEventWithCode.

All requests are unauthenticated (no Authorization header).  The booking
code provides the org scope, appointment type scope, and CREATE permission.
...
1. Happy path on a RESTRICTED primary calendar (accepts_public_scheduling=False)
   ...
   that authorizes appointment-type-scoped tokens against member calendars.
...
4. Calendar-scoped code (no appointment type) → NOT_PERMITTED.
...
@pytest.fixture
def organization():
    return baker.make(Organization, name="AppointmentType-Book-With-Code Test Org")
...
def primary_calendar(organization):
    ...
    Using a restricted calendar ensures the test exercises the new appointment-type-scoped
    ...
    return baker.make(
        ...
        external_id="primary-cal-appointment-type-code-test",
...
def secondary_calendar(organization):
    ...
    return baker.make(
        ...
        organization=organization,
        name="Room Calendar",
        external_id="room-cal-appointment-type-code-test",
        provider=CalendarProvider.INTERNAL,
...
def calendar_scoped_code(permission_service, organization, primary_calendar):
    """A CREATE code scoped to a single calendar — wrong scope for appointment type mutation."""
    token, code = permission_service.create_booking_token(
        organization_id=organization.id,
        permissions=[EventManagementPermissions.CREATE],
...
def _appointment_type_booking_input(
    code: str,
    slot_selections: list[dict],
    **overrides,
) -> dict:
    """Build the default happy-path appointment type mutation input dict."""
    base = {
        "code": code,
...
class TestCreateAppointmentTypeEventWithCodeHappyPath:
    """Scenario 1: Valid appointment type code + restricted primary calendar + available slot → success.

    This is the critical test that validates the can_perform_scheduling fix.  With a
    RESTRICTED primary calendar (accepts_public_scheduling=False) and an appointment-type-scoped
    token, the fix must authorize via the appointment-type-membership branch.  Before the fix
    ...
    def test_happy_path_creates_appointment_type_event_and_consumes_code(
        ...
        data = post_graphql(
            anon_client,
            CREATE_APPOINTMENT_TYPE_EVENT_WITH_CODE,
            {"input": _appointment_type_booking_input(code, selections)},
        )
        ...
        assert result["event"]["title"] == "AppointmentType Appointment"
        ...
        # The event must exist in the DB, on the primary calendar, linked to the appointment type.
...
class TestCreateAppointmentTypeEventWithCodeReplay:
    ...
    def test_replay_returns_already_used(
        ...
        _token, code = appointment_type_booking_code
        selections = _slot_selections(appointment_type, primary_calendar, secondary_calendar)
        input_data = _appointment_type_booking_input(code, selections)

        # First call — must succeed.
        first = post_graphql(
            anon_client, CREATE_APPOINTMENT_TYPE_EVENT_WITH_CODE, {"input": input_data}
        )
        assert first["data"]["createAppointmentTypeEventWithCode"]["success"] is True

        # Second call — same code must return ALREADY_USED.
        second = post_graphql(
            anon_client, CREATE_APPOINTMENT_TYPE_EVENT_WITH_CODE, {"input": input_data}
        )
        result = second["data"]["createAppointmentTypeEventWithCode"]
...
class TestCreateAppointmentTypeEventWithCodeFailedWriteDoesNotConsume:
    ...
    def test_slot_outside_availability_does_not_consume_code(
        ...
        out_of_window_input = _appointment_type_booking_input(
            code,
            selections,
    ...
    def test_invalid_slot_selection_does_not_consume_code(
        ...
        data = post_graphql(
            anon_client,
            CREATE_APPOINTMENT_TYPE_EVENT_WITH_CODE,
            {"input": _appointment_type_booking_input(code, bad_selections)},
        )
...
@pytest.mark.django_db
class TestCreateAppointmentTypeEventWithCodeCalendarScopedCode:
    """Scenario 4: Calendar-scoped code (token.calendar set, no appointment type) → NOT_PERMITTED."""
...
class TestCreateAppointmentTypeEventWithCodeLifecycleRejections:
    ...
    def test_invalid_code_returns_invalid_code(
        ...
        data = post_graphql(
            anon_client,
            CREATE_APPOINTMENT_TYPE_EVENT_WITH_CODE,
            {"input": _appointment_type_booking_input("aW52YWxpZGJvb2tpbmdjb2Rl", selections)},
        )
...
class TestCreateAppointmentTypeEventWithCodeCrossOrg:
    ...
    7a — Org-A code + org-A slot selections → success, event.organization_id == org A.
         org-B appointment type/calendars exist in the DB but are untouched.
    ...
    def test_org_a_code_books_only_org_a_resources(
        ...
        Org B's appointment type, slots, calendars, and availability windows are created in the DB
        ...
        org_a = organization
        grp_a = baker.make(AppointmentType, organization=org_a, name="Org-An AppointmentType")
        cal_a1 = baker.make(
            Calendar,
            organization=org_a,
        ...
        slot_a1 = AppointmentTypeSlot.objects.create(
            organization=org_a,
            appointment_type=grp_a,
            name="A-Physicians",
            order=0,
            required_count=1,
        )
        slot_a2 = AppointmentTypeSlot.objects.create(
            organization=org_a, appointment_type=grp_a, name="A-Rooms", order=1, required_count=1
        )
        AppointmentTypeSlotMembership.objects.create(
        ...
        # --- build org B (distinct DB rows; appointment type has same structural shape) ---
        org_b = baker.make(Organization, name="Org-B (cross-org target)")
        grp_b = baker.make(AppointmentType, organization=org_b, name="Org-B AppointmentType")
        cal_b1 = baker.make(
            Calendar,
            organization=org_b,
        ...
        slot_b1 = AppointmentTypeSlot.objects.create(
            organization=org_b,
            appointment_type=grp_b,
            name="B-Physicians",
            order=0,
            required_count=1,
        )
        ...
        # --- mint an org-An appointment type code ---
        _token, code = permission_service.create_booking_token(
        ...
        assert event.appointment_type_fk_id == grp_a.id
    ...
    def test_org_a_code_cannot_book_org_b_calendar(
        ...
        to an org-An appointment type booking code.  The server must reject it (SLOT_UNAVAILABLE, since
        the foreign calendar is not a member of the org-An appointment type's slots) and must NOT consume
        ...
        AppointmentTypeSlotMembership.objects.create(
            organization=org_a, slot=slot_a, calendar=cal_a
        )
        AvailableTime.objects.create(
            organization=org_a,
            calendar=cal_a,
