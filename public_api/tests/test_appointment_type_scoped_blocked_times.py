"""Integration tests for the public GraphQL surface of appointment-type-scoped
blocked times.

Direct mirror of ``test_appointment_type_scoped_availability_windows.py`` for blocks.
Covers ``appointmentTypeScopedBlockedTimes`` (query) and
``batchUpsertAppointmentTypeScopedBlockedTimes`` (batch-upsert mutation): batch apply,
...
APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES_QUERY = """
...
BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES_MUTATION = """
mutation BatchUpsertAppointmentTypeScopedBlockedTimes($input: BatchAppointmentTypeScopedBlockedTimesInput!) {
    batchUpsertAppointmentTypeScopedBlockedTimes(input: $input) {
        success
        errorMessage
        blockedTimes {
            id
...
class TestAppointmentTypeScopedBlockedTimesPublicAPI:
    ...
    # ------------------------------------------------------------------
    # appointmentTypeScopedBlockedTimes -- query
    # ------------------------------------------------------------------

    def test_query_returns_appointment_type_scoped_blocks_for_the_slot(self):
        org = self._setup_org()
        ...
        assert blocks[0]["appointmentTypeSlotId"] == slot.id
    ...
    def test_query_cross_organization_scoping_returns_empty(self):
        ...
        system_user, token, auth_service = self._make_org_wide_system_user(
            org_a, [PublicAPIResources.APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES]
        )
        ...
        data = response.json()
        assert data.get("errors", []) == []
        assert data["data"]["appointmentTypeScopedBlockedTimes"] == []

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedBlockedTimes -- apply
    # ------------------------------------------------------------------
    ...
    def test_batch_upsert_applies_mixed_operations(self):
        ...
        slot = self._make_appointment_type_slot(org, calendar)
        system_user, token, auth_service = self._make_org_wide_system_user(
            org, [PublicAPIResources.BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES]
        )
        ...
        data = response.json()
        assert data.get("errors", []) == []
        result = data["data"]["batchUpsertAppointmentTypeScopedBlockedTimes"]
        ...
        remaining = BlockedTime.objects.for_appointment_type_slot(slot.id).filter_by_organization(
            org.id
        )
        assert remaining.count() == 2

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedBlockedTimes -- idempotent replay
    # ------------------------------------------------------------------
    ...
    def test_identical_replay_is_a_no_op(self):
        ...
        first_result = first.json()["data"]["batchUpsertAppointmentTypeScopedBlockedTimes"]
        ...
        second_result = second.json()["data"]["batchUpsertAppointmentTypeScopedBlockedTimes"]
    ...
    def test_reason_participates_in_the_idempotent_create_match_key(self):
        """``reason`` is part of ``_find_matching_appointment_type_scoped_block``'s match
        key (calendar, appointment type slot, start, end, timezone, reason, rrule). A
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedBlockedTimes -- RESTRICTED billing root still blocks
    # ------------------------------------------------------------------
    ...
    def test_restricted_organization_batch_rejected_wholesale(self):
        ...
        assert (
            BlockedTime.objects.for_appointment_type_slot(slot.id)
            .filter_by_organization(org.id)
            .count()
            == 0
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedBlockedTimes -- cross-organization scoping
    # ------------------------------------------------------------------
    ...
    def test_cross_organization_appointment_type_slot_is_rejected(self):
        ...
        system_user, token, auth_service = self._make_org_wide_system_user(
            org_a, [PublicAPIResources.BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES]
        )
        ...
        assert (
            not BlockedTime.objects.for_appointment_type_slot(slot_b.id)
    ...
    def test_cross_owner_scoped_token_rejected_wholesale(self):
        """A scoped token may not write appointment-type-scoped blocks on a calendar it
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedBlockedTimes -- IDOR (block/calendar cross-check
    # on update/delete)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedBlockedTimes -- create outside the slot's roster
    # ------------------------------------------------------------------
    ...
    def test_query_anonymous_request_denied(self):
        ...
        response = self.client.post(
            "/graphql/",
            data={
                "query": APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES_QUERY,
    ...
    def test_mutation_anonymous_request_denied(self):
        ...
        response = self.client.post(
            "/graphql/",
            data={
                "query": BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES_MUTATION,
                "variables": {
                    "input": {
                        "organizationId": org.id,
                        "appointmentTypeSlotId": slot.id,
    ...
    def test_query_token_without_resource_grant_denied(self):
        """An authenticated token that lacks the APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES
    ...
    def test_mutation_token_without_resource_grant_denied(self):
        ...
        BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES resource grant is denied, and
        nothing is written."""
        org = self._setup_org()
        calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar)
    ...
    def test_existing_appointment_type_scoped_availability_windows_query_shape_unchanged(self):
        """Byte-for-byte shape check: the frozen appointmentTypeScopedAvailabilityWindows
        query's response is unaffected by the appointment-type-scoped additions."""
        org = self._setup_org()
        calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar)
        system_user, token, auth_service = self._make_org_wide_system_user(
            org, [PublicAPIResources.APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS]
        )
        ...
        query = """
        query AppointmentTypeScopedAvailabilityWindows($appointmentTypeSlotId: Int!, $calendarId: Int) {
            appointmentTypeScopedAvailabilityWindows(appointmentTypeSlotId: $appointmentTypeSlotId, calendarId: $calendarId) {
                id
                calendarId
                appointmentTypeSlotId
                startTime
        ...
        data = response.json()
        assert data.get("errors", []) == []
        windows = data["data"]["appointmentTypeScopedAvailabilityWindows"]
