...
def test_aggregate_query_writes_audit_record(
    ...
    """Running a calendarEventAggregate query writes exactly one AGGREGATE_QUERY audit record."""
    ...
    system_user, token, auth_service = org_wide_token(organization, ["calendar_event"])
...
def test_aggregate_query_with_zero_rows_writes_audit(
    ...
    """A query returning zero rows still writes exactly one AGGREGATE_QUERY audit record."""
    ...
    # Verify the audit record was written with row_count = 0.
    repository = OrganizationAuditRepository()
    page = repository.query(
        OrganizationAuditQuery(
            actions=[AuditAction.AGGREGATE_QUERY],
