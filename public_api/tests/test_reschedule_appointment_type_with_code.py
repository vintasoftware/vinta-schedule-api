"""Integration tests for rescheduleAppointmentTypeEventWithCode.

All requests are unauthenticated (no Authorization header).  The appointment type
reschedule code provides the org scope, the appointment-type scope, the
specific event scope, and the RESCHEDULE permission.

Scenario coverage:
1. Happy path — valid appointment-type-reschedule code + in-window new slot → success on a
...
4. Calendar-scoped reschedule code (token.appointment type is None) → NOT_PERMITTED
...
7. The rescheduled event is exactly `token.event` and still has `appointment_type_fk`
   set (still an appointment-type event after reschedule).
"""
...
@pytest.fixture
def organization():
    return baker.make(Organization, name="Reschedule-AppointmentType-With-Code Test Org")


@pytest.fixture
def primary_calendar(organization):
    """The primary calendar for the appointment type.  RESTRICTED: accepts_public_scheduling=False.
...
def appointment_type_event(organization, appointment_type, primary_calendar, secondary_calendar):
    """An appointment-type primary CalendarEvent with a linked non-primary BlockedTime.

    Constructed directly with baker/model calls (bypassing the appointment type service
...
def calendar_scoped_reschedule_code(
    ...
    """A RESCHEDULE code scoped to a single calendar only (no appointment type)."""
    token, code = permission_service.create_booking_token(
        organization_id=organization.id,
        permissions=[EventManagementPermissions.RESCHEDULE],
...
@pytest.fixture
def create_only_code(permission_service, organization, appointment_type):
...
@pytest.fixture
def cancel_only_code(permission_service, organization, appointment_type, appointment_type_event):
    """A CANCEL-only code bound to the appointment-type event — wrong permission for rescheduling."""
    token, code = permission_service.create_booking_token(
        organization_id=organization.id,
...
# Original slot set by the ``appointment_type_event`` fixture (10:00-11:00 UTC on 2030-06-01).
...
def _reschedule_appointment_type_input(code: str, **overrides) -> dict:
...
def _get_blocked_time(organization: Organization, event_id: int, calendar_id: int) -> BlockedTime:
    """Load the BlockedTime row linked to an appointment-type event for a specific calendar."""
    return BlockedTime.objects.filter_by_organization(organization.id).get(
        external_id=f"appointment-type-event-{event_id}-cal-{calendar_id}"
    )
...
class TestRescheduleAppointmentTypeWithCodeHappyPath:
    """Scenario 1: Valid appointment-type-reschedule code + in-window slot → success.

    Verifies:
    - Primary CalendarEvent times are updated.
    - Non-primary BlockedTime times are updated.
    - Event id is preserved (same pk as the original appointment_type_event).
    - Title, description, external attendee are preserved.
    - Code is consumed.
    - Works on a RESTRICTED primary calendar.
    - The returned event still has appointment_type_fk set.
    """
    ...
    def test_happy_path_updates_primary_event_and_blocked_times(
        ...
        token, code = appointment_type_reschedule_code

        data = post_graphql(
            anon_client,
            RESCHEDULE_APPOINTMENT_TYPE_WITH_CODE,
            {"input": _reschedule_appointment_type_input(code)},
        )

        assert "errors" not in data or not data.get("errors"), data
        result = data["data"]["rescheduleAppointmentTypeEventWithCode"]
        ...
        assert returned_event_id == appointment_type_event.id
        ...
        assert appointment_type_event.end_time_tz_unaware.replace(tzinfo=None) == NEW_END.replace(
            tzinfo=None
        )

        # Non-primary BlockedTime must also be updated.
        bt = _get_blocked_time(organization, appointment_type_event.id, secondary_calendar.id)
    ...
    @patch("public_api.extensions.OrganizationRateLimiter.on_execute")
    def test_event_is_still_appointment_type_after_reschedule(
        self,
        mock_rate_limiter,
        anon_client,
        appointment_type_reschedule_code,
        ...
        """The rescheduled event still has appointment_type_fk set."""
        mock_rate_limiter.return_value = iter([None])
        ...
        appointment_type_event.refresh_from_db()
        assert appointment_type_event.appointment_type_fk_id == appointment_type.id
    ...
    def test_title_description_and_attendees_preserved(
        ...
        assert result["event"]["title"] == "Original AppointmentType Title"
        ...
        assert appointment_type_event.description == "Original description."
...
@pytest.mark.django_db
class TestRescheduleAppointmentTypeWithCodeReplay:
    ...
    def test_replay_returns_already_used(
        ...
        mock_rate_limiter.return_value = iter([None])
        _token, code = appointment_type_reschedule_code
        input_data = _reschedule_appointment_type_input(code)

        # First call — must succeed.
        first = post_graphql(
            anon_client, RESCHEDULE_APPOINTMENT_TYPE_WITH_CODE, {"input": input_data}
        )
        assert first["data"]["rescheduleAppointmentTypeEventWithCode"]["success"] is True

        # Second call — same code must return ALREADY_USED.
        second = post_graphql(
            anon_client, RESCHEDULE_APPOINTMENT_TYPE_WITH_CODE, {"input": input_data}
        )
        result = second["data"]["rescheduleAppointmentTypeEventWithCode"]
...
@pytest.mark.django_db
class TestRescheduleAppointmentTypeWithCodeSlotUnavailable:
    ...
    def test_out_of_window_slot_does_not_consume_code(
        ...
        data = post_graphql(
            ...
            RESCHEDULE_APPOINTMENT_TYPE_WITH_CODE,
            {
                "input": _reschedule_appointment_type_input(
                    code,
    ...
    def test_after_failed_reschedule_code_can_succeed(
        ...
        assert (
            fail["data"]["rescheduleAppointmentTypeEventWithCode"]["errorCode"]
            == "SLOT_UNAVAILABLE"
        )
        token.refresh_from_db()
        assert token.used_at is None
...
@pytest.mark.django_db
class TestRescheduleAppointmentTypeWithCodeCalendarScopedCode:
    """Scenario 4: Calendar-scoped reschedule code (no appointment type) → NOT_PERMITTED."""
...
@pytest.mark.django_db
class TestRescheduleAppointmentTypeWithCodeWrongPermission:
...
@pytest.mark.django_db
class TestRescheduleAppointmentTypeWithCodeLifecycleRejections:
    ...
    def test_invalid_code_returns_invalid_code(
        ...
        data = post_graphql(
            anon_client,
            RESCHEDULE_APPOINTMENT_TYPE_WITH_CODE,
            {"input": _reschedule_appointment_type_input("aW52YWxpZGdyb3VwcmVzY2hlZHVsZQ==")},
        )
...
@pytest.mark.django_db
class TestRescheduleAppointmentTypeWithCodeNonUTCTimezone:
    """Non-UTC regression: primary event and linked BlockedTimes derive the same
    UTC instant after a reschedule in a non-UTC zone.

    The test builds the appointment-type event via the real AppointmentTypeService
    (``create_appointment_type_event``) so that the BlockedTime rows are written by the
    ...
    def test_non_utc_reschedule_aligns_primary_and_blocked_times(
        ...
        # --- Build appointment type (PRIVATE) + availability ---------------------------------
        # The appointment type is private: only an appointment-type-scoped code authorizes scheduling.
        # The reschedule code is what we are testing — not the creation gate.
        grp = baker.make(
            AppointmentType,
            organization=org,
            name="Recife AppointmentType",
            accepts_public_scheduling=False,
        )
        slot_a = AppointmentTypeSlot.objects.create(
            organization=org, appointment_type=grp, name="Physicians", order=0, required_count=1
        )
        slot_b = AppointmentTypeSlot.objects.create(
            organization=org, appointment_type=grp, name="Rooms", order=1, required_count=1
        )
        AppointmentTypeSlotMembership.objects.create(
        ...
        # --- Create the appointment-type event via the real service -------------------------
        # Wire a permission service with an appointment-type-scoped CREATE token so that the
        # appointment-type-level authorization gate passes for this private appointment type.  The setup
        ...
        bt = BlockedTime.objects.filter_by_organization(org.id).get(
            external_id=f"appointment-type-event-{event.id}-cal-{secondary_cal.id}"
        )
        bt.refresh_from_db()
