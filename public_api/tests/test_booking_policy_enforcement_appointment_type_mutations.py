"""Integration tests for appointment type booking policy enforcement at the mutation layer.

Coverage:
- ``createAppointmentTypeEvent`` (authenticated path):
...
- ``createAppointmentTypeEventWithCode`` (code-gated path):
...
def _make_appointment_type_and_slot(
    org: Organization, calendar: Calendar
) -> tuple[AppointmentType, AppointmentTypeSlot]:
    """Create a single-slot appointment type containing *calendar* and return (appointment type, slot).

    duration matches ``_START``/``_END`` (1 hour) below -- a public appointment type with
    ...
    slot = AppointmentTypeSlot.objects.create(
        organization=org, appointment_type=appointment_type, name="Main", order=0
    )
    AppointmentTypeSlotMembership.objects.create(organization=org, slot=slot, calendar=calendar)
    return appointment_type, slot


def _real_deps(org: Organization) -> AppointmentTypeMutationDependencies:
    """Build real (non-mocked) AppointmentTypeMutationDependencies with policy support."""
...
def _invoke_mutation(
    ...
    calendar: Calendar,
    deps: AppointmentTypeMutationDependencies,
) -> object:
    """Invoke the createAppointmentTypeEvent mutation directly (no HTTP).
    ...
    ``calendar_integration/mutations.py``'s ``create_appointment_type_event``),
    ...
    input_data = AppointmentTypeEventInput(
        ...
        timezone="UTC",
        slot_selections=[
            AppointmentTypeSlotSelectionInput(slot_id=slot.id, calendar_ids=[calendar.id])
        ],
    )
    mock_info = Mock()
    ...
    with patch(
        ...
        return mutations.create_appointment_type_event(mock_info, input_data)  # type: ignore[arg-type]


# ---------------------------------------------------------------------------
# Tests — createAppointmentTypeEvent (authenticated path)
# ---------------------------------------------------------------------------


@pytest.mark.django_db
class TestCreateAppointmentTypeEventPolicyEnforcement:
    """Policy enforcement via the authenticated ``createAppointmentTypeEvent`` mutation."""
    ...
    def test_no_policy_write_unchanged(self):
        ...
        appointment_type, slot = _make_appointment_type_and_slot(org, cal)
        deps = _real_deps(org)

        result = _invoke_mutation(org, appointment_type, slot, cal, deps)
    ...
    def test_lead_time_violation_returns_error_message(self):
        ...
        create_booking_policy(appointment_type=appointment_type, lead_time_seconds=lead_s)
        deps = _real_deps(org)

        with patch("calendar_integration.services.appointment_type_service.timezone") as mock_tz:
    ...
    def test_horizon_violation_returns_error_message(self):
        ...
        create_booking_policy(appointment_type=appointment_type, max_horizon_seconds=horizon_s)
        deps = _real_deps(org)
    ...
    def test_buffer_violation_returns_error_message(self):
        ...
        assert (
            ...
            .filter(title="Test AppointmentType Booking")
            .count()
            == 0
        )
        assert (
...
# ---------------------------------------------------------------------------
# Tests — createAppointmentTypeEventWithCode (code-gated path)
# ---------------------------------------------------------------------------
...
def _appointment_type_booking_code(
    org: Organization, appointment_type: AppointmentType
) -> tuple[object, str]:
    """Mint an appointment type booking code for the given appointment type."""
...
def _post_appointment_type_code_gated(variables: dict) -> dict:
    with _patch("public_api.extensions.OrganizationRateLimiter.on_execute") as mock_rl:
        mock_rl.return_value = iter([None])
        client = APIClient()
        response = client.post(
            "/graphql/",
            data={"query": _CREATE_APPOINTMENT_TYPE_EVENT_WITH_CODE, "variables": variables},
            format="json",
        )
...
def _appointment_type_code_input(
    code: str,
    appointment_type: AppointmentType,
    slot: AppointmentTypeSlot,
    calendar: Calendar,
    **overrides: object,
    ...
    base: dict = {
        "code": code,
        "title": "Code AppointmentType Booking",
        "description": "",
...
@pytest.mark.django_db
class TestCreateAppointmentTypeEventWithCodePolicyEnforcement:
    """Policy enforcement via the code-gated ``createAppointmentTypeEventWithCode`` mutation."""

    def test_no_policy_booking_succeeds_off_state(self):
        """Off-state: no BookingPolicy → code-gated appointment type write behaves exactly as before."""
        org = _org("code-off-state")
        cal = _managed_cal(org)
        appointment_type, slot = _make_appointment_type_and_slot(org, cal)
        token_obj, code = _appointment_type_booking_code(org, appointment_type)

        data = _post_appointment_type_code_gated(
            variables={"input": _appointment_type_code_input(code, appointment_type, slot, cal)}
        )
    ...
    def test_compliant_code_gated_booking_succeeds(self):
        """A code-gated appointment type booking satisfying all policy rules succeeds."""
