"""Integration tests for the public GraphQL surface of appointment-type-scoped quota
rules.

Direct mirror of ``test_appointment_type_scoped_blocked_times.py`` for quota rules,
minus the recurrence/reason machinery (quota rules are non-recurring and
have no time range -- just a period and a cap). Covers
``appointmentTypeScopedQuotaRules`` (query) and ``batchUpsertAppointmentTypeScopedQuotaRules``
...
APPOINTMENT_TYPE_SCOPED_QUOTA_RULES_QUERY = """
query AppointmentTypeScopedQuotaRules($appointmentTypeSlotId: Int!, $calendarId: Int) {
    appointmentTypeScopedQuotaRules(appointmentTypeSlotId: $appointmentTypeSlotId, calendarId: $calendarId) {
        id
        calendarId
...
BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_QUOTA_RULES_MUTATION = """
mutation BatchUpsertAppointmentTypeScopedQuotaRules($input: BatchAppointmentTypeScopedQuotaRulesInput!) {
    batchUpsertAppointmentTypeScopedQuotaRules(input: $input) {
        success
        errorMessage
        quotaRules {
            id
...
class TestAppointmentTypeScopedQuotaRulesPublicAPI:
    ...
    def _make_appointment_type_slot(
        self, org: Organization, *calendars: Calendar
    ) -> AppointmentTypeSlot:
        appointment_type = AppointmentType.objects.create(organization=org, name="Surgery")
        slot = AppointmentTypeSlot.objects.create(
            organization=org, appointment_type=appointment_type, name="Lead"
        )
    ...
    # ------------------------------------------------------------------
    # appointmentTypeScopedQuotaRules -- query
    # ------------------------------------------------------------------

    def test_query_returns_appointment_type_scoped_quota_rules_for_the_slot(self):
        org = self._setup_org()
        calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar)
        ...
        response = self._post(
            ...
            token,
            auth_service,
            {"appointmentTypeSlotId": slot.id, "calendarId": None},
        )

        assert response.status_code == 200
        data = response.json()
        assert data.get("errors", []) == []
        rules = data["data"]["appointmentTypeScopedQuotaRules"]
        assert len(rules) == 1
        assert rules[0]["id"] == rule.id
        assert rules[0]["calendarId"] == calendar.id
        assert rules[0]["appointmentTypeSlotId"] == slot.id
        assert rules[0]["period"] == QuotaPeriod.WEEK
        assert rules[0]["cap"] == 3

    def test_query_cross_organization_scoping_returns_empty(self):
        """An appointment type slot belonging to another organization is invisible."""
        org_a = self._setup_org()
        org_b = self._setup_org()
        calendar_b = self._make_calendar(org_b)
        slot_b = self._make_appointment_type_slot(org_b, calendar_b)

        AppointmentTypeSlotQuotaRule.objects.create(
            organization=org_b,
            calendar=calendar_b,
            appointment_type_slot=slot_b,
            period=QuotaPeriod.WEEK,
            cap=3,
        )

        system_user, token, auth_service = self._make_org_wide_system_user(
            org_a, [PublicAPIResources.APPOINTMENT_TYPE_SCOPED_QUOTA_RULES]
        )
        ...
        response = self._post(
            ...
            token,
            auth_service,
            {"appointmentTypeSlotId": slot_b.id, "calendarId": None},
        )

        assert response.status_code == 200
        data = response.json()
        assert data.get("errors", []) == []
        assert data["data"]["appointmentTypeScopedQuotaRules"] == []

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- apply
    # ------------------------------------------------------------------
    ...
    def test_batch_upsert_applies_mixed_operations(self):
        ...
        slot = self._make_appointment_type_slot(org, calendar)
        system_user, token, auth_service = self._make_org_wide_system_user(
            org, [PublicAPIResources.BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_QUOTA_RULES]
        )

        existing = AppointmentTypeSlotQuotaRule.objects.create(
            organization=org,
            calendar=calendar,
            appointment_type_slot=slot,
            period=QuotaPeriod.WEEK,
        ...
        to_delete = AppointmentTypeSlotQuotaRule.objects.create(
            organization=org,
            calendar=calendar,
            appointment_type_slot=slot,
        ...
        data = response.json()
        assert data.get("errors", []) == []
        result = data["data"]["batchUpsertAppointmentTypeScopedQuotaRules"]
        ...
        assert not (
            AppointmentTypeSlotQuotaRule.objects.filter_by_organization(org.id)
            .filter(id=to_delete.id)
            .exists()
        )

        remaining = AppointmentTypeSlotQuotaRule.objects.for_appointment_type_slot(
            slot.id
        ).filter_by_organization(org.id)
        assert remaining.count() == 2

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- idempotent replay
    # ------------------------------------------------------------------
    ...
    def test_identical_replay_is_a_no_op(self):
        ...
        first_result = first.json()["data"]["batchUpsertAppointmentTypeScopedQuotaRules"]
        ...
        second_result = second.json()["data"]["batchUpsertAppointmentTypeScopedQuotaRules"]
    ...
    def test_cap_participates_in_the_idempotent_create_match_key(self):
        ...
        appointment type slot, period, cap). A ``create`` op naming the SAME period but
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- atomicity on mid-batch failure
    # ------------------------------------------------------------------
    ...
    def test_batch_atomicity_rolls_back_all_ops_on_later_collision(self):
        ...
        assert (
            AppointmentTypeSlotQuotaRule.objects.filter_by_organization(org.id)
            .filter(calendar_fk_id=calendar.id, appointment_type_slot_fk_id=slot.id)
            .count()
            == 0
        ), "Batch atomicity: all operations must be rolled back on mid-batch collision."

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- uniqueness -> validation error
    # ------------------------------------------------------------------
    ...
    def test_update_colliding_with_existing_period_is_surfaced_as_a_clean_failure(self):
        ...
        day_rule = AppointmentTypeSlotQuotaRule.objects.create(
            organization=org,
            calendar=calendar,
            appointment_type_slot=slot,
            period=QuotaPeriod.DAY,
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- invalid period value
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- RESTRICTED billing root still blocks
    # ------------------------------------------------------------------
    ...
    def test_restricted_organization_batch_rejected_wholesale(self):
        ...
        assert (
            AppointmentTypeSlotQuotaRule.objects.for_appointment_type_slot(slot.id)
            .filter_by_organization(org.id)
            .count()
            == 0
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- cross-organization scoping
    # ------------------------------------------------------------------

    def test_cross_organization_appointment_type_slot_is_rejected(self):
        org_a = self._setup_org()
        org_b = self._setup_org()
        calendar_b = self._make_calendar(org_b)
        slot_b = self._make_appointment_type_slot(org_b, calendar_b)
        ...
        system_user, token, auth_service = self._make_org_wide_system_user(
            org_a, [PublicAPIResources.BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_QUOTA_RULES]
        )
        ...
        response = self._post(
            ...
            auth_service,
            {
                "input": {
                    ...
                    "appointmentTypeSlotId": slot_b.id,
                    "operations": [
                        {
                            "action": "create",
        ...
        assert result["quotaRules"] == []
        assert (
            not AppointmentTypeSlotQuotaRule.objects.for_appointment_type_slot(slot_b.id)
    ...
    def test_cross_owner_scoped_token_rejected_wholesale(self):
        """A scoped token may not write appointment-type-scoped quota rules on a
        ...
        slot = self._make_appointment_type_slot(org, calendar_b)

        system_user_a, token_a, auth_service_a = self._make_scoped_system_user(
            org, membership_a, [PublicAPIResources.BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_QUOTA_RULES]
        )
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- IDOR (rule/calendar cross-check
    # on update/delete)
    # ------------------------------------------------------------------
    ...
    def test_update_with_rule_from_another_calendar_rejected_wholesale(self):
        ...
        _owner_b, _membership_b, calendar_b = self._make_owner_with_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar_a, calendar_b)

        foreign_rule = AppointmentTypeSlotQuotaRule.objects.create(
            organization=org,
            calendar=calendar_b,
            appointment_type_slot=slot,
            period=QuotaPeriod.WEEK,
        ...
        result = data["data"]["batchUpsertAppointmentTypeScopedQuotaRules"]
        assert result["success"] is False
        assert result["errorMessage"] == "AppointmentType slot not found."
        assert result["quotaRules"] == []
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedQuotaRules -- create outside the slot's roster
    # ------------------------------------------------------------------
    ...
    def test_create_with_calendar_outside_slot_roster_rejected_wholesale(self):
        ...
        it is not a member of the target appointmentTypeSlotId's roster -- rejected with
        the not-found shape, nothing created."""
        org = self._setup_org()
        roster_calendar = self._make_calendar(org)
        outside_calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, roster_calendar)
    ...
    def test_query_anonymous_request_denied(self):
        ...
        response = self.client.post(
            "/graphql/",
            data={
                "query": APPOINTMENT_TYPE_SCOPED_QUOTA_RULES_QUERY,
                "variables": {"appointmentTypeSlotId": slot.id, "calendarId": None},
            },
            format="json",
        )
    ...
    def test_mutation_anonymous_request_denied(self):
        ...
        response = self.client.post(
            "/graphql/",
            data={
                "query": BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_QUOTA_RULES_MUTATION,
                "variables": {
                    "input": {
                        "organizationId": org.id,
                        "appointmentTypeSlotId": slot.id,
    ...
    def test_query_token_without_resource_grant_denied(self):
        """An authenticated token that lacks the APPOINTMENT_TYPE_SCOPED_QUOTA_RULES
    ...
    def test_mutation_token_without_resource_grant_denied(self):
        ...
        BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_QUOTA_RULES resource grant is denied, and
        nothing is written."""
        org = self._setup_org()
        calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar)
        # Grant an unrelated resource only.
        system_user, token, auth_service = self._make_org_wide_system_user(
            org, [PublicAPIResources.APPOINTMENT_TYPE_SCOPED_QUOTA_RULES]
        )
    ...
    def test_existing_appointment_type_scoped_blocked_times_query_shape_unchanged(self):
        """Byte-for-byte shape check: the pre-existing appointmentTypeScopedBlockedTimes
        query's response is unaffected by the appointment-type-scoped additions."""
        org = self._setup_org()
        calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar)
        system_user, token, auth_service = self._make_org_wide_system_user(
            org, [PublicAPIResources.APPOINTMENT_TYPE_SCOPED_BLOCKED_TIMES]
        )
        ...
        query = """
        query AppointmentTypeScopedBlockedTimes($appointmentTypeSlotId: Int!, $calendarId: Int) {
            appointmentTypeScopedBlockedTimes(appointmentTypeSlotId: $appointmentTypeSlotId, calendarId: $calendarId) {
                id
                calendarId
                appointmentTypeSlotId
                startTime
        ...
        data = response.json()
        assert data.get("errors", []) == []
        blocks = data["data"]["appointmentTypeScopedBlockedTimes"]
        assert len(blocks) == 1
