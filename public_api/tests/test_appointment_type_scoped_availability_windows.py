...
Covers ``appointmentTypeScopedAvailabilityWindows`` (query) and
``batchUpsertAppointmentTypeScopedAvailabilityWindows`` (batch-upsert mutation):
...
APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS_QUERY = """
...
class TestAppointmentTypeScopedAvailabilityWindowsPublicAPI:
    ...
    # ------------------------------------------------------------------
    # appointmentTypeScopedAvailabilityWindows -- query
    # ------------------------------------------------------------------

    def test_query_returns_appointment_type_scoped_windows_for_the_slot(self):
        org = self._setup_org()
        ...
        assert windows[0]["appointmentTypeSlotId"] == slot.id
    ...
    def test_query_cross_organization_scoping_returns_empty(self):
        ...
        system_user, token, auth_service = self._make_org_wide_system_user(
            org_a, [PublicAPIResources.APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS]
        )
        ...
        data = response.json()
        assert data.get("errors", []) == []
        assert data["data"]["appointmentTypeScopedAvailabilityWindows"] == []

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedAvailabilityWindows -- apply
    # ------------------------------------------------------------------
    ...
    def test_batch_upsert_applies_mixed_operations(self):
        ...
        remaining = AvailableTime.objects.for_appointment_type_slot(slot.id).filter_by_organization(
            org.id
        )
        assert remaining.count() == 2

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedAvailabilityWindows -- idempotent replay
    # ------------------------------------------------------------------
    ...
    def test_identical_replay_is_a_no_op(self):
        ...
        first_result = first.json()["data"]["batchUpsertAppointmentTypeScopedAvailabilityWindows"]
        ...
        second_result = second.json()["data"]["batchUpsertAppointmentTypeScopedAvailabilityWindows"]
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedAvailabilityWindows -- over-limit rejects whole batch
    # ------------------------------------------------------------------
    ...
    def test_over_limit_batch_rejected_whole_with_same_body_as_base_batch_write(self):
        ...
        appointment_type_calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, appointment_type_calendar)
        system_user, token, auth_service = self._make_org_wide_system_user(
            org,
            [
        ...
        # --- New appointment-type-scoped batch write, same ceiling, same net growth. ---
        appointment_type_create_ops = [
            {
                "action": "create",
                "calendarId": appointment_type_calendar.id,
                "startTime": datetime.datetime(
        ...
        appointment_type_response = self._post(
            BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS_MUTATION,
            system_user,
            token,
            auth_service,
            {
                "input": {
                    "organizationId": org.id,
                    "appointmentTypeSlotId": slot.id,
                    "operations": appointment_type_create_ops,
                }
            },
        )
        assert appointment_type_response.status_code == 200
        appointment_type_data = appointment_type_response.json()
        assert len(appointment_type_data["errors"]) == 1
        appointment_type_extensions = appointment_type_data["errors"][0]["extensions"]
        ...
        assert appointment_type_extensions == base_extensions
        assert appointment_type_extensions["code"] == "limit_exceeded"
        assert appointment_type_extensions["resource"] == AVAILABILITY_WINDOWS
        ...
        assert (
            ...
            .count()
            == 0
        ), "The over-limit appointment-type-scoped batch must create nothing."

    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedAvailabilityWindows -- cross-organization scoping
    # ------------------------------------------------------------------
    ...
    def test_cross_organization_appointment_type_slot_is_rejected(self):
        ...
        system_user, token, auth_service = self._make_org_wide_system_user(
            org_a, [PublicAPIResources.BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS]
        )
        ...
        assert (
            not AvailableTime.objects.for_appointment_type_slot(slot_b.id)
    ...
    def test_cross_owner_scoped_token_rejected_wholesale(self):
        """A scoped token may not write appointment-type-scoped windows on a calendar it
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedAvailabilityWindows -- IDOR (window/calendar
    ...
    # ------------------------------------------------------------------
    # batchUpsertAppointmentTypeScopedAvailabilityWindows -- create outside the slot's roster
    # ------------------------------------------------------------------
    ...
    def test_query_anonymous_request_denied(self):
        ...
        response = self.client.post(
            "/graphql/",
            data={
                "query": APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS_QUERY,
    ...
    def test_mutation_anonymous_request_denied(self):
        ...
        response = self.client.post(
            "/graphql/",
            data={
                "query": BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS_MUTATION,
                "variables": {
                    "input": {
                        "organizationId": org.id,
                        "appointmentTypeSlotId": slot.id,
    ...
    def test_query_token_without_resource_grant_denied(self):
        """An authenticated token that lacks the APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS
    ...
    def test_mutation_token_without_resource_grant_denied(self):
        ...
        BATCH_UPSERT_APPOINTMENT_TYPE_SCOPED_AVAILABILITY_WINDOWS resource grant is denied,
        and nothing is written."""
        org = self._setup_org()
        calendar = self._make_calendar(org)
        slot = self._make_appointment_type_slot(org, calendar)
    ...
    def test_existing_batch_update_availability_windows_response_shape_unchanged(self):
        ...
        mutation's response is unaffected by the appointment-type-scoped additions."""
        org = self._setup_org()
        calendar = self._make_calendar(org)
        system_user, token, auth_service = self._make_org_wide_system_user(
        ...
        # from before the appointment-type-scoped additions.
