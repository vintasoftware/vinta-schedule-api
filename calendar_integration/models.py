...
class RecurringMixin(SingleOrganizationModelMixin, SafeRelationNullInitMixin, BaseModel):
    ...
    def _get_occurrences_in_range(
        ...
        if hasattr(self, "recurring_occurrences"):
            ...
            occurrences = (
                self.__class__.objects.filter_by_organization(self.organization_id)
                .annotate_recurring_occurrences_on_date_range(  # type: ignore
...
class ExternalEventChangeRequest(
    ...
    # be a single OrganizationSafeForeignKey.  They are declared manually here because
    # OrganizationSafeForeignKey forwards ``on_delete`` to *both* the concrete FK and
    # the ForeignObject.  With SET_NULL, Django's cascade collector would try to null the
...
class BookingPolicy(SingleOrganizationModelMixin, SafeRelationNullInitMixin, BaseModel):
    ...
    ``OrganizationSafeForeignKey`` relations) CASCADEs: the policy is removed and
