...
appointment-type-scoped reads take ``token.appointment_type``, falling back to
``token.event.appointment_type``. A code resolving to neither raises the
uniform ``OpaqueCodeError`` -- this is also what naturally rejects a
single-calendar code on the two appointment-type-scoped endpoints (their ``appointment_type``
resolves to ``None``). It is NOT enough, however, to reject an appointment-type-scoped
code on the calendar-scoped reads: ``AppointmentTypeService.create_appointment_type_event``
...
to an appointment type -- naively falling through to it would leak that specific
calendar's availability to a patient holding only an appointment type code. So each
resolver first checks the token's OWN scope column
(``appointment_type_fk_id`` / ``calendar_fk_id``) and raises immediately if it
belongs to the OTHER scope, before ever consulting the ``event`` fallback.
See ``_resolve_calendar_scope_opaquely`` / ``_resolve_appointment_type_scope_opaquely``.

**Pinned duration** (the two bookable-slots endpoints only): duration pinning
lives on ``AppointmentType.duration``, not on the token -- a calendar-scoped
...
appointment-type-scoped code's appointment type pins a duration, so that status alone cannot
disclose pin state. Once present, on the appointment-type-scoped endpoint an appointment type that
...
**Phase 9 addendum -- codeless, slug-addressed public-appointment-type reads.** Below
...
reads for an ``AppointmentType`` whose ``accepts_public_scheduling`` is
...
see the "Codeless, slug-addressed public-appointment-type reads" section below for why,
and do not conflate the two. They mirror the codeless branch of
``BookingCodeAppointmentTypeEventViewSet.create`` in ``booking_views.py`` exactly.
"""
...
from calendar_integration.models import AppointmentType, Calendar, CalendarManagementToken
...
def _resolve_calendar_scope_opaquely(token: CalendarManagementToken) -> Calendar:
    ...
    This is also what rejects an appointment-type-scoped code on the calendar-scoped
    reads -- a token scoped to an appointment type (never a calendar) resolves to
    ``None`` here, same as any other invalid scope.

    Guard: a token that is itself scoped to an appointment TYPE (``appointment_type``
    set on the token) must never fall through to ``token.event.calendar`` --
    for an appointment type booking, ``event.calendar`` is always the specific staff
    member's underlying calendar the event landed on, and disclosing it here
    would defeat the point of appointment type scoping. That fallback is only for a
    token that carries no appointment type scope of its own.
    """
...
def _resolve_appointment_type_scope_opaquely(token: CalendarManagementToken) -> AppointmentType:
    """Resolve the appointment type bound to ``token`` (``token.appointment_type``
    first, then ``token.event.appointment_type``), or raise the uniform 403.

    This is also what rejects a single-calendar code on the appointment-type-scoped
    reads -- a token scoped to a single calendar (never an appointment type) resolves to
    ``None`` here, same as any other invalid scope.
...
def _resolve_duration(
    appointment_type: AppointmentType | None, request: Request
) -> datetime.timedelta:
    """Resolve the search duration for a bookable-slots read.

    ``appointment_type`` is the token's own resolved ``AppointmentType`` on the
    appointment-type-scoped endpoint, or ``None`` on the calendar-scoped endpoint --
    ...
    alone never discloses whether a resolved appointment type pins a duration. Only
    once presence is established does the pin take over: when ``appointment_type`` is
    set and ``appointment_type.duration`` is set, it is returned UNCONDITIONALLY and the
    ...
    reads" Guiding Decision). Only when there is no pin (no appointment type, or an appointment type
    ...
    if appointment_type is not None and appointment_type.duration is not None:
        return appointment_type.duration
...
_DURATION_SECONDS_PARAMETER = OpenApiParameter(
    ...
    "(a request omitting it is a 400 whether or not the resolved code's appointment type pins "
    "a duration -- presence alone must never disclose pin state). Duration pinning "
    "lives on the AppointmentType, not on the code: on the calendar-scoped endpoint "
    "this value always stands as given -- a calendar-scoped code carries no "
    "duration constraint at all. On the appointment-type-scoped endpoint, when the resolved "
    "code's appointment type pins a duration, the pin silently overrides this parameter's "
    "VALUE: a mismatched or malformed value produces the exact same response as "
    "the pinned value, so this endpoint cannot be used to probe whether an appointment type is "
    "pinned or what the pin is. When the appointment type pins no duration, the value must "
...
# ---------------------------------------------------------------------------
# Appointment-type-scoped reads
# ---------------------------------------------------------------------------


@extend_schema(tags=["Booking Codes"])
class BookingCodeAppointmentTypeBookableSlotsViewSet(BookingCodeViewMixin, GenericViewSet):
    ...
    @extend_schema(
        ...
        summary="Bookable slot proposals for the appointment type bound to a booking code",
    )
    def list(self, request: Request, *args, **kwargs) -> Response:
        permission_service = self.calendar_permission_service
        appointment_type_service = self.appointment_type_service
        if permission_service is None or appointment_type_service is None:
            raise CalendarServiceNotInjectedError(
                "calendar_permission_service / appointment_type_service not configured; "
                "check the DI container."
            )
        ...
        token = resolve_booking_code_opaquely(request, permission_service)
        appointment_type = _resolve_appointment_type_scope_opaquely(token)
        org = _resolve_org_opaquely(token)

        # --- pinned-duration silent override -- see module docstring / _resolve_duration. ---
        duration = _resolve_duration(appointment_type, request)
        slot_step = _resolve_slot_step(request)
...
@extend_schema(tags=["Booking Codes"])
class BookingCodeAppointmentTypeAvailabilityViewSet(BookingCodeViewMixin, GenericViewSet):
    ...
    serializer_class = AppointmentTypeAvailabilityQuerySerializer
    # No natural model-bound queryset (scope comes from the resolved token, not a
    # URL object) -- set purely to silence drf-spectacular's "Failed to obtain
    # model through view's queryset" schema-generation warning.
    queryset = AppointmentType.objects.none()
    ...
    @extend_schema(
        ...
        summary="Per-range availability for the appointment type bound to a booking code",
    )
...
# ---------------------------------------------------------------------------
# Codeless, slug-addressed public-appointment-type reads (Phase 9)
# ---------------------------------------------------------------------------
#
...
# none is read. The appointment type instead comes from the path's `public_slug` --
# the client's OWN input, exactly like the codeless branch of
# `BookingCodeAppointmentTypeEventViewSet.create` in `booking_views.py`, which these
...
#   read's token-bound scope), so disclosing "no such appointment type" tells the
#   caller nothing it did not already know -- it had to have the slug to
#   ask in the first place.
# - A slug that resolves to a real, but non-public
#   (`accepts_public_scheduling=False`), appointment type -> `403`. The appointment type exists,
#   but this route is not open to it.
#
# Do NOT reuse `resolve_booking_code_opaquely` / `_resolve_appointment_type_scope_opaquely`
# here -- those exist for the opposite contract (one indistinguishable 403,
# never a 404, because a CODE's scope must stay secret). An appointment-type-scoped read
...
# surface; every read below stays strictly appointment-type-level.
#
...
# appointment-type-aggregated" Guiding Decision. That decision requires every appointment type
# read here to never attribute an interval to a specific member calendar.
# `find_bookable_slots` and `check_appointment_type_availability` (used below) already
...
# equivalent appointment-type-level primitive for a continuous availability/busy
...
# pool is free" would report the whole appointment type busy solely because ONE
...
# appointment-type-aggregated reads below and stops there.


def _resolve_public_appointment_type(public_slug: str) -> AppointmentType:
    """Resolve a codeless discovery read's addressed, public-scheduling appointment type.

    Two-step resolution, mirroring ``BookingCodeAppointmentTypeEventViewSet.create``'s
    ...
    2. Require ``accepts_public_scheduling``. A slug that resolves to a
       real, but non-public, appointment type is a real ``403``: the appointment type exists, but
       this route is not open to it.

    Does NOT check ``appointment_type.duration`` -- callers that need the search
    duration call ``_resolve_public_appointment_type_duration`` next, which folds in
    ...
    if not appointment_type.accepts_public_scheduling:
        raise NotPermittedAPIException("This appointment type does not accept public scheduling.")
    return appointment_type


def _resolve_public_appointment_type_duration(
    ...
    the only length a codeless booking can ever have is the appointment type's own
    ...
    appointment type with no duration configured is a grandfathered misconfiguration
    (Phase 0's invariant: ``AppointmentTypeService.create_appointment_type`` /
    ``update_appointment_type`` refuse to set ``accepts_public_scheduling=True`` without
    ...
    names its own ranges), but an appointment type the write side would refuse
    ...
    explicitly forbid.
    """
    if appointment_type.duration is None:
        raise NotPermittedAPIException("This appointment type does not accept public scheduling.")
    return appointment_type.duration


@extend_schema(tags=["Booking Codes"])
class PublicAppointmentTypeBookableSlotsViewSet(BookingCodeViewMixin, GenericViewSet):
    ...
    @extend_schema(
        ...
        summary="Bookable slot proposals for a public appointment type, no code required",
    )
    def list(self, request: Request, *args, **kwargs) -> Response:
        appointment_type_service = self.appointment_type_service
        if appointment_type_service is None:
            raise CalendarServiceNotInjectedError(
                "appointment_type_service not configured; check the DI container."
            )
        ...
        appointment_type = _resolve_public_appointment_type(kwargs["public_slug"])
        duration = _resolve_public_appointment_type_duration(appointment_type)
        slot_step = _resolve_slot_step(request)
...
@extend_schema(tags=["Booking Codes"])
class PublicAppointmentTypeAvailabilityViewSet(BookingCodeViewMixin, GenericViewSet):
    ...
    @extend_schema(
        request=AppointmentTypeAvailabilityQuerySerializer,
        responses={200: AppointmentTypeRangeAvailabilitySerializer(many=True)},
        summary="Per-range availability for a public appointment type, no code required",
    )
    def create(self, request: Request, *args, **kwargs) -> Response:
        appointment_type_service = self.appointment_type_service
        if appointment_type_service is None:
        ...
        # BookingCodeAppointmentTypeAvailabilityViewSet.create above. ---
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        ranges = [(r["start_time"], r["end_time"]) for r in serializer.validated_data["ranges"]]
        for start_time, end_time in ranges:
        ...
        appointment_type = _resolve_public_appointment_type(kwargs["public_slug"])
        # check_appointment_type_availability does not itself consume a duration value,
        # but an appointment type the write side would refuse regardless of the
        # requested span should not report itself as available either -- see
        # _resolve_public_appointment_type_duration's own docstring for why this gate
        # applies uniformly to both endpoints in this section.
        _resolve_public_appointment_type_duration(appointment_type)
