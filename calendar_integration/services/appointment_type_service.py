...
from calendar_integration.services.dataclasses import (
    ...
    AppointmentTypeScopedAvailabilityWriteResult,
    AppointmentTypeScopedBlockWriteResult,
...
def _time_range_fully_covered(
    ...
    calendar's appointment-type-scoped availability configuration after a write.
...
class AppointmentTypeService:
    ...
    def _audit_appointment_type_write(
        self,
        action: str,
        subject_instance: Model,
        diff: dict | None = None,
    ) -> None:
        """Emit an audit record for an appointment-type business write.
    ...
    def initialize(self, organization: Organization) -> None:
        ...
        (i.e. `create_appointment_type_event`), the caller must also separately initialize
        or authenticate `self.calendar_service` with the same organization — the
        appointment-type-event flow needs external-provider adapters if any of the
    ...
    def _assert_initialized(self) -> None:
        if self.organization is None:
            raise CalendarServiceOrganizationNotSetError(
                "AppointmentTypeService requires an organization. Call initialize()."
            )
    ...
    def _is_quota_uniqueness_constraint_violation(e: IntegrityError) -> bool:
        """Check if an IntegrityError is the (appointment_type_slot, calendar, period) unique
        constraint violation on AppointmentTypeSlotQuotaRule.
        ...
        """
        constraint_name = "appointmenttypeslotquotarule_unique_slot_calendar_period"
    ...
    def _get_appointment_type_by_id(self, appointment_type_id: int) -> AppointmentType:
        self._assert_initialized()
        return AppointmentType.objects.filter_by_organization(self.organization_id).get(
            id=appointment_type_id
        )

    def _resolve_effective_pool_ids(
        self,
        slots: list[AppointmentTypeSlotInputData],
    ) -> dict[str, list[int]]:
        ...
        `update_appointment_type` call that never touched `pool_ids` at all, with an
        ...
        `AppointmentTypeSlotPool` / `CalendarPool` query at all.
        """
        return {
    ...
    def _pool_rosters(self, pool_ids: Iterable[int]) -> dict[int, set[int]]:
        ...
        if missing:
            raise AppointmentTypeValidationError(
    ...
    def _validate_slots_input(
        self,
        slots: Iterable[AppointmentTypeSlotInputData],
        existing_slots: dict[str, AppointmentTypeSlot] | None = None,
    ) -> tuple[list[AppointmentTypeSlotInputData], set[int]]:
        ...
        for slot_data in slots:
            if slot_data.name in seen_slot_names:
                raise AppointmentTypeValidationError(f"Duplicate slot name: {slot_data.name!r}.")
            ...
            # A calendar may belong to at most one slot per appointment type. Availability and
            # bookable-slot computation count each slot's pool independently, so an
            # overlapping calendar would be double-counted and the appointment type reported
            ...
            for cid in sorted(effective_calendar_ids):
                ...
                if other_slot is not None and other_slot != slot_data.name:
                    raise AppointmentTypeValidationError(
                        ...
                        f"belong to at most one slot per appointment type."
                    )
    ...
    def create_appointment_type(
        self, data: AppointmentTypeInputData, bypass_limits: bool = False
    ) -> AppointmentType:
        """Create an AppointmentType with its slots and memberships.

        :param bypass_limits: When True, skips the ``appointment_types`` limit guard below.
            Only management commands and one-off repair scripts should pass this -- never
            a request-handling path.
        :raises OverLimitError: When the organization is at its effective ``appointment_types``
        ...
        # REST and the existing GraphQL ``create_appointment_type`` mutation alike --
        # inherits it: an appointment type that accepts public scheduling must have a
        # duration. A codeless public-appointment-type booking presents no code, so it
        # inherits no per-code pin; the appointment type is the only place a length
        # constraint can live for that path (see AppointmentType.duration's
        # help_text). Both client-facing surfaces can supply it alongside the
        # privacy flag -- ``duration`` on the REST AppointmentTypeSerializer,
        # ``duration_seconds`` on the GraphQL AppointmentTypeInput -- so a public
        # appointment type is creatable in one call on either. What still raises here is
        # asking for public scheduling without a length: ``create_appointment_type``
        ...
        if accepts_public_scheduling and data.duration is None:
            raise AppointmentTypeValidationError(
                "An AppointmentType that accepts public scheduling must have a duration set. "
                "Send ``duration`` to the REST AppointmentTypeSerializer or ``duration_seconds`` "
                "to the create_appointment_type GraphQL mutation, in the same call that makes "
                "the appointment type public."
            )
        ...
        self._create_slots(appointment_type, slots_data)
        self._audit_appointment_type_write(AuditAction.CREATE, appointment_type)
        return appointment_type

    @transaction.atomic()
    def update_appointment_type(
        self, appointment_type_id: int, data: AppointmentTypeInputData
    ) -> AppointmentType:
        """Reconcile an AppointmentType's slots and memberships with `data`.
        ...
        every remaining calendar's appointment-type-scoped configuration for it. Removing
        ...
        appointment-type-scoped windows, blocked time, and quota rules survive (see
        `_reconcile_slot`).
        """
        self._assert_initialized()
        self._check_not_restricted()
        appointment_type = self._get_appointment_type_by_id(appointment_type_id)
        ...
        existing_slots = {s.name: s for s in appointment_type.slots.all()}
        ...
        before = {
            ...
            "accepts_public_scheduling": appointment_type.accepts_public_scheduling,
        }
        ...
        if data.accepts_public_scheduling is not None:
            appointment_type.accepts_public_scheduling = data.accepts_public_scheduling
        if data.duration is not None:
            appointment_type.duration = data.duration

        # Same invariant as create_appointment_type, evaluated against the RESULTING state
        # (the incoming values where provided, the persisted ones otherwise) --
        # not just what this call happens to touch. An appointment type already public with
        # a duration cannot be updated into a public appointment type with no duration
        # (impossible via this dataclass's tri-state semantics: ``duration`` can
        # only be set here, never cleared), but a private appointment type with no duration
        # being flipped public in the same call must still be caught.
        if appointment_type.accepts_public_scheduling and appointment_type.duration is None:
            raise AppointmentTypeValidationError(
                "An AppointmentType that accepts public scheduling must have a duration set. "
                "Send ``duration`` to the REST AppointmentTypeSerializer or ``duration_seconds`` "
                "to the update_appointment_type GraphQL mutation, in the same call that makes "
                "the appointment type public."
            )
        ...
        appointment_type.save(update_fields=update_fields)
        ...
        for name, slot in existing_slots.items():
            if name not in incoming_names:
                ...
                # to every remaining calendar's appointment-type-scoped windows, blocked
                ...
                if has_future_selection:
                    raise AppointmentTypeSlotInUseError()
                slot.delete()
        ...
        for slot_data in slots_data:
            if slot_data.name in existing_slots:
                ...
                self._create_slots(appointment_type, [slot_data])
        ...
        self._audit_appointment_type_write(
            AuditAction.UPDATE, appointment_type, diff=compute_diff(before, after)
        )

        return appointment_type

    @transaction.atomic()
    def delete_appointment_type(self, appointment_type_id: int) -> None:
        """Delete an AppointmentType. Refuses if any events (past or future) reference
        it, matching the PROTECT FK on `CalendarEvent.appointment_type`."""
        self._assert_initialized()
        self._check_not_restricted()
        appointment_type = self._get_appointment_type_by_id(appointment_type_id)
        ...
        if (
            ...
            .filter(appointment_type_fk=appointment_type)
            .exists()
        ):
            raise AppointmentTypeHasFutureEventsError(
                "Cannot delete AppointmentType because it has bookings."
            )

        # Build the audit subject before the row is deleted (pk is needed).
        self._audit_appointment_type_write(AuditAction.DELETE, appointment_type)
    ...
    def create_pool(self, data: CalendarPoolInputData) -> CalendarPool:
        ...
        self._audit_appointment_type_write(AuditAction.CREATE, pool)
        return pool
    ...
    def update_pool(self, pool_id: int, data: CalendarPoolInputData) -> CalendarPool:
        ...
        ``AppointmentTypeService._reconcile_slot_pools``, which recomputes each
        ...
        self._audit_appointment_type_write(
            AuditAction.UPDATE, pool, diff=compute_diff(before, after)
        )
        if to_remove or to_add:
            # Separate audit entry for the roster change, mirroring
            # `_reconcile_slot`'s split between the appointment-type-level fields
    ...
    def delete_pool(self, pool_id: int) -> None:
        ...
        naming the referencing appointment types (mirrors ``delete_appointment_type``'s
        refuse-when-referenced posture -- see the plan's Pool deletion
        decision). ``AppointmentTypeSlotPool.pool`` also ``PROTECT``s the pool at
        the schema level, so this check exists to give a structured,
        appointment-type-naming error instead of a bare ``IntegrityError`` surfacing from
        ...
        appointment_type_names = list(
            AppointmentType.objects.filter_by_organization(self.organization_id)
            .filter(slots__pool_attachments__pool=pool)
            .values_list("name", flat=True)
            .distinct()
        )
        if appointment_type_names:
            raise CalendarPoolInUseError(appointment_type_names)

        # Build the audit subject before the row is deleted (pk is needed).
        self._audit_appointment_type_write(AuditAction.DELETE, pool)
        pool.delete()

    def _create_slots(
        self,
        appointment_type: AppointmentType,
        slots_data: Iterable[AppointmentTypeSlotInputData],
    ) -> None:
        for slot_data in slots_data:
    ...
    def _reconcile_slot_pools(
        ...
        *,
        ...
        from inside ``update_appointment_type`` / ``create_appointment_type``'s ``transaction.atomic()``,
        ...
        CREATE the surrounding ``create_appointment_type`` call already records, and an
        ...
        # created cannot have any AppointmentTypeSlotPool rows yet, so the query
        # below would provably return nothing for it.
        attached_pool_ids = (
            known_attached_pool_ids
            if known_attached_pool_ids is not None
            else set(
                AppointmentTypeSlotPool.objects.filter_by_organization(self.organization_id)
                .filter(slot_fk=slot)
        ...
        if to_detach:
            AppointmentTypeSlotPool.objects.filter_by_organization(self.organization_id).filter(
                slot_fk=slot, pool_fk_id__in=to_detach
            ).delete()
        if to_attach:
            AppointmentTypeSlotPool.objects.bulk_create(
                [
                    AppointmentTypeSlotPool(
                        organization=self.organization,
                        slot_fk=slot,
        ...
        existing_row_ids_by_pair: dict[tuple[int | None, int], int] = {
            ...
            for row_id, pool_id, calendar_id in (
                AppointmentTypeSlotMembership.objects.filter_by_organization(self.organization_id)
                .projected()
                .filter(slot_fk=slot)
    ...
    def _reconcile_slot(
        self,
        slot: AppointmentTypeSlot,
        slot_data: AppointmentTypeSlotInputData,
    ) -> None:
        ...
        if to_remove:
            ...
            # AppointmentTypeSlotMembership row(s); it never fails on existing
            # bookings and never touches the calendar's appointment-type-scoped rows --
        ...
        if to_remove or to_add:
            ...
            # (`update_appointment_type`) separately audits the appointment-type-level field
            # changes; this call is what keeps a roster change from landing
            # as an empty-diff "appointment type updated" row.
            self._audit_appointment_type_write(
                AuditAction.UPDATE,
                slot,
                diff=compute_diff(
        ...
        # happens here for a client that never learned about pools, and an appointment type
        # with no pools resolves through the exact path it did before.
        if slot_data.pool_ids is not None:
            self._reconcile_slot_pools(slot, slot_data.pool_ids)

    # ------------------------------------------------------------------
    # Appointment-type-scoped availability windows (writes)
    # ------------------------------------------------------------------

    def _resolve_appointment_type_scoped_membership(
        self, appointment_type_slot_id: int, calendar_id: int
    ) -> AppointmentTypeSlotMembership:
        """Resolve the roster entry (calendar, appointment type slot) an appointment-type-scoped
        availability write targets.

        Raises the same not-found-shaped ``AppointmentTypeSlotConfigNotFoundError``
        ...
        appointment-type-scoped window, block, or quota rule was written for such a
        ...
        membership = (
            ...
            .filter(slot_fk_id=appointment_type_slot_id, calendar_fk_id=calendar_id)
            .order_by("id")
            .first()
        )
        if membership is None:
            raise AppointmentTypeSlotConfigNotFoundError()
        return membership

    def _get_appointment_type_scoped_window(self, window_id: int) -> AvailableTime:
        """Fetch an appointment-type-scoped ``AvailableTime`` row by id, scoped to this org.

        Reads through the ``unscoped`` accessor (never the default manager,
        which excludes appointment-type-scoped rows) and requires ``appointment_type_slot`` to be
        ...
        try:
            return (
                ...
                .select_related("appointment_type_slot", "calendar", "recurrence_rule")
                .get(id=window_id, appointment_type_slot_fk__isnull=False)
            )
        except AvailableTime.DoesNotExist:
            raise AppointmentTypeSlotConfigNotFoundError() from None

    def _authorize_appointment_type_scoped_write(
        self, acting_user: User, calendar: Calendar, appointment_type_slot: AppointmentTypeSlot
    ) -> None:
        """Gate an appointment-type-scoped availability write to the calendar's owner (within
        an appointment type they can see) or an org admin.
        ...
        member cannot learn an appointment type exists through the error shape.
        """
        if self.calendar_permission_service is None or not (
            self.calendar_permission_service.can_manage_appointment_type_scoped_calendar_config(
                user=acting_user, calendar=calendar, appointment_type_slot=appointment_type_slot
            )
        ):
            raise AppointmentTypeSlotConfigNotFoundError()
    ...
    def _audit_appointment_type_scoped_availability_write(
        self,
        action: str,
        ...
        """Emit an audit record for an appointment-type-scoped availability window write.

        Unlike ``_audit_appointment_type_write`` (which resolves the actor from a bound
    ...
    def _appointment_type_scoped_available_times_expanded(
        self,
        calendar_id: int,
        ...
        """Expand every appointment-type-scoped ``AvailableTime`` for ``(calendar, appointment_type_slot)``
        that overlaps ``[start_date, end_date)``, recurrence included.

        Mirrors ``AvailabilityService.get_available_times_expanded``, but reads
        through the appointment-type-scoped accessor instead of the default (base-rows-only)
        manager. Delegates to ``slot_engine.expand_appointment_type_scoped_available_times``,
        the single-pair case of the same batched implementation the
        discovery-side fetch uses. Occurrence
        expansion for appointment-type-scoped masters is safe because (a) no write path
        creates an appointment-type-scoped recurrence exception yet, and (b)
        ...
        appointment-type-scoped, ensuring appointment-type-scoped exception rows are found if one ever
        becomes reachable.
        """
        org_id = self.organization_id
        return slot_engine.expand_appointment_type_scoped_available_times(
            org_id, [appointment_type_slot_id], [calendar_id], start_date, end_date
        )

    def _find_orphaned_bookings(
        self, calendar_id: int, appointment_type_slot: AppointmentTypeSlot, now: datetime.datetime
    ) -> list[CalendarEvent]:
        """Confirmed future bookings in ``appointment_type_slot`` for ``calendar_id`` that
        fall outside the calendar's current appointment-type-scoped availability
        configuration.

        Runs against every appointment-type-scoped window that currently exists for this
        ...
        Performance: expands all appointment-type-scoped windows ONCE over the union range
        ...
        org_id = self.organization_id
        selections = (
            CalendarEventAppointmentTypeSelection.objects.filter_by_organization(org_id)
            .filter(
                slot_fk=appointment_type_slot,
                calendar_fk_id=calendar_id,
        ...
        # Expand ALL appointment-type-scoped windows once over the union range.
        all_windows = self._appointment_type_scoped_available_times_expanded(
            calendar_id, appointment_type_slot.id, min_start, max_end
        )
    ...
    @transaction.atomic()
    def create_appointment_type_scoped_availability_window(
        self,
        acting_user: User,
        appointment_type_slot_id: int,
        calendar_id: int,
        ...
        """Create an appointment-type-scoped availability window for ``(calendar, appointment_type_slot)``.

        Writes through the explicit appointment-type-scoped accessor
        (``AvailableTime.objects.unscoped().create(..., appointment_type_slot=...)``),
        ...
        an org admin (see
        ``CalendarPermissionService.can_manage_appointment_type_scoped_calendar_config``).

        On creating the FIRST appointment-type-scoped window for a (calendar, appointment_type_slot),
        ...
        organization = self.bound_organization
        membership = self._resolve_appointment_type_scoped_membership(
            appointment_type_slot_id, calendar_id
        )
        self._authorize_appointment_type_scoped_write(
            acting_user, membership.calendar, membership.slot
        )
        ...
        window = AvailableTime.objects.unscoped().create(
            ...
            appointment_type_slot=membership.slot,
            start_time_tz_unaware=start_time,
            end_time_tz_unaware=end_time,
            timezone=tz,
            recurrence_rule=recurrence_rule,
        )
        self._audit_appointment_type_scoped_availability_write(
            AuditAction.CREATE, acting_user, window
        )
        ...
        return AppointmentTypeScopedAvailabilityWriteResult(
            window=window, orphaned_bookings=orphaned_bookings
        )

    @transaction.atomic()
    def update_appointment_type_scoped_availability_window(
        self,
        acting_user: User,
        window_id: int,
        ...
        """Partially update an appointment-type-scoped availability window (only provided
        ...
        window's appointment type slot for its calendar that no longer falls inside the
        calendar's appointment-type-scoped configuration is collected and returned.
        ...
        window = self._get_appointment_type_scoped_window(window_id)
        self._authorize_appointment_type_scoped_write(
            acting_user, window.calendar, window.appointment_type_slot
        )
        ...
        orphaned_bookings = self._find_orphaned_bookings(
            ...
            appointment_type_slot=cast(AppointmentTypeSlot, window.appointment_type_slot),
            now=now,
        )
        return AppointmentTypeScopedAvailabilityWriteResult(
            window=window, orphaned_bookings=orphaned_bookings
    ...
    @transaction.atomic()
    def delete_appointment_type_scoped_availability_window(
        self, acting_user: User, window_id: int
    ) -> None:
        """Delete an appointment-type-scoped availability window (a single ``AvailableTime`` row).
        ...
        entirely (which cascades away its windows through the ``appointment_type_slot``
        foreign key's ``on_delete=CASCADE``) is a distinct action, exercised
        through ``update_appointment_type``.
        """
        self._assert_initialized()
        self._check_not_restricted()
        window = self._get_appointment_type_scoped_window(window_id)
        ...
        self._audit_appointment_type_scoped_availability_write(
            AuditAction.DELETE, acting_user, window
        )
        window.delete()

    def _find_matching_appointment_type_scoped_window(
        self,
        appointment_type_slot_id: int,
        calendar_id: int,
        start_time: datetime.datetime,
        ...
        """Find an existing appointment-type-scoped window with identical content, if any.
        ...
        Matched on (calendar, appointment type slot, start_time, end_time, timezone,
    ...
    def batch_upsert_appointment_type_scoped_availability_windows(
        ...
        """Apply an atomic bulk-upsert batch of appointment-type-scoped availability
        window create/update/delete operations within one appointment type slot's
        roster (public API).
        ...
        * **Idempotent create.** A ``create`` operation is an upsert, not a
          blind insert: :meth:`_find_matching_appointment_type_scoped_window` looks for
          an existing appointment-type-scoped window with identical content first. A
        ...
        * **update / delete** require ``window_id`` and act on exactly the
        ...
        ``:raises AppointmentTypeSlotConfigNotFoundError:`` below) if they
        ...
        :raises AppointmentTypeSlotConfigNotFoundError: a create op's
            ``calendar_id`` is not a member of ``appointment_type_slot_id``'s roster, or
            an update/delete op's ``window_id`` does not resolve to a
            appointment-type-scoped window in this slot, or that window does not belong
            to the op's own ``calendar_id``.
        :raises ValueError: an operation's shape is invalid (unknown action,
            missing required field).
        :return: every appointment-type-scoped window in ``appointment_type_slot_id``'s roster
        ...
        for op in operations:
            if op["action"] in ("update", "delete"):
                window = self._get_appointment_type_scoped_window(op["window_id"])
                if window.appointment_type_slot_fk_id != appointment_type_slot_id:
                    raise AppointmentTypeSlotConfigNotFoundError()
        ...
        memberships_by_calendar: dict[int, AppointmentTypeSlotMembership] = {}
        for op in operations:
            if op["action"] == "create" and op["calendar_id"] not in memberships_by_calendar:
                memberships_by_calendar[op["calendar_id"]] = (
                    self._resolve_appointment_type_scoped_membership(
                        appointment_type_slot_id, op["calendar_id"]
                    )
                )

        # Idempotent-create resolution: match each create op against an
        # existing appointment-type-scoped window before it counts toward net growth.
        matched_existing: dict[int, AvailableTime] = {}
        for i, op in enumerate(operations):
            if op["action"] != "create":
                continue
            existing = self._find_matching_appointment_type_scoped_window(
        ...
        # Reads through ``unscoped()`` because appointment-type-scoped rows are invisible
    ...
    # ------------------------------------------------------------------
    # Appointment-type-scoped blocked time (writes)
    # ------------------------------------------------------------------

    def _get_appointment_type_scoped_block(self, block_id: int) -> BlockedTime:
        """Fetch an appointment-type-scoped ``BlockedTime`` row by id, scoped to this org.

        Reads through the ``unscoped`` accessor (never the default manager,
        which excludes appointment-type-scoped rows) and requires ``appointment_type_slot`` to be
        set, so an id belonging to a base row raises the same not-found error
        as a genuinely missing id. Mirrors ``_get_appointment_type_scoped_window``.
        """
        org_id = self.organization_id
        try:
            return (
                ...
                .filter_by_organization(org_id)
                .select_related("appointment_type_slot", "calendar", "recurrence_rule")
                .get(id=block_id, appointment_type_slot_fk__isnull=False)
            )
        except BlockedTime.DoesNotExist:
            raise AppointmentTypeSlotConfigNotFoundError() from None

    def _audit_appointment_type_scoped_block_write(
        self,
        action: str,
        acting_user: "User | SystemUser",
        ...
        """Emit an audit record for an appointment-type-scoped blocked-time write.

        Mirrors ``_audit_appointment_type_scoped_availability_write``: the acting
    ...
    def _appointment_type_scoped_blocked_times_expanded(
        self,
        calendar_id: int,
        appointment_type_slot_id: int,
        start_date: datetime.datetime,
        ...
        """Expand every appointment-type-scoped ``BlockedTime`` for ``(calendar, appointment_type_slot)``
        that overlaps ``[start_date, end_date)``, recurrence included.

        Mirrors ``_appointment_type_scoped_available_times_expanded``, delegating to
        ``slot_engine.expand_appointment_type_scoped_blocked_times`` -- the single-pair
        case of the same batched implementation the discovery-side fetch
        uses.
        """
        org_id = self.organization_id
        return slot_engine.expand_appointment_type_scoped_blocked_times(
            org_id, [appointment_type_slot_id], [calendar_id], start_date, end_date
        )

    def _find_bookings_orphaned_by_appointment_type_scoped_block(
        self, calendar_id: int, appointment_type_slot: AppointmentTypeSlot, now: datetime.datetime
    ) -> list[CalendarEvent]:
        """Confirmed future bookings in ``appointment_type_slot`` for ``calendar_id`` that
        fall INSIDE the calendar's current appointment-type-scoped blocked time, after a
        block write.
        ...
        (the block now removes that time). Runs against every appointment-type-scoped
        ...
        # Expand ALL appointment-type-scoped blocks once over the union range.
        all_blocks = self._appointment_type_scoped_blocked_times_expanded(
            calendar_id, appointment_type_slot.id, min_start, max_end
        )
    ...
    @transaction.atomic()
    def create_appointment_type_scoped_blocked_time(
        self,
        acting_user: User,
        appointment_type_slot_id: int,
        calendar_id: int,
        ...
        """Create an appointment-type-scoped blocked time for ``(calendar, appointment_type_slot)``.

        Writes through the explicit appointment-type-scoped accessor
        (``BlockedTime.objects.unscoped().create(..., appointment_type_slot=...)``),
        never the default manager. Carries the same recurrence expressiveness
        ...
        Unlike an appointment-type-scoped WINDOW -- where windows OR together, so only the
        FIRST one flips a calendar from fall-through to narrowed and can
        orphan a booking -- every appointment-type-scoped BLOCK independently subtracts
        ...
        block = BlockedTime.objects.unscoped().create(
            ...
            # -- base or appointment-type-scoped -- on the same calendar.
            external_id=f"appointment-type-scoped-block-{uuid.uuid4()}",
            recurrence_rule=recurrence_rule,
        )
        self._audit_appointment_type_scoped_block_write(AuditAction.CREATE, acting_user, block)

        orphaned_bookings = self._find_bookings_orphaned_by_appointment_type_scoped_block(
            calendar_id=calendar_id,
            appointment_type_slot=membership.slot,
            now=now,
        )

        return AppointmentTypeScopedBlockWriteResult(
            block=block, orphaned_bookings=orphaned_bookings
        )

    @transaction.atomic()
    def update_appointment_type_scoped_blocked_time(
        self,
        acting_user: User,
        block_id: int,
        start_time: datetime.datetime | None = None,
        ...
        """Partially update an appointment-type-scoped blocked time (only provided fields
        change -- mirrors ``update_appointment_type_scoped_availability_window``).
        ...
        After the update is applied, every confirmed future booking in the
        block's appointment type slot for its calendar that now falls INSIDE the
        calendar's appointment-type-scoped blocked time is collected and returned.
        ...
        block = self._get_appointment_type_scoped_block(block_id)
        self._authorize_appointment_type_scoped_write(
            acting_user, block.calendar, block.appointment_type_slot
        )
        ...
        self._audit_appointment_type_scoped_block_write(
            AuditAction.UPDATE, acting_user, block, diff=compute_diff(before, after)
        )

        orphaned_bookings = self._find_bookings_orphaned_by_appointment_type_scoped_block(
            calendar_id=cast(int, block.calendar_fk_id),
            appointment_type_slot=cast(AppointmentTypeSlot, block.appointment_type_slot),
            now=now,
        )
        return AppointmentTypeScopedBlockWriteResult(
            block=block, orphaned_bookings=orphaned_bookings
    ...
    @transaction.atomic()
    def delete_appointment_type_scoped_blocked_time(self, acting_user: User, block_id: int) -> None:
        """Delete an appointment-type-scoped blocked time (a single ``BlockedTime`` row).

        A recurring block is stored as one row; deleting it removes the whole
        series (mirrors ``delete_appointment_type_scoped_availability_window``). No
        ...
        ``appointment_type_slot`` foreign key's ``on_delete=CASCADE``) is a distinct
        action, exercised through ``update_appointment_type``.
        """
        self._assert_initialized()
        self._check_not_restricted()
        block = self._get_appointment_type_scoped_block(block_id)
        ...
        self._audit_appointment_type_scoped_block_write(AuditAction.DELETE, acting_user, block)
        block.delete()

    def _find_matching_appointment_type_scoped_block(
        self,
        appointment_type_slot_id: int,
        calendar_id: int,
        start_time: datetime.datetime,
        ...
        """Find an existing appointment-type-scoped block with identical content, if any.

        Backs the batch-upsert write path's idempotent ``create`` (mirrors
        ``_find_matching_appointment_type_scoped_window``): an upstream system that
        ...
        on (calendar, appointment type slot, start_time, end_time, timezone, reason,
        rrule). ``start_time``/``end_time`` are compared as the aware
        instants they are -- paired with an exact ``timezone`` match this is
        a safe point lookup, not the cross-timezone comparison
        ``start_time_tz_unaware`` is unsafe for (see AGENTS.md).
    ...
    def batch_upsert_appointment_type_scoped_blocked_times(
        ...
        """Apply an atomic bulk-upsert batch of appointment-type-scoped blocked-time
        create/update/delete operations within one appointment type slot's roster
        (public API).

        Mirrors ``batch_upsert_appointment_type_scoped_availability_windows``'s
        ...
        write appointment-type-scoped blocks either.
        ...
        * **All-or-nothing.** The whole batch runs inside one transaction
          ...
          applied.
        * **Idempotent create.** A ``create`` operation is an upsert, not a
          blind insert: :meth:`_find_matching_appointment_type_scoped_block` looks for
          an existing appointment-type-scoped block with identical content first. A
        ...
        * **update / delete** require ``block_id`` and act on exactly the
        ...
        :raises AppointmentTypeSlotConfigNotFoundError: a create op's
            ...
            appointment-type-scoped block in this slot, or that block does not belong
            to the op's own ``calendar_id``.
        :raises ValueError: an operation's shape is invalid (unknown action,
            missing required field).
        :return: every appointment-type-scoped block in ``appointment_type_slot_id``'s roster (all
        ...
        for op in operations:
            if op["action"] in ("update", "delete"):
                block = self._get_appointment_type_scoped_block(op["block_id"])
                if block.appointment_type_slot_fk_id != appointment_type_slot_id:
                    raise AppointmentTypeSlotConfigNotFoundError()
        ...
        # Idempotent-create resolution: match each create op against an
        # existing appointment-type-scoped block before writing anything.
        matched_existing: dict[int, BlockedTime] = {}
        for i, op in enumerate(operations):
            if op["action"] != "create":
                continue
            existing = self._find_matching_appointment_type_scoped_block(
                appointment_type_slot_id=appointment_type_slot_id,
                calendar_id=op["calendar_id"],
                start_time=op["start_time"],
                end_time=op["end_time"],
        ...
        for i, op in enumerate(operations):
            ...
            if action == "create":
                ...
                block = BlockedTime.objects.unscoped().create(
                    ...
                    # collision with any other block -- base or appointment-type-scoped -- on the
                    # same calendar (mirrors create_appointment_type_scoped_blocked_time).
                    external_id=f"appointment-type-scoped-block-{uuid.uuid4()}",
                    recurrence_rule=recurrence_rule,
                )
                self._audit_appointment_type_scoped_block_write(
                    AuditAction.CREATE, acting_principal, block
                )
            elif action == "update":
                ...
                block = blocks_by_op_id[op["block_id"]]
                self._audit_appointment_type_scoped_block_write(
                    AuditAction.DELETE, acting_principal, block
                )
                block.delete()
    ...
    def _get_appointment_type_scoped_quota_rule(self, rule_id: int) -> AppointmentTypeSlotQuotaRule:
        """Fetch an appointment-type-scoped ``AppointmentTypeSlotQuotaRule`` by id, scoped to
        this org.

        Unlike ``_get_appointment_type_scoped_window``/``_get_appointment_type_scoped_block``, there
        is no ``unscoped()``/default-exclude split to navigate here -- every
        ``AppointmentTypeSlotQuotaRule`` row is appointment-type-scoped by construction
        ...
        try:
            return (
                AppointmentTypeSlotQuotaRule.objects.filter_by_organization(org_id)
                .select_related("appointment_type_slot", "calendar")
                .get(id=rule_id)
            )
        except AppointmentTypeSlotQuotaRule.DoesNotExist:
            raise AppointmentTypeSlotConfigNotFoundError() from None

    def _audit_appointment_type_scoped_quota_rule_write(
        self,
        action: str,
        acting_user: "User | SystemUser",
        subject_instance: AppointmentTypeSlotQuotaRule,
        diff: dict | None = None,
    ) -> None:
        """Emit an audit record for an appointment-type-scoped quota-rule write.

        Mirrors ``_audit_appointment_type_scoped_block_write``/``_audit_appointment_type_scoped_availability_write``:
    ...
    def _find_matching_appointment_type_scoped_quota_rule(
        self,
        appointment_type_slot_id: int,
        calendar_id: int,
        period: str,
        cap: int,
    ) -> AppointmentTypeSlotQuotaRule | None:
        """Find an existing appointment-type-scoped quota rule with identical content, if any.

        Backs the batch-upsert write path's idempotent ``create`` (mirrors
        ``_find_matching_appointment_type_scoped_block``): replaying the same batch
        ...
        constraint. Matched on (calendar, appointment type slot, period, cap) -- a
        ...
        organization = self.bound_organization
        return (
            AppointmentTypeSlotQuotaRule.objects.for_appointment_type_slot(appointment_type_slot_id)
            .filter_by_organization(organization.id)
    ...
    @transaction.atomic()
    def create_appointment_type_scoped_quota_rule(
        self,
        acting_user: User,
        appointment_type_slot_id: int,
        calendar_id: int,
        ...
        """Create an appointment-type-scoped quota rule capping ``calendar_id``'s live
        bookings made through ``appointment_type_slot_id`` within one ``period``.
        ...
        :raises AppointmentTypeValidationError: ``cap`` is not a positive
            integer, or a rule for ``(calendar_id, appointment_type_slot_id, period)``
        ...
        self._check_not_restricted()
        if cap < 1:
            raise AppointmentTypeValidationError("cap must be at least 1.")
        if period not in QuotaPeriod.values:
            raise AppointmentTypeValidationError(f"Invalid period: {period!r}.")
        ...
        try:
            ...
            with transaction.atomic():
                rule = AppointmentTypeSlotQuotaRule.objects.create(
                    organization=organization,
                    appointment_type_slot=membership.slot,
                    calendar=membership.calendar,
                    period=period,
        ...
        self._audit_appointment_type_scoped_quota_rule_write(AuditAction.CREATE, acting_user, rule)
        return rule

    @transaction.atomic()
    def update_appointment_type_scoped_quota_rule(
        self,
        acting_user: User,
        rule_id: int,
        ...
        """Partially update an appointment-type-scoped quota rule (only provided fields
        change -- mirrors ``update_appointment_type_scoped_blocked_time``).
        ...
        rule = self._get_appointment_type_scoped_quota_rule(rule_id)
        self._authorize_appointment_type_scoped_write(
            acting_user, rule.calendar, rule.appointment_type_slot
        )
        ...
        if update_fields:
            try:
                # See create_appointment_type_scoped_quota_rule -- nested atomic() scopes
        ...
        self._audit_appointment_type_scoped_quota_rule_write(
            AuditAction.UPDATE, acting_user, rule, diff=compute_diff(before, after)
        )
        return rule

    @transaction.atomic()
    def delete_appointment_type_scoped_quota_rule(self, acting_user: User, rule_id: int) -> None:
        """Delete an appointment-type-scoped quota rule.

        Mirrors ``delete_appointment_type_scoped_blocked_time``/``delete_appointment_type_scoped_availability_window``:
        ...
        self._audit_appointment_type_scoped_quota_rule_write(AuditAction.DELETE, acting_user, rule)
        rule.delete()
    ...
    def batch_upsert_appointment_type_scoped_quota_rules(
        ...
        operations: Iterable[dict],
        acting_principal: "User | SystemUser",
    ) -> list[AppointmentTypeSlotQuotaRule]:
        """Apply an atomic bulk-upsert batch of appointment-type-scoped quota-rule
        create/update/delete operations within one appointment type slot's roster
        (public API).

        Direct mirror of ``batch_upsert_appointment_type_scoped_blocked_times``'s
        ...
        organization cannot write appointment-type-scoped quota rules either.

        * **All-or-nothing.** The whole batch runs inside one transaction.
        * **Idempotent create.** A ``create`` op is an upsert:
          :meth:`_find_matching_appointment_type_scoped_quota_rule` looks for an existing
          ...
          the unique-constraint violation as :class:`AppointmentTypeValidationError`,
        ...
        * **update / delete** require ``rule_id`` and act on exactly the row
        ...
        :raises AppointmentTypeSlotConfigNotFoundError: a create op's
            ...
            appointment-type-scoped quota rule in this slot, or that rule does not
            belong to the op's own ``calendar_id``.
        :raises AppointmentTypeValidationError: a ``cap`` is not a positive
            integer, or a create/update collides with the unique
            (calendar, slot, period) constraint.
        :raises ValueError: an operation's shape is invalid (unknown action,
            missing required field).
        :return: every appointment-type-scoped quota rule in ``appointment_type_slot_id``'s roster
            (all calendars) after the batch is applied.
        """
        self._assert_initialized()
        organization = self.bound_organization
        ...
        for op in operations:
            ...
            if (
                ...
                raise AppointmentTypeValidationError(f"Invalid period: {op['period']!r}.")
        ...
        # batch before anything is written.
        rules_by_op_id: dict[int, AppointmentTypeSlotQuotaRule] = {}
        for op in operations:
            if op["action"] in ("update", "delete"):
                rule = self._get_appointment_type_scoped_quota_rule(op["rule_id"])
                if rule.appointment_type_slot_fk_id != appointment_type_slot_id:
                    raise AppointmentTypeSlotConfigNotFoundError()
        ...
        # Idempotent-create resolution: match each create op against an
        # existing appointment-type-scoped quota rule before writing anything.
        matched_existing: dict[int, AppointmentTypeSlotQuotaRule] = {}
        for i, op in enumerate(operations):
            if op["action"] != "create":
                continue
            existing = self._find_matching_appointment_type_scoped_quota_rule(
                appointment_type_slot_id=appointment_type_slot_id,
                calendar_id=op["calendar_id"],
                period=op["period"],
                cap=op["cap"],
    ...
    # Queries
    # ------------------------------------------------------------------
    def get_appointment_type_events(
        self,
        appointment_type_id: int,
        start: datetime.datetime,
        end: datetime.datetime,
    ) -> CalendarEventQuerySet:
        """Return the events booked under an appointment type that overlap [start, end].
    ...
    def find_stale_selections(
        ...
        """List every ``(event, slot, calendar)`` triple in this appointment type whose
        ...
        selection is stale when no ``AppointmentTypeSlotMembership`` row exists
        ...
        ``CalendarEventAppointmentTypeSelection``, so the whole sweep -- regardless of
        how many selections exist or how many are stale -- costs exactly one
        query, the same shape as ``_slot_pools_with_appointment_type_scoped_flags``'s
        ...
        half-open overlap semantics as ``get_appointment_type_events``. Omitting both
        (the default) returns every stale selection in the appointment type regardless
        ...
        :raises AppointmentTypeValidationError: ``offset`` is negative, or
        ...
        if offset < 0:
            raise AppointmentTypeValidationError("Offset must be non-negative")
        if limit <= 0 or limit > 100:
            raise AppointmentTypeValidationError("Limit must be between 1 and 100")

        current_membership = AppointmentTypeSlotMembership.objects.filter_by_organization(
            self.organization_id
        ).filter(
        ...
        selections = (
            ...
            .filter(event_fk__appointment_type_fk=appointment_type)
    ...
    def _slot_pools_with_appointment_type_scoped_flags(
        self, slots: Iterable[AppointmentTypeSlot]
        ...
        appointment-type-scoped availability window, ANY appointment-type-scoped blocked time, and
        ANY appointment-type-scoped quota rule configured for that slot (regardless of
        ...
        the slot's calendar pool, so an unconfigured appointment type costs exactly as
        ...
        fetch the actual appointment-type-scoped spans / rules (a fixed, non-per-candidate
        number of additional queries -- see
        ``slot_engine.fetch_appointment_type_scoped_available_spans`` /
        ``slot_engine.fetch_appointment_type_scoped_blocking_spans`` /
        ``slot_engine.fetch_appointment_type_scoped_quota_rules``).

        Returns ``(slot_pool_by_id, appointment_type_scoped_window_calendar_ids_by_slot,
        appointment_type_scoped_block_calendar_ids_by_slot,
        appointment_type_scoped_quota_calendar_ids_by_slot)``. The second, third, and
        ...
        appointment-type-scoped window / block / quota rule exists anywhere in the appointment type.
        """
        org_id = self.organization_id
        slot_pool_by_id: dict[int, set[int]] = {}
        appointment_type_scoped_window_calendar_ids_by_slot: dict[int, set[int]] = {}
        appointment_type_scoped_block_calendar_ids_by_slot: dict[int, set[int]] = {}
        appointment_type_scoped_quota_calendar_ids_by_slot: dict[int, set[int]] = {}
        for s in slots:
            rows = (
                AppointmentTypeSlotMembership.objects.filter_by_organization(org_id)
                .filter(slot_fk=s)
                .annotate(
                    has_appointment_type_scoped_window=Exists(
                        AvailableTime.objects.unscoped()
                        .filter_by_organization(org_id)
                        .filter(
                            calendar_fk_id=OuterRef("calendar_fk_id"),
                            appointment_type_slot_fk_id=s.id,
                        )
                    ),
                    has_appointment_type_scoped_block=Exists(
                        BlockedTime.objects.unscoped()
                        .filter_by_organization(org_id)
                        .filter(
                            calendar_fk_id=OuterRef("calendar_fk_id"),
                    ...
                    has_appointment_type_scoped_quota=Exists(
                        AppointmentTypeSlotQuotaRule.objects.filter_by_organization(org_id).filter(
                            calendar_fk_id=OuterRef("calendar_fk_id"),
                            appointment_type_slot_fk_id=s.id,
                        )
                    ),
                ...
                .values_list(
                    "calendar_fk_id",
                    "has_appointment_type_scoped_window",
                    "has_appointment_type_scoped_block",
                    "has_appointment_type_scoped_quota",
                )
            )
            pool: set[int] = set()
            ...
            if window_configured:
                appointment_type_scoped_window_calendar_ids_by_slot[s.id] = window_configured
            if block_configured:
                appointment_type_scoped_block_calendar_ids_by_slot[s.id] = block_configured
            if quota_configured:
                appointment_type_scoped_quota_calendar_ids_by_slot[s.id] = quota_configured
        return (
            slot_pool_by_id,
            appointment_type_scoped_window_calendar_ids_by_slot,
            appointment_type_scoped_block_calendar_ids_by_slot,
            appointment_type_scoped_quota_calendar_ids_by_slot,
        )

    def check_appointment_type_availability(
        self,
        appointment_type_id: int,
        ranges: Iterable[tuple[datetime.datetime, datetime.datetime]],
        with_bulk_modifications: bool = False,
    ) -> list[AppointmentTypeRangeAvailability]:
        ...
        Appointment-type-scoped availability windows are intersected in AFTER base
        availability: a calendar with no
        appointment-type-scoped window configured for a slot is unaffected (fall-through
        default, zero added queries -- see
        ``_slot_pools_with_appointment_type_scoped_flags``); a calendar WITH one is listed
        as available for a range only when that range is fully covered by at
        least one of its appointment-type-scoped windows -- narrowing only, never widening.

        Appointment-type-scoped blocked time is applied AFTER base availability
        ...
        Appointment-type-scoped quota rules are checked LAST, after base
        ...
        ``slot_engine.fetch_appointment_type_scoped_quota_period_counts`` for the
        ...
        slots = list(appointment_type.slots.all())
        (
            slot_pool_by_id,
            appointment_type_scoped_window_calendar_ids_by_slot,
            appointment_type_scoped_block_calendar_ids_by_slot,
            appointment_type_scoped_quota_calendar_ids_by_slot,
        ) = self._slot_pools_with_appointment_type_scoped_flags(slots)

        # Self-gating early-out: only fetch expanded appointment-type-scoped spans when at
        # least one calendar anywhere in the appointment type actually has one configured.
        ...
        appointment_type_scoped_spans_by_slot: slot_engine.AppointmentTypeScopedSpansBySlot = {}
        if (
            appointment_type_scoped_window_calendar_ids_by_slot
            and union_start is not None
            and union_end is not None
        ):
            configured_slot_ids = list(appointment_type_scoped_window_calendar_ids_by_slot.keys())
            configured_calendar_ids: set[int] = set()
            for ids in appointment_type_scoped_window_calendar_ids_by_slot.values():
                configured_calendar_ids.update(ids)
            appointment_type_scoped_spans_by_slot = (
                slot_engine.fetch_appointment_type_scoped_available_spans(
                    self.organization_id,
                    configured_slot_ids,
        ...
        appointment_type_scoped_block_spans_by_slot: slot_engine.AppointmentTypeScopedSpansBySlot = {}
        if (
            appointment_type_scoped_block_calendar_ids_by_slot
            and union_start is not None
            and union_end is not None
        ):
            configured_block_slot_ids = list(
                appointment_type_scoped_block_calendar_ids_by_slot.keys()
            )
            configured_block_calendar_ids: set[int] = set()
            for ids in appointment_type_scoped_block_calendar_ids_by_slot.values():
                configured_block_calendar_ids.update(ids)
            appointment_type_scoped_block_spans_by_slot = (
                slot_engine.fetch_appointment_type_scoped_blocking_spans(
                    self.organization_id,
        ...
        appointment_type_scoped_quota_rules_by_slot: slot_engine.AppointmentTypeScopedQuotaRulesBySlot = {}
        appointment_type_scoped_quota_counts_by_slot: slot_engine.AppointmentTypeScopedQuotaCountsBySlot = {}
        ...
        if (
            ...
            configured_quota_slot_ids = list(
                appointment_type_scoped_quota_calendar_ids_by_slot.keys()
            )
            configured_quota_calendar_ids: set[int] = set()
            for ids in appointment_type_scoped_quota_calendar_ids_by_slot.values():
                configured_quota_calendar_ids.update(ids)
            quota_rules = slot_engine.fetch_appointment_type_scoped_quota_rules(
                org.id, configured_quota_slot_ids, configured_quota_calendar_ids
            )
            appointment_type_scoped_quota_rules_by_slot = (
                slot_engine.appointment_type_quota_rules_by_slot(quota_rules)
            )
            ...
            if covering_range is not None:
                appointment_type_scoped_quota_counts_by_slot = (
                    slot_engine.fetch_appointment_type_scoped_quota_period_counts(
        ...
        for start, end in ranges:
            ...
            for s in slots:
                ...
                window_configured_ids = appointment_type_scoped_window_calendar_ids_by_slot.get(
                    s.id
                )
                block_configured_ids = appointment_type_scoped_block_calendar_ids_by_slot.get(s.id)
                quota_configured_ids = appointment_type_scoped_quota_calendar_ids_by_slot.get(s.id)
                if (
                    ...
                    window_spans_for_slot = appointment_type_scoped_spans_by_slot.get(s.id, {})
                    block_spans_for_slot = appointment_type_scoped_block_spans_by_slot.get(s.id, {})
                    quota_rules_for_slot = appointment_type_scoped_quota_rules_by_slot.get(s.id, {})
                    quota_counts_for_slot = appointment_type_scoped_quota_counts_by_slot.get(
                        s.id, {}
                    )
                    final_available = set()
    ...
    # ------------------------------------------------------------------
    # Booking policy enforcement (appointment type path)
    # ------------------------------------------------------------------

    def _check_appointment_type_booking_policy(
        self,
        appointment_type: AppointmentType,
        start_time: datetime.datetime,
        end_time: datetime.datetime,
        ...
        """Enforce the resolved EffectivePolicy for an appointment type booking request.

        Mirrors the shape of ``CalendarService._check_booking_policy`` for
        the appointment type write path.  Steps:
        ...
        2. Resolve the ``EffectivePolicy`` via ``resolve_for_appointment_type`` — the same
           resolver ``find_bookable_slots`` uses, so enforcement and
           discovery agree.
        3. Collect ALL participant calendar IDs across every slot pool of the appointment type
        ...
        This check runs inside ``create_appointment_type_event``'s ``@transaction.atomic()``
        decorator, so a violation rolls back the entire appointment type write — no events or
        ...
        policy = self.booking_policy_service.resolve_for_appointment_type(appointment_type)
        if policy == EffectivePolicy.unconstrained():
            # No policy anywhere for this appointment type → skip all enforcement.
            return

        # Collect ALL calendar IDs in the appointment type's slot pools — the same set
    ...
    # ------------------------------------------------------------------
    # Appointment-type event creation
    # ------------------------------------------------------------------
    @transaction.atomic()
    def create_appointment_type_event(self, data: AppointmentTypeEventInputData) -> CalendarEvent:
        """Create an event booked through an AppointmentType.
        ...
        busy. A `CalendarEventAppointmentTypeSelection` row is written for every
        (slot, calendar) pick.

        The primary calendar is the first `calendar_id` listed in the
        lowest-`order` slot of the appointment type.
        ...
        if self.calendar_service is None:
            raise AppointmentTypeValidationError(
                "AppointmentTypeService.calendar_service must be provided to create appointment type events."
            )
        ...
        appointment_type = self._get_appointment_type_by_id(data.appointment_type_id)

        # --- Appointment-type-level authorization gate ---
        ...
        # (appointment type owners, org admins) bypass this check: they already passed Django's
        # view-level authentication and the AppointmentTypePermission object check.
        #
        ...
        # None), ``can_perform_appointment_type_scheduling`` decides:
        #   1. ``appointment_type.accepts_public_scheduling=True`` → allow (codeless public).
        #   2. An appointment-type-scoped token/code with CREATE permission → allow.
        ...
        if not caller_is_authenticated_user:
            # ``start_time`` / ``end_time`` are passed here (not just ``appointment_type``) so a
            # appointment-type-scoped booking code that pins a duration is enforced at this
            # gate. ``create_event`` below is called with ``appointment_type_authorized=True``,
            ...
            # gate an appointment type booking passes through, so the pin has to live here or
            # appointment type booking would never inherit it.
            if (
                self.calendar_permission_service is None
                or not self.calendar_permission_service.can_perform_appointment_type_scheduling(
                    appointment_type=appointment_type,
        ...
        slots = list(appointment_type.slots.order_by("order", "id"))
        if not slots:
            raise AppointmentTypeValidationError("AppointmentType has no slots to satisfy.")

        selections_by_slot_id = self._validate_selections(
            appointment_type, slots, data.slot_selections
        )
        ...
        # Appointment-type-scoped availability windows: reject a directly-named calendar
        # outside its configured window, AFTER base availability -- narrowing
        # only ever narrows.
        self._assert_calendars_within_appointment_type_scoped_windows(
        ...
        self._check_appointment_type_booking_policy(
            appointment_type=appointment_type,
            start_time=data.start_time,
            end_time=data.end_time,
            now=timezone.now(),
        ...
        # Signal to ``CalendarEventService.create_event`` that appointment-type-level
        ...
        # blocking a booking the appointment type itself permits, without changing the
        ...
        event.appointment_type_fk = appointment_type
        event.save(update_fields=["appointment_type_fk"])

        CalendarEventAppointmentTypeSelection.objects.bulk_create(
            [
                CalendarEventAppointmentTypeSelection(
                    organization=self.organization,
                    event_fk=event,
    ...
    def cancel_appointment_type_event(self, event_id: int, delete_series: bool = False) -> None:
        """Cancel an appointment-type event by deleting the primary event and its linked non-primary BlockedTimes.

        The primary event is deleted via ``CalendarService.delete_event`` which also cascades
        the ``CalendarEventAppointmentTypeSelection`` rows (FK on_delete=CASCADE).  Non-primary
        ...
        Preconditions:
          - ``self.calendar_service`` is set and initialized/authenticated for the same
            organization (mirrors the requirement on ``create_appointment_type_event``).
          - The event identified by ``event_id`` must be an appointment-type event
            (``appointment_type_fk`` set) belonging to this service's organization.
        """
        self._assert_initialized()
        ...
        if self.calendar_service is None:
            raise AppointmentTypeValidationError(
                "AppointmentTypeService.calendar_service must be provided to cancel appointment type events."
            )
        if self.calendar_service.organization is None:
            raise AppointmentTypeValidationError(
                "The injected CalendarService is not initialized with an organization."
        ...
        # Load the appointment-type event to validate it is truly appointment-type.
        try:
        ...
        if event.appointment_type_fk_id is None:
            raise AppointmentTypeValidationError(
                f"Event {event_id} is not an appointment type event (appointment_type_fk is not set)."
            )
        ...
        # Delete the primary event (cascades CalendarEventAppointmentTypeSelection via FK).
    ...
    def reschedule_appointment_type_event(
        ...
        Preconditions:
          - ``self.calendar_service`` is set and initialized/authenticated for the
            same organization (mirrors the requirement on ``create_appointment_type_event``).
          - The event identified by ``event_id`` must be an appointment-type event
            (``appointment_type_fk`` set) belonging to this service's organization.
        ...
        Appointment-type-scoped availability windows ARE re-checked here for every
        calendar currently selected for this event (primary and non-primary
        alike, via ``CalendarEventAppointmentTypeSelection``) -- every enforcement
        ...
        instead. Appointment-type-scoped blocks and quota rules are checked the same way.
        ...
        ``_assert_calendars_within_appointment_type_scoped_windows`` is passed this
        ...
        # ``cancel_appointment_type_event`` above -- this method writes non-primary
        # BlockedTime rows itself, ahead of (and outside of) the blocked
        # ``calendar_service.update_event`` call below.
        self._check_not_restricted()
        if self.calendar_service is None:
            raise AppointmentTypeValidationError(
                "AppointmentTypeService.calendar_service must be provided to reschedule appointment type events."
            )
        if self.calendar_service.organization is None:
            raise AppointmentTypeValidationError(
                "The injected CalendarService is not initialized with an organization."
        ...
        # Load the appointment-type event.
        try:
            event = (
                CalendarEvent.objects.filter_by_organization(self.organization_id)
        ...
        # Appointment-type-scoped availability windows: reject the reschedule if ANY
        # calendar currently selected for this event is outside its
        # appointment-type-scoped window for the NEW time.
        ...
        # Update the non-primary BlockedTimes linked to this appointment-type event.
        # They are identified by the external_id convention set in
        # _create_non_primary_blocked_times: ``appointment-type-event-{event.id}-cal-{cid}``.
    ...
    def _validate_selections(
        self,
        appointment_type: AppointmentType,
        slots: list[AppointmentTypeSlot],
        selections: Iterable[AppointmentTypeSlotSelectionInputData],
    ) -> dict[int, AppointmentTypeSlotSelectionInputData]:
        """Validate a set of (slot, calendars) picks against the appointment type's slots.
        ...
        set to grandfather against. (``reschedule_appointment_type_event`` re-checks
        appointment-type-scoped windows/blocks/quota for an existing event's persisted
        selections directly, without going through this method.)
        """
        slot_by_id = {s.id: s for s in slots}

        seen_slot_ids: set[int] = set()
        selections_by_slot_id: dict[int, AppointmentTypeSlotSelectionInputData] = {}
        for sel in selections:
            ...
            if sel.slot_id not in slot_by_id:
                raise AppointmentTypeValidationError(
                    f"slot_id {sel.slot_id} does not belong to appointment type {appointment_type.id}."
                )
        ...
        for slot in slots:
            ...
            if slot_selection is None:
                raise AppointmentTypeValidationError(f"Slot {slot.name!r} has no selection.")
    ...
    def _assert_calendars_within_appointment_type_scoped_windows(
        self,
        ...
        appointment-type-scoped rule configured for that slot at ``[start, end)`` (spec
        ...
        beat everything", quota last): a pair with a configured appointment-type-scoped
        block that OVERLAPS ``[start, end)`` is rejected with
        ``AppointmentTypeScopedRuleType.INSIDE_BLOCK`` before the window check even
        runs, regardless of what any window or quota rule says. A pair with
        no block hit is then rejected with ``AppointmentTypeScopedRuleType.OUTSIDE_WINDOW``
        ...
        ``AppointmentTypeScopedRuleType.QUOTA_CONSUMED`` when ANY of its rules (e.g. a
        ...
        A calendar with NEITHER an appointment-type-scoped block, window, NOR quota rule
        ...
        Called from both ``create_appointment_type_event`` (after the
        base-availability check) and ``reschedule_appointment_type_event`` (spec:
        every enforcement surface agrees).

        ``rescheduling_event_old_start``: only passed by
        ``reschedule_appointment_type_event``, as the event-being-moved's CURRENT
        ...
        sees its own row. A fresh ``create_appointment_type_event`` call never passes
        this (there is no prior row to exclude), and discovery
        (``calendar_free_for_window`` / ``check_appointment_type_availability``) never
        ...
        configured_block_pairs = set(
            ...
            .values_list("appointment_type_slot_fk_id", "calendar_fk_id")
            .distinct()
        )
        if configured_block_pairs:
            ...
            block_spans_by_slot = slot_engine.fetch_appointment_type_scoped_blocking_spans(
            ...
            for slot_id, calendar_id in pairs:
                ...
                if any(slot_engine.intervals_overlap(span, (start, end)) for span in spans):
                    raise AppointmentTypeScopedRuleViolationError(
                        calendar_id=calendar_id,
                        rule_type=AppointmentTypeScopedRuleType.INSIDE_BLOCK,
                    )
        ...
        if configured_pairs:
            ...
            spans_by_slot = slot_engine.fetch_appointment_type_scoped_available_spans(
            ...
            for slot_id, calendar_id in pairs:
                ...
                if not slot_engine.window_fully_covered_by_spans(spans, start, end):
                    raise AppointmentTypeScopedRuleViolationError(calendar_id=calendar_id)

        # Quota is checked LAST, after block and window both pass.
        configured_quota_rules = slot_engine.fetch_appointment_type_scoped_quota_rules(
            org_id, slot_ids, calendar_ids
        )
        if not configured_quota_rules:
            return

        quota_rules_by_slot = slot_engine.appointment_type_quota_rules_by_slot(
            configured_quota_rules
        )
        ...
        quota_counts_by_slot: slot_engine.AppointmentTypeScopedQuotaCountsBySlot = {}
        if covering_range is not None:
            quota_counts_by_slot = slot_engine.fetch_appointment_type_scoped_quota_period_counts(
        ...
        for slot_id, calendar_id in pairs:
            ...
            for rule in rules:
                ...
                if count >= rule.cap:
                    raise AppointmentTypeScopedRuleViolationError(
                        calendar_id=calendar_id,
                        rule_type=AppointmentTypeScopedRuleType.QUOTA_CONSUMED,
                    )
    ...
    def _create_non_primary_blocked_times(
        ...
        for cid in non_primary_ids:
            ...
            if calendar.calendar_type == CalendarType.BUNDLE:
                raise AppointmentTypeValidationError(
                    "Bundle calendars cannot be selected for appointment type events."
                )
            ...
            BlockedTime.objects.create(
                ...
                external_id=f"appointment-type-event-{event.id}-cal-{cid}",
            )
    ...
    def find_bookable_slots(
        ...
        where every slot in the appointment type has at least `required_count` calendars
        available, filtered by the resolved appointment type booking policy.
        ...
        Policy-awareness check: when no ``BookingPolicy`` resolves for the appointment type
        ...
        appointment-type-scoped window configured for its slot
        is unaffected (fall-through default, zero added queries -- see
        ``_slot_pools_with_appointment_type_scoped_flags``); a calendar WITH one is only
        counted toward its slot's ``required_count`` when the candidate window
        is fully covered by at least one of its appointment-type-scoped windows --
        narrowing only, never widening base availability.

        Appointment-type-scoped blocked time is applied AFTER base availability and
        ...
        Appointment-type-scoped quota rules are checked LAST, after base availability,
        ...
        The counting call (see ``slot_engine.fetch_appointment_type_scoped_quota_period_counts``)
        ...
        if slot_step <= datetime.timedelta(0):
            raise AppointmentTypeValidationError("slot_step must be a positive timedelta.")
        if duration <= datetime.timedelta(0):
            raise AppointmentTypeValidationError("duration must be a positive timedelta.")
        ...
        (
            slot_pool_by_id,
            appointment_type_scoped_calendar_ids_by_slot,
            appointment_type_scoped_block_calendar_ids_by_slot,
            appointment_type_scoped_quota_calendar_ids_by_slot,
        ) = self._slot_pools_with_appointment_type_scoped_flags(slots)
        ...
        # ------------------------------------------------------------------
        # Appointment-type-scoped availability windows -- self-gating early-out.
        # ------------------------------------------------------------------
        # `appointment_type_scoped_calendar_ids_by_slot` was already computed above by
        ...
        # calendar anywhere in the appointment type actually has an appointment-type-scoped window
        # configured do we pay for the (fixed, non-per-candidate) expanded
        # fetch below.
        appointment_type_scoped_spans_by_slot: slot_engine.AppointmentTypeScopedSpansBySlot = {}
        if appointment_type_scoped_calendar_ids_by_slot:
            configured_slot_ids = list(appointment_type_scoped_calendar_ids_by_slot.keys())
            configured_calendar_ids: set[int] = set()
            for ids in appointment_type_scoped_calendar_ids_by_slot.values():
                configured_calendar_ids.update(ids)
            appointment_type_scoped_spans_by_slot = (
                slot_engine.fetch_appointment_type_scoped_available_spans(
                    self.organization_id,
        ...
        # ------------------------------------------------------------------
        # Appointment-type-scoped blocked time -- self-gating early-out, same shape as
        # windows above.
        # ------------------------------------------------------------------
        appointment_type_scoped_block_spans_by_slot: slot_engine.AppointmentTypeScopedSpansBySlot = {}
        if appointment_type_scoped_block_calendar_ids_by_slot:
            configured_block_slot_ids = list(
                appointment_type_scoped_block_calendar_ids_by_slot.keys()
            )
            configured_block_calendar_ids: set[int] = set()
        ...
        # ------------------------------------------------------------------
        # Appointment-type-scoped quota rules -- self-gating early-out, same shape as
        # windows/blocks above.
        # ------------------------------------------------------------------
        # `appointment_type_scoped_quota_calendar_ids_by_slot` was already computed above
        # by folding a THIRD EXISTS() subquery into the SAME per-slot
        # membership query -- zero added round trips. Only when at least one
        # calendar anywhere in the appointment type actually has a quota rule configured
        ...
        # `slot_engine.fetch_appointment_type_scoped_quota_period_counts`). The number of
        ...
        appointment_type_scoped_quota_rules_by_slot: slot_engine.AppointmentTypeScopedQuotaRulesBySlot = {}
        appointment_type_scoped_quota_counts_by_slot: slot_engine.AppointmentTypeScopedQuotaCountsBySlot = {}
        if appointment_type_scoped_quota_calendar_ids_by_slot:
            configured_quota_slot_ids = list(
                appointment_type_scoped_quota_calendar_ids_by_slot.keys()
            )
            configured_quota_calendar_ids: set[int] = set()
            ...
            for slot_id, pool_ids in slot_pool_by_id.items():
                ...
                for cid in pool_ids:
                    if slot_engine.calendar_free_for_window(
                        ...
                        available_spans,
                        blocking_spans,
                        appointment_type_scoped_calendar_ids_by_slot.get(slot_id),
                        appointment_type_scoped_spans_by_slot.get(slot_id),
                        appointment_type_scoped_block_calendar_ids_by_slot.get(slot_id),
                        appointment_type_scoped_block_spans_by_slot.get(slot_id),
                        appointment_type_scoped_quota_calendar_ids_by_slot.get(slot_id),
                        appointment_type_scoped_quota_rules_by_slot.get(slot_id),
                        appointment_type_scoped_quota_counts_by_slot.get(slot_id),
                        week_start,
                    ):
                        available_count += 1
        ...
        # that instantiate AppointmentTypeService directly without DI), or when the
        # resolved policy is unconstrained (no BookingPolicy anywhere for this
        # appointment type), skip ALL policy work so the output is byte-for-byte the
