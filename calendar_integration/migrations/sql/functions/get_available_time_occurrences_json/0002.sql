-- PostgreSQL function to calculate recurring available time occurrences with bulk modifications and return as JSON
CREATE OR REPLACE FUNCTION get_available_time_occurrences_json_with_bulk_modifications(
...
DECLARE
    ...
    occurrences TEXT[] := '{}';
BEGIN
    -- Use the enhanced calculate_recurring_available_times_with_bulk_modifications function
    FOR occurrence_row IN 
        SELECT 
            ...
            source_available_time_id
        FROM calculate_recurring_available_times_with_bulk_modifications(p_available_time_id, p_start_date, p_end_date, p_max_occurrences)
    ...
    LOOP
        ...
        occurrences := array_append(
            ...
            json_build_object(
                ...
                'source_available_time_id', occurrence_row.source_available_time_id
            )::TEXT
        );
    END LOOP;
