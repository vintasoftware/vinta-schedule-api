...
BEGIN
    ...
    IF p_start_date > v_blocked_time.start_time THEN
        CASE v_rule.frequency
            WHEN 'DAILY' THEN
                -- Calculate how many intervals have passed since blocked time start
                v_occurrence_count := EXTRACT(EPOCH FROM (p_start_date - v_blocked_time.start_time))::INTEGER / (86400 * v_rule.interval);
            ...
            WHEN 'WEEKLY' THEN
                ...
                ELSE
                    ...
                    v_occurrence_count := EXTRACT(EPOCH FROM (p_start_date - v_blocked_time.start_time))::INTEGER / (604800 * v_rule.interval);
            ...
            WHEN 'YEARLY' THEN
                ...
                v_occurrence_count := EXTRACT(YEAR FROM p_start_date) - EXTRACT(YEAR FROM v_blocked_time.start_time);
    ...
    IF p_start_date > v_blocked_time.start_time AND v_rule.count IS NOT NULL THEN
        CASE v_rule.frequency
            WHEN 'DAILY' THEN
            ...
            WHEN 'WEEKLY' THEN
                ...
                ELSE
                    ...
                    v_total_occurrences := FLOOR(EXTRACT(EPOCH FROM (p_start_date - v_blocked_time.start_time)) / (604800 * v_rule.interval)) + 1;
            ...
            WHEN 'MONTHLY' THEN
                ...
                v_total_occurrences := FLOOR(((EXTRACT(YEAR FROM p_start_date) - EXTRACT(YEAR FROM v_blocked_time.start_time)) * 12 + 
                                     (EXTRACT(MONTH FROM p_start_date) - EXTRACT(MONTH FROM v_blocked_time.start_time))) / v_rule.interval) + 1;
            ...
            WHEN 'YEARLY' THEN
                ...
                v_total_occurrences := FLOOR((EXTRACT(YEAR FROM p_start_date) - EXTRACT(YEAR FROM v_blocked_time.start_time)) / v_rule.interval) + 1;
...
END;
$$ LANGUAGE plpgsql;
-- PostgreSQL function to calculate recurring blockedtime occurrences within a date range
-- This function handles all RecurrenceRule configurations and considers RecurrenceExceptions
...
BEGIN
    ...
    -- Check if blockedtime is recurring
    IF v_blocked_time.recurrence_rule_fk_id IS NULL THEN
        -- Non-recurring blockedtime, just check if it falls within range
        IF v_blocked_time.start_time >= p_start_date AND v_blocked_time.start_time <= p_end_date THEN
            occurrence_start := v_blocked_time.start_time;
            occurrence_end := v_blocked_time.end_time;
            is_exception := FALSE;
    ...
    -- Calculate blockedtime duration
    v_duration := v_blocked_time.end_time - v_blocked_time.start_time;
    ...
    -- Start from blockedtime's start time, but adjust to start_date if needed
    v_current_date := v_blocked_time.start_time;
    
    -- If the requested start_date is after the blockedtime start, we need to calculate
    -- where to begin to avoid counting occurrences outside the range
    IF p_start_date > v_blocked_time.start_time THEN
        CASE v_rule.frequency
            WHEN 'DAILY' THEN
                -- Calculate how many intervals have passed since blockedtime start
                v_occurrence_count := EXTRACT(EPOCH FROM (p_start_date - v_blocked_time.start_time))::INTEGER / (86400 * v_rule.interval);
                v_current_date := v_blocked_time.start_time + (v_occurrence_count * v_rule.interval || ' days')::INTERVAL;
