"""Backfill CalendarManagementToken.membership_user_id from user_id where an
OrganizationMembership(user_id, organization_id) pair exists.

Strategy
--------
For each CalendarManagementToken row whose membership_user_id is still NULL,
whose user_id IS NOT NULL, and whose (user_id, organization_id) has a matching
...
to an ``external_attendee`` instead of a registered user.  These rows are NOT
orphans — they legitimately have no membership — and are left with
``membership_user_id = NULL`` without any report or warning.
...
report is written to .vinta-ai-workflows/one-off-runs/ with columns:
    token_id, user_id, organization_id
...
``calendar_integration/migrations/_0034_backfill_helpers.py`` so tests can
...
def backfill_membership_user_id(apps, schema_editor):
    """Populate membership_user_id for CalendarManagementToken rows with a matching membership."""
    backfill_membership_user_id_sql()
    ...
    csv_filename = f"calendarmanagementtoken_orphans_{ts}.csv"
    ...
    try:
        ...
        with open(csv_path, "w", newline="") as f:
            writer = csv.writer(f)
            writer.writerow(["token_id", "user_id", "organization_id"])
            writer.writerows(orphans)
        logger.warning(
            "CalendarManagementToken backfill: %d orphan row(s) whose (user_id, organization_id) "
        ...
        logger.warning(
            "CalendarManagementToken backfill: %d orphan row(s) — could not write CSV to %s.  "
            "Logging individually below.",
            len(orphans),
            csv_path,
        )
        for token_id, user_id, organization_id in orphans:
            logger.warning(
                "  orphan CalendarManagementToken id=%s user_id=%s organization_id=%s",
                token_id,
                user_id,
                organization_id,
            )
...
def reverse_backfill_membership_user_id(apps, schema_editor):
    """Roll back: clear membership_user_id on all CalendarManagementToken rows."""
    with connection.cursor() as cursor:
        cursor.execute(
            "UPDATE calendar_integration_calendarmanagementtoken SET membership_user_id = NULL"
        )
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("calendar_integration", "0033_calendarmanagementtoken_membership_and_more"),
