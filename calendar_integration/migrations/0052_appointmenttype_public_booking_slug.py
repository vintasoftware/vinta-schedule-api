"""Add ``AppointmentType.public_booking_slug`` -- nullable, no default (step 1/3).

Phase 3b of the REST_CODE_GATED_SCHEDULING plan. Phase 3 shipped the codeless
public-appointment-type-booking route keyed by ``AppointmentType``'s integer primary key,
...
caller walk ``appointment_type_id`` 1..N and learn, from the 404/403/201 split, which
appointment types exist in ANY organization and which accept public scheduling. This
...
handed every pre-existing ``AppointmentType`` row the *same* slug -- the
opposite of what a unique, unguessable-per-appointment-type identifier requires, and
...
class Migration(migrations.Migration):
    """Add AppointmentType.public_booking_slug, nullable, no default (1/3)."""

    dependencies = [
        (
            "calendar_integration",
            "0051_calendarmanagementtoken_minted_by_membership_and_appointmenttype_duration",
        ),
    ]
