...
semantics are enforced here by a raw-SQL composite FK:
    ...
    ALTER TABLE calendar_integration_eventattendance
      ADD CONSTRAINT evattendance_membership_protect_fk
...
references it raises at commit (the attendance's dangling reference is detected
then). Because deleting a ``User`` CASCADEs to their ``OrganizationMembership``
rows, this FK also blocks ``User`` deletion while a live ``EventAttendance``
references the membership — an intended behaviour change.

Why DEFERRABLE (org-cascade reason)
-----------------------------------
Every other FK on ``calendar_integration_eventattendance`` is
...
collector removes the ``OrganizationMembership`` and the ``EventAttendance`` in an
order it chooses and is **blind** to the ``ForeignObject`` dependency (it carries
no DB constraint Django can see). If it deletes the membership first, a
...
attendance succeeds, while a membership-only delete (attendance still live) still
...
avoids a single long-held strong lock. This matters more here than for
CalendarOwnership: EventAttendance is higher-volume, so the non-blocking
``VALIDATE`` is what keeps the change online.

Reverse
-------
``DROP CONSTRAINT evattendance_membership_protect_fk`` — restores the schema to
the post-0031 state (no DB FK on the ForeignObject relation). No orphaned objects
remain.
"""
...
class Migration(migrations.Migration):
    """Add the raw-SQL composite PROTECT FK on EventAttendance.membership."""

    atomic = False

    dependencies = [
        ("calendar_integration", "0031_remove_eventattendance_user"),
