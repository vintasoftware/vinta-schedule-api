"""Integration tests for the codeless, slug-addressed public-appointment-type discovery reads.

Phase 9 adds two read endpoints under the same
``appointment-types/<public_slug>/`` prefix the Phase 3b codeless write already
uses, carrying no ``X-Booking-Code`` at all:

- ``GET  /public/booking/appointment-types/<public_slug>/bookable-slots/``
- ``POST /public/booking/appointment-types/<public_slug>/availability/``

They are gated on the addressed appointment type's own ``accepts_public_scheduling``,
mirroring the codeless write's 404/403 contract exactly (unknown slug ->
404, non-public appointment type -> 403) -- see
``test_booking_rest_codeless_appointment_type.py`` for that write-side contract and
...
public-appointment-type reads" section at the bottom of
``calendar_integration/booking_read_views.py`` for why (no appointment-type-level,
...
APPOINTMENT_TYPE_DURATION = datetime.timedelta(minutes=30)
...
def _bookable_slots_url(public_slug: str) -> str:
    return f"/public/booking/appointment-types/{public_slug}/bookable-slots/"


def _availability_url(public_slug: str) -> str:
    return f"/public/booking/appointment-types/{public_slug}/availability/"
...
@pytest.fixture
def organization():
    return baker.make(Organization, name="REST Codeless AppointmentType-Reads Test Org")
...
def _make_slot_calendar(organization: Organization, external_id: str) -> Calendar:
    ...
    # mirrors ``appointment_type_calendar`` in ``test_booking_rest_reads.py``.
    return baker.make(
        Calendar,
...
def slot_calendar(organization):
    return _make_slot_calendar(organization, "public-appointment-type-reads-slot-cal")


@pytest.fixture
def public_appointment_type(organization, slot_calendar):
    """An AppointmentType open to codeless discovery and booking."""
    return _make_appointment_type(
        organization,
        accepts_public_scheduling=True,
        slot_calendar=slot_calendar,
        name="Public Discovery AppointmentType",
        duration=APPOINTMENT_TYPE_DURATION,
    )


@pytest.fixture
def private_appointment_type(organization, slot_calendar):
    """An AppointmentType that does NOT accept public scheduling."""
    return _make_appointment_type(
        organization,
        accepts_public_scheduling=False,
        slot_calendar=slot_calendar,
        name="Private Discovery AppointmentType",
        duration=APPOINTMENT_TYPE_DURATION,
    )


@pytest.fixture
def public_appointment_type_with_no_duration(organization, slot_calendar):
    """A grandfathered public appointment type with no duration configured.

    ``AppointmentTypeService.create_appointment_type`` / ``update_appointment_type`` refuse to set
    ...
    return _make_appointment_type(
        ...
        accepts_public_scheduling=True,
        slot_calendar=slot_calendar,
        name="Grandfathered Public AppointmentType",
        duration=None,
    )


@pytest.fixture
def appointment_type_code(permission_service, organization, public_appointment_type):
    """A Phase 5 CREATE code scoped to ``public_appointment_type`` -- lets a test compare
    the codeless read against its code-gated counterpart for the same appointment type."""
...
def _slot_selections(appointment_type: AppointmentType, slot_calendar: Calendar) -> list[dict]:
    slot = appointment_type.slots.get(name="Physicians")
    return [{"slot_id": slot.id, "calendar_ids": [slot_calendar.id]}]


def _appointment_type_booking_payload(
    start_time: str, end_time: str, slot_selections: list[dict]
) -> dict:
    return {
...
class TestPublicBookableSlotsRead:
    def test_public_appointment_type_slots_readable_with_no_credential(
    ...
    def test_private_appointment_type_returns_403(self, anon_client, private_appointment_type):
        response = _get_slots(
            anon_client,
    ...
    def test_null_duration_returns_403_rather_than_guessing_a_length(
        self, anon_client, public_appointment_type_with_no_duration
    ):
        response = _get_slots(
            anon_client,
            public_appointment_type_with_no_duration.public_booking_slug,
            {"search_window_start": SEARCH_WINDOW_START, "search_window_end": SEARCH_WINDOW_END},
        )
    ...
    def test_flipping_accepts_public_scheduling_off_disables_the_read(
        ...
        before = _get_slots(anon_client, public_appointment_type.public_booking_slug, params)
        assert before.status_code == status.HTTP_200_OK, before.content

        public_appointment_type.accepts_public_scheduling = False
        public_appointment_type.save()

        after = _get_slots(anon_client, public_appointment_type.public_booking_slug, params)
    ...
    def test_duration_seconds_query_param_is_not_accepted_and_has_no_effect(
        ...
        all -- the search always uses ``appointment_type.duration``. Sending one (even a
        value the appointment type's actual pin would refuse at write time) must not
        ...
        without_param = _get_slots(anon_client, public_appointment_type.public_booking_slug, params)
    ...
    def test_matches_the_phase5_code_gated_read_for_the_same_appointment_type(
        self, anon_client, public_appointment_type, appointment_type_code
    ):
        ...
        (``AppointmentTypeService.find_bookable_slots``) -- for the same appointment type,
        ...
        coded_response = anon_client.get(
            "/public/booking/appointment-type-bookable-slots/",
            {
                "search_window_start": SEARCH_WINDOW_START,
                "search_window_end": SEARCH_WINDOW_END,
                "duration_seconds": int(APPOINTMENT_TYPE_DURATION.total_seconds()),
                "slot_step_seconds": 1800,
            },
            headers={"X-Booking-Code": appointment_type_code},
        )
    ...
    def test_every_proposal_is_actually_bookable_through_the_codeless_write(
        self, anon_client, organization, public_appointment_type, slot_calendar
    ):
        response = _get_slots(
            anon_client,
            public_appointment_type.public_booking_slug,
        ...
        selections = _slot_selections(public_appointment_type, slot_calendar)
        booking_response = anon_client.post(
            _events_url(public_appointment_type.public_booking_slug),
...
class TestPublicAvailabilityRead:
    ...
    def test_public_appointment_type_availability_readable_with_no_credential(
        self, anon_client, public_appointment_type
    ):
        response = _post_availability(
            anon_client, public_appointment_type.public_booking_slug, self._ranges_body()
        )
    ...
    def test_private_appointment_type_returns_403(self, anon_client, private_appointment_type):
        response = _post_availability(
            anon_client, private_appointment_type.public_booking_slug, self._ranges_body()
        )
    ...
    def test_null_duration_returns_403_rather_than_reporting_availability(
        ...
        """An appointment type the write side always refuses (fail closed, no duration
    ...
    def test_matches_the_phase5_code_gated_read_for_the_same_appointment_type(
        ...
        coded_response = anon_client.post(
            "/public/booking/appointment-type-availability/",
            self._ranges_body(),
            format="json",
            headers={"X-Booking-Code": appointment_type_code},
        )
    ...
    def test_available_calendar_ids_are_retained_for_slot_selection(
        self, anon_client, public_appointment_type, slot_calendar
    ):
        """Unlike the bookable-slots read, this endpoint deliberately keeps
        ``available_calendar_ids`` per slot -- appointment type booking's
        ``slot_selections`` genuinely needs those ids to build a valid
        create request. See the "Codeless discovery is appointment-type-aggregated"
...
class TestWindowReadsAreNotShipped:
    def test_availability_windows_route_does_not_exist(self, anon_client, public_appointment_type):
        response = anon_client.get(
            f"/public/booking/appointment-types/{public_appointment_type.public_booking_slug}"
            "/availability-windows/"
        )

        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_unavailable_windows_route_does_not_exist(self, anon_client, public_appointment_type):
        response = anon_client.get(
            f"/public/booking/appointment-types/{public_appointment_type.public_booking_slug}"
            "/unavailable-windows/"
        )
