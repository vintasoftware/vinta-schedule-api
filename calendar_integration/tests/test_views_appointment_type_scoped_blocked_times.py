...
Direct mirror of ``test_views_appointment_type_scoped_availability_windows.py`` for
blocks. Covers:
...
def _list_url(appointment_type_id: int, slot_id: int) -> str:
    return reverse(
        "api:AppointmentTypeScopedBlockedTimes-list",
...
def _detail_url(appointment_type_id: int, slot_id: int, pk: int) -> str:
    return reverse(
        "api:AppointmentTypeScopedBlockedTimes-detail",
...
class TestAppointmentTypeScopedBlockedTimeLifecycle:
    def test_full_lifecycle_as_owner(
        ...
        assert block_data["appointment_type_slot_id"] == appointment_type_slot.id
        ...
        retrieve_response = client.get(
            _detail_url(appointment_type.id, appointment_type_slot.id, block_id)
        )
...
@pytest.mark.django_db
class TestAppointmentTypeScopedBlockedTimeNonDisclosure:
...
@pytest.mark.django_db
class TestAppointmentTypeScopedBlockedTimeOrphanedBookings:
...
@pytest.mark.django_db
class TestAppointmentTypeScopedBlockedTimeQueryBudget:
    def test_list_query_count_is_bounded_and_does_not_scale_with_block_count(
        ...
        """`AppointmentTypeScopedBlockedTimeSerializer` never nests `calendar` (it
