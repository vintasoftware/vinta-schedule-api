...
Direct mirror of ``test_views_appointment_type_scoped_blocked_times.py`` for quota
...
def _list_url(appointment_type_id: int, slot_id: int) -> str:
    return reverse(
        "api:AppointmentTypeScopedQuotaRules-list",
...
def _detail_url(appointment_type_id: int, slot_id: int, pk: int) -> str:
    return reverse(
        "api:AppointmentTypeScopedQuotaRules-detail",
...
class TestAppointmentTypeScopedQuotaRuleLifecycle:
    def test_full_lifecycle_as_owner(
        ...
        assert rule_data["appointment_type_slot_id"] == appointment_type_slot.id
        assert rule_data["period"] == QuotaPeriod.WEEK
        assert rule_data["cap"] == 3
        rule_id = rule_data["id"]

        assert (
            AppointmentTypeSlotQuotaRule.objects.filter_by_organization(calendar.organization_id)
            .filter(id=rule_id)
            .exists()
        )
        ...
        retrieve_response = client.get(
            _detail_url(appointment_type.id, appointment_type_slot.id, rule_id)
        )
        ...
        assert not AppointmentTypeSlotQuotaRule.original_manager.filter(id=rule_id).exists()
    ...
    def test_patch_requires_positive_cap(
        ...
        rule = AppointmentTypeSlotQuotaRule.objects.filter_by_organization(
            calendar.organization_id
        ).get(id=rule_id)
        assert rule.cap == 3
...
@pytest.mark.django_db
class TestAppointmentTypeScopedQuotaRuleUniqueness:
    ...
    def test_update_colliding_with_existing_period_is_validation_error(
        ...
        update_response = client.patch(
            _detail_url(appointment_type.id, appointment_type_slot.id, day_rule_id),
            {"period": QuotaPeriod.WEEK},
            format="json",
        )
        ...
        # partially apply.
        reloaded = AppointmentTypeSlotQuotaRule.objects.filter_by_organization(
            calendar.organization_id
        ).get(id=day_rule_id)
...
@pytest.mark.django_db
class TestAppointmentTypeScopedQuotaRuleNonDisclosure:
    ...
    def test_owner_of_other_calendar_in_same_appointment_type_denied_same_shape_as_stranger(
        ...
        assert not AppointmentTypeSlotQuotaRule.original_manager.filter(
            appointment_type_slot_fk=appointment_type_slot
        ).exists()
...
@pytest.mark.django_db
class TestAppointmentTypeScopedQuotaRuleQueryBudget:
    def test_list_query_count_is_bounded_and_does_not_scale_with_rule_count(
        ...
        """`AppointmentTypeScopedQuotaRuleSerializer` never nests `calendar` (it sources
