...
publicly-scheduling appointment type -- see 0051's ``AppointmentType.duration`` help_text.
0058 backfills ``duration = timedelta(minutes=30)`` onto every pre-existing
``AppointmentType`` row with a NULL duration, public and private alike (see
...
* the backfill helper itself
  ...
  backfill_appointmenttype_duration``), driven directly against the database.
  ...
  identical before and after 0058. The LIVE ``AppointmentType`` model can
...
AFTER_BACKFILL = "0058_backfill_appointmenttype_duration"
...
class TestBackfillHelper:
    ...
    def test_fills_every_null_duration_public_and_private_alike(self, organization):
        public_appointment_type = baker.make(
            AppointmentType,
            organization=organization,
            accepts_public_scheduling=True,
            duration=None,
        )
        private_appointment_type = baker.make(
            AppointmentType,
            organization=organization,
        ...
        public_appointment_type.refresh_from_db()
        private_appointment_type.refresh_from_db()
        assert public_appointment_type.duration == THIRTY_MINUTES
        assert private_appointment_type.duration == THIRTY_MINUTES
    ...
    def test_appointment_type_with_existing_duration_is_left_untouched(self, organization):
        ...
        needs_fill = baker.make(AppointmentType, organization=organization, duration=None)
    ...
    def test_rerun_after_partial_failure_fills_only_the_still_null_subset(self, organization):
        appointment_types = [
            baker.make(AppointmentType, organization=organization, duration=None) for _ in range(4)
        ]

        backfill_appointmenttype_duration()
        for appointment_type in appointment_types:
            appointment_type.refresh_from_db()
            assert appointment_type.duration == THIRTY_MINUTES
        ...
        renulled_ids = [appointment_types[0].id, appointment_types[1].id]
        untouched = appointment_types[2]
        explicitly_set = appointment_types[3]
    ...
    def test_drain_loop_drains_multiple_batches_and_picks_up_concurrent_insert(
        ...
        appointment_types = [
            baker.make(AppointmentType, organization=organization, duration=None) for _ in range(5)
        ]

        real_cursor = connection.cursor
        call_count = {"n": 0}
        late_appointment_type_id: dict[str, int] = {}

        def _patched_cursor(*args, **kwargs):
            call_count["n"] += 1
            if call_count["n"] == 2 and "id" not in late_appointment_type_id:
        ...
        assert "id" in late_appointment_type_id, (
            "the concurrent insert never landed -- test setup is broken, not the backfill"
        )

        all_ids = [appointment_type.id for appointment_type in appointment_types] + [
            late_appointment_type_id["id"]
        ]
...
class TestBackfillMigrationChain:
    ...
    ``AppointmentType`` model matches the schema at every point in this test,
    ...
    def test_forward_reverse_reapply_round_trip(self, organization):
        ...
        try:
            ...
            already_set_appointment_type = baker.make(
                AppointmentType,
                organization=organization,
                duration=datetime.timedelta(seconds=45),
            )
            ids = [
                public_appointment_type.id,
                private_appointment_type.id,
                already_set_appointment_type.id,
            ]
            ...
            assert first_pass[public_appointment_type.id] == THIRTY_MINUTES
            assert first_pass[private_appointment_type.id] == THIRTY_MINUTES
            assert first_pass[already_set_appointment_type.id] == datetime.timedelta(seconds=45)
