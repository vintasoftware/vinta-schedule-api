"""Integration tests for booking-time policy enforcement on ``create_appointment_type_event``.

Coverage:
- **Lead-time violation**: appointment type booking too soon is rejected.
- **Max-horizon violation**: appointment type booking too far ahead is rejected.
- **Buffer violation (event-envelope)**: appointment type booking inside an existing event's dead zone
  is rejected.
- **Compliant booking**: a booking satisfying all rules succeeds.
- **No policy → unchanged behavior** (data-presence off-state): existing appointment type booking tests
...
- **No rows on violation**: neither events nor blocked times are persisted after a rejected
  appointment type booking (the transaction-atomicity guarantee).
...
- **Discovery/enforcement agreement**: a time ``find_bookable_slots`` would NOT offer is
  also rejected by ``create_appointment_type_event``; acceptance scenario #7 (horizon case) asserted
  for the appointment type path.
"""
...
def _managed_cal(org: Organization) -> Calendar:
    ...
    return Calendar.objects.create(
        organization=org,
        name=f"appointment-type-enforce-cal-{n}",
        external_id=f"appointment-type-enforce-{n}",
...
def _make_service(org: Organization) -> AppointmentTypeService:
...
def _make_appointment_type(
    service: AppointmentTypeService, *calendars: Calendar
) -> AppointmentType:
    """Create a single-slot appointment type whose pool contains all provided calendars.

    ``duration`` matches ``_BOOKING_START``/``_BOOKING_END`` (1 hour) below --
    a public appointment type with no duration fails closed in
    ``CalendarPermissionService.can_perform_appointment_type_scheduling``, which would
    ...
    return service.create_appointment_type(
        AppointmentTypeInputData(
            name=f"appointment-type-enforce-{_unique()}",
            accepts_public_scheduling=True,
...
def _book(
    ...
    """Attempt an appointment type booking of `calendar` for the given window."""
    slot = appointment_type.slots.get()
    return service.create_appointment_type_event(
        AppointmentTypeEventInputData(
            title="Test AppointmentType Booking",
            description="",
            start_time=start,
            end_time=end,
            timezone="UTC",
...
class TestAppointmentTypeBookingPolicyEnforcement:
    """Policy enforcement on the appointment type ``create_appointment_type_event`` path."""

    @pytest.fixture
    def org(self, db):
        return Organization.objects.create(
            name="AppointmentTypeEnforceOrg", should_sync_rooms=False
        )
    ...
    def test_no_policy_write_unchanged(self, org, calendar):
        """With no BookingPolicy anywhere, create_appointment_type_event behaves exactly as before."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        event = _book(service, appointment_type, calendar)
    ...
    def test_lead_time_violation_raises(self, org, calendar):
        """An appointment type booking whose start < now + lead_time is rejected."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        lead_seconds = int((_BOOKING_START - _NOW).total_seconds()) + 3600  # 1 extra hour
        create_booking_policy(appointment_type=appointment_type, lead_time_seconds=lead_seconds)

        with pytest.raises(BookingPolicyViolationError):
            with patch(
                "calendar_integration.services.appointment_type_service.timezone"
            ) as mock_tz:
                mock_tz.now.return_value = _NOW
                _book(service, appointment_type, calendar)

    def test_lead_time_compliant_succeeds(self, org, calendar):
        """A booking with zero lead_time (no constraint) succeeds."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        create_booking_policy(appointment_type=appointment_type, lead_time_seconds=0)
    ...
    def test_horizon_violation_raises(self, org, calendar):
        """An appointment type booking whose start > now + max_horizon is rejected.

        Covers acceptance scenario #7: beyond-horizon appointment type booking is
        rejected and nothing is persisted.
        """
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        short_horizon = int((_BOOKING_START - _NOW).total_seconds()) - 3600
        create_booking_policy(appointment_type=appointment_type, max_horizon_seconds=short_horizon)
    ...
    def test_horizon_zero_means_unbounded(self, org, calendar):
        ...
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        create_booking_policy(appointment_type=appointment_type, max_horizon_seconds=0)
    ...
    def test_buffer_after_violation_raises(self, org, calendar):
        """An appointment type booking inside an existing event's after-buffer dead zone is rejected."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        ...
        create_booking_policy(appointment_type=appointment_type, buffer_after_seconds=3601)
    ...
    def test_buffer_after_flush_boundary_allowed(self, org, calendar):
        ...
        existing_end = datetime.datetime(2030, 6, 1, 9, 0, 0, tzinfo=datetime.UTC)
        _event_in_db(calendar, existing_start, existing_end)
        create_booking_policy(appointment_type=appointment_type, buffer_after_seconds=3600)

        event = _book(service, appointment_type, calendar)
        assert event is not None

    def test_buffer_before_violation_raises(self, org, calendar):
        """An appointment type booking inside an existing event's before-buffer dead zone is rejected."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        ...
        _event_in_db(calendar, existing_start, existing_end)
        create_booking_policy(appointment_type=appointment_type, buffer_before_seconds=1801)
    ...
    def test_no_rows_persisted_on_lead_time_violation(self, org, calendar):
        """A lead-time violation rolls back the entire appointment type write — zero rows persisted."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        ...
        assert (
            CalendarEventAppointmentTypeSelection.objects.filter_by_organization(org.id).count()
            == 0
        )

    def test_no_rows_persisted_on_horizon_violation(self, org, calendar):
        """A max-horizon violation rolls back the entire appointment type write — zero rows persisted."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        short_horizon = int((_BOOKING_START - _NOW).total_seconds()) - 3600
        create_booking_policy(appointment_type=appointment_type, max_horizon_seconds=short_horizon)
    ...
    def test_discovery_enforcement_agreement_lead_time(self, org, calendar):
        """A time find_bookable_slots would not offer is also rejected by create_appointment_type_event.
        ...
        gs_discovery = AppointmentTypeService(booking_policy_service=booking_policy_service)
        ...
        # Enforcement: create_appointment_type_event must also reject.
        with pytest.raises(BookingPolicyViolationError):
            with patch(
                "calendar_integration.services.appointment_type_service.timezone"
            ) as mock_tz:
    ...
    def test_discovery_enforcement_agreement_horizon(self, org, calendar):
        ...
        This is the appointment-type-path assertion of Acceptance scenario #7.
        """
        booking_policy_service = BookingPolicyService()
        booking_policy_service.initialize(org)
    ...
    def test_discovery_enforcement_agreement_positive(self, org, calendar):
        """A time find_bookable_slots DOES offer is accepted by create_appointment_type_event."""
        booking_policy_service = BookingPolicyService()
        booking_policy_service.initialize(org)
        ...
        event = _book(service, appointment_type, calendar)
        assert event is not None
        # The full appointment type write actually persisted (symmetry with the
    ...
    # ------------------------------------------------------------------
    # Policy resolves via resolve_for_appointment_type (org-default fallthrough)
    # ------------------------------------------------------------------

    def test_org_default_policy_enforced_for_appointment_type(self, org, calendar):
        """An org-default policy is enforced for an appointment type with no explicit policy."""
        service = _make_service(org)
        appointment_type = _make_appointment_type(service, calendar)
        # No appointment-type-specific policy — set org default with a blocking lead time.
        lead_seconds = int((_BOOKING_START - _NOW).total_seconds()) + 3600
