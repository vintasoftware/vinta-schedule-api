...
``calculate_appointment_type_quota_period_counts`` / its JSON wrapper
``get_appointment_type_quota_period_counts_json``, exercised through the Django
ORM wrapper ``GetAppointmentTypeQuotaPeriodCountsJSON``.
...
bookings made outside the appointment type never counting.
"""
...
def other_slot(
    ...
    """A second slot in the same appointment type, sharing the same calendar -- proves
...
def _create_appointment_type_booking(
    organization: Organization,
    calendar: Calendar,
    ...
    """A booking made THROUGH `appointment_type_slot` for `calendar` -- CalendarEvent plus
    the CalendarEventAppointmentTypeSelection link, mirroring what
    AppointmentTypeService.create_appointment_type_event writes."""
    ...
    CalendarEventAppointmentTypeSelection.objects.create(
        organization=organization, event=event, slot=appointment_type_slot, calendar=calendar
    )
    return event
...
def _create_direct_booking(
    ...
    """A booking made directly on the calendar, with NO appointment type selection --
    must never count toward any appointment type's quota."""
    end = start + datetime.timedelta(hours=duration_hours)
...
def test_day_period_buckets_by_calendar_day(
    organization: Organization, calendar: Calendar, appointment_type_slot: AppointmentTypeSlot
) -> None:
    # Two bookings on the same day, one on the next day.
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5, 9)
    )
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5, 14)
    )
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 6, 9)
    )
...
def test_month_period_buckets_by_calendar_month(
    ...
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5)
    )
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 28)
    )
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 2, 3)
    )
...
@pytest.mark.django_db
def test_week_period_monday_start_appointment_types_sunday_into_preceding_week(
    organization: Organization, calendar: Calendar, appointment_type_slot: AppointmentTypeSlot
) -> None:
    ...
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 11)
    )  # Sunday
...
def test_cancelled_booking_excluded(
    ...
    kept = _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5)
    )
    cancelled = _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5, 14)
    )

    # Cancelling an appointment-type booking deletes the CalendarEvent row (mirrors
    # AppointmentTypeService.cancel_appointment_type_event), which cascades its
    # CalendarEventAppointmentTypeSelection.
    cancelled.delete()
...
def test_reschedule_across_period_boundary_moves_the_count(
    ...
    booking = _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5)
    )
    ...
    # CalendarEventAppointmentTypeSelection row (it isn't touched), only the time moves.
...
# ---------------------------------------------------------------------------
# Bookings outside the appointment type (or in a different slot) are excluded
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_direct_booking_outside_appointment_type_excluded(
    organization: Organization, calendar: Calendar, appointment_type_slot: AppointmentTypeSlot
) -> None:
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5)
...
def test_booking_through_a_different_slot_excluded(
    ...
    """A booking made through `other_slot` (same calendar, same appointment type, different
    slot) must not count toward `appointment_type_slot`'s quota."""
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 1, 5)
    )
    _create_appointment_type_booking(organization, calendar, other_slot, _utc(2026, 1, 5, 14))
...
def test_search_window_excludes_bookings_outside_it(
    ...
    _create_appointment_type_booking(
        organization, calendar, appointment_type_slot, _utc(2026, 2, 5)
    )  # outside range
