"""Tests for appointment-type-scoped blocked-time writes on ``AppointmentTypeService``.
...
# ---------------------------------------------------------------------------
# create_appointment_type_scoped_blocked_time
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_admin_happy_path(
    ...
    with patch("vinta_audit_logs.tasks.persist_audit_record") as mock_task:
        with django_capture_on_commit_callbacks(execute=True):
            result = service.create_appointment_type_scoped_blocked_time(
    ...
    assert block.appointment_type_slot_fk_id == appointment_type_slot.id
    ...
    assert (
        BlockedTime.objects.for_appointment_type_slot(appointment_type_slot.id)
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_owner_happy_path(
    service: AppointmentTypeService,
    owner_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_denies_non_owner_without_disclosing_appointment_type(
    service: AppointmentTypeService,
    stranger_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_denies_owner_outside_target_calendar(
    service: AppointmentTypeService,
    other_owner_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
    ...
    assert (
        not BlockedTime.objects.unscoped()
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_recurrence_and_timezone_round_trip(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_unique_external_id_per_calendar(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
    ...
    first = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
        appointment_type_slot_id=appointment_type_slot.id,
        calendar_id=calendar.id,
        start_time=_utc(2025, 9, 2, 9),
    ...
    second = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
        appointment_type_slot_id=appointment_type_slot.id,
        calendar_id=calendar.id,
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_detects_orphaned_bookings_on_every_create(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    ...
    """Unlike windows (only the FIRST one can orphan), every appointment-type-scoped
...
# ---------------------------------------------------------------------------
# update_appointment_type_scoped_blocked_time
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_update_appointment_type_scoped_blocked_time_records_diff(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
    ...
    created = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
        appointment_type_slot_id=appointment_type_slot.id,
        calendar_id=calendar.id,
        start_time=_utc(2025, 9, 2, 9),
    ...
    with patch("vinta_audit_logs.tasks.persist_audit_record") as mock_task:
        with django_capture_on_commit_callbacks(execute=True):
            result = service.update_appointment_type_scoped_blocked_time(
                acting_user=admin_user,
                block_id=block_id,
...
@pytest.mark.django_db
def test_update_appointment_type_scoped_blocked_time_timezone_round_trip(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_update_appointment_type_scoped_blocked_time_explicit_none_clears_recurrence(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_update_appointment_type_scoped_blocked_time_omitted_rrule_string_leaves_it_unchanged(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_update_appointment_type_scoped_blocked_time_denies_non_owner(
    service: AppointmentTypeService,
    admin_user: User,
    stranger_user: User,
    calendar: Calendar,
    ...
    with pytest.raises(AppointmentTypeSlotConfigNotFoundError):
        service.update_appointment_type_scoped_blocked_time(
...
@pytest.mark.django_db
def test_update_appointment_type_scoped_blocked_time_reports_newly_orphaned_booking(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type: AppointmentType,
    ...
    it as orphaned, and modifies neither the event nor the appointment type selection."""
    now = django_timezone.now()
    tuesday = _next_weekday(now, weekday=1)
...
# ---------------------------------------------------------------------------
# delete_appointment_type_scoped_blocked_time
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_delete_appointment_type_scoped_blocked_time_admin(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
    ...
    with patch("vinta_audit_logs.tasks.persist_audit_record") as mock_task:
        with django_capture_on_commit_callbacks(execute=True):
            service.delete_appointment_type_scoped_blocked_time(
                acting_user=admin_user, block_id=block_id
            )
...
@pytest.mark.django_db
def test_delete_appointment_type_scoped_blocked_time_denies_non_owner(
    service: AppointmentTypeService,
    admin_user: User,
    stranger_user: User,
    calendar: Calendar,
    ...
    with pytest.raises(AppointmentTypeSlotConfigNotFoundError):
        service.delete_appointment_type_scoped_blocked_time(
            acting_user=stranger_user, block_id=block_id
        )
...
@pytest.mark.django_db
def test_deleting_slot_through_update_appointment_type_cascades_appointment_type_scoped_blocks(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type: AppointmentType,
    ...
    # on BlockedTime.appointment_type_slot) to every appointment-type-scoped block that referenced it.
...
# ---------------------------------------------------------------------------
# Removing a calendar from a slot keeps its appointment-type-scoped blocks
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_removing_calendar_from_slot_keeps_appointment_type_scoped_blocks(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    other_calendar: Calendar,
    ...
    a slot's membership, its appointment-type-scoped blocked time is kept, not deleted --
    ...
    # Create appointment-type-scoped blocks for BOTH calendars.
    block1_result = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
    ...
    block2_result = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
        appointment_type_slot_id=slot.id,
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_recurring_blocked_time_detects_orphaned_bookings_on_future_occurrence(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type: AppointmentType,
    ...
    recurring_block = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
        appointment_type_slot_id=appointment_type_slot.id,
        calendar_id=calendar.id,
    ...
    overlapping_block = service.create_appointment_type_scoped_blocked_time(
        acting_user=admin_user,
        appointment_type_slot_id=appointment_type_slot.id,
        calendar_id=calendar.id,
        start_time=block_start,
...
@pytest.mark.django_db
def test_create_appointment_type_scoped_blocked_time_detects_partially_overlapping_booking_as_orphaned(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    appointment_type: AppointmentType,
