...
@pytest.mark.django_db
def test_validate_selections_grandfathers_departed_calendar_without_addition(
    grouped_service, clinic_group, internal_calendars
):
    """Updating an event that already holds a departed calendar, without
    adding anything, succeeds."""
    physicians_slot = clinic_group.slots.get(name="Physicians")
    rooms_slot = clinic_group.slots.get(name="Rooms")
    ...
    updated_group = _remove_phys_a_from_roster(grouped_service, clinic_group, internal_calendars)
    slots = list(updated_group.slots.order_by("order", "id"))

    # Re-submitting the SAME selection (phys_a still on the event, nothing new
    # added) must succeed even though phys_a has left the roster.
    result = grouped_service._validate_selections(
        updated_group,
        slots,
        [
            CalendarGroupSlotSelectionInputData(
    ...
    assert result[physicians_slot.id].calendar_ids == [internal_calendars["phys_a"].id]


@pytest.mark.django_db
def test_validate_selections_still_rejects_genuinely_new_calendar_on_update(
    grouped_service, clinic_group, internal_calendars
):
    """A grandfathered calendar passes, but adding a calendar that was never
    selected for this event and is not currently in the roster is still
    rejected -- the "added" half of the added-vs-retained split. Proves the
    caller cannot forge "already recorded" by simply naming a calendar: only
    calendars actually persisted on this event's CalendarEventGroupSelection
    ...
    with pytest.raises(CalendarGroupValidationError):
        grouped_service._validate_selections(
            updated_group,
            slots,
            [
                # phys_a is retained (grandfathered) AND a never-selected
                # outsider is newly added -> the addition is still rejected.
                CalendarGroupSlotSelectionInputData(
                    slot_id=physicians_slot.id,
                    calendar_ids=[internal_calendars["phys_a"].id, outsider.id],
                ),
                CalendarGroupSlotSelectionInputData(
                    slot_id=rooms_slot.id, calendar_ids=[internal_calendars["room_1"].id]
                ),
