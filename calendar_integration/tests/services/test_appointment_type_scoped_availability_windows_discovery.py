"""Tests for appointment-type-scoped availability windows in discovery and booking
validation.

Covers:
- The surgeon scenario end to end: a Tuesday/Thursday window in one appointment type
  narrows discovery there while a second appointment type with no appointment-type-scoped
...
- The required "unchanged path" test: an appointment type with NO appointment-type-scoped
  ...
  the SAME number of queries as the engine without appointment-type-scoped window
...
def calendar(organization: Organization) -> Calendar:
    ...
    (a single wide AvailableTime block; the appointment-type-scoped window is what
...
# ---------------------------------------------------------------------------
# Acceptance 1 -- narrowing works, and is scoped to one appointment type.
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_appointment_type_scoped_window_narrows_discovery_in_one_appointment_type_only(
...
@pytest.mark.django_db
def test_appointment_type_scoped_window_outside_base_availability_offers_nothing(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_create_appointment_type_event_rejects_calendar_outside_appointment_type_scoped_window(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
    ...
    assert exc_info.value.rule_type == AppointmentTypeScopedRuleType.OUTSIDE_WINDOW
...
@pytest.mark.django_db
def test_create_appointment_type_event_allows_calendar_inside_appointment_type_scoped_window(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
...
@pytest.mark.django_db
def test_reschedule_appointment_type_event_rejects_move_outside_appointment_type_scoped_window(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
...
# ---------------------------------------------------------------------------
# check_appointment_type_availability -- same intersection, per-range shape.
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_check_appointment_type_availability_intersects_appointment_type_scoped_window(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
    ...
    [tue_result, wed_result] = service.check_appointment_type_availability(
...
def test_unconfigured_appointment_type_discovery_is_byte_for_byte_unchanged(
    ...
    ``check_appointment_type_availability``, against this exact fixture shape: a
    single-slot appointment type with one managed calendar and one 5-day AvailableTime
    row) were captured against the engine without appointment-type-scoped window support
    (this file's appointment-type-scoped-window changes reverted via ``git stash``)
    ...
    # 5 days (Mon-Fri), matching output captured before appointment-type-scoped window
    # support existed.
...
# ---------------------------------------------------------------------------
# FIX A: Recurrence exception handling for appointment-type-scoped masters
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_appointment_type_scoped_recurring_exception_is_honored_when_master_is_appointment_type_scoped(
    organization: Organization,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
) -> None:
    """Verifies that when an appointment-type-scoped recurring AvailableTime master has a
    per-occurrence recurrence exception that is also appointment-type-scoped, the
    ...
    master is appointment-type-scoped, ensuring appointment-type-scoped exception rows are found
    instead of being silently skipped by the default manager.
    """
    # Create an appointment-type-scoped recurring master: 9-10 AM every Tuesday and Thursday.
    ...
    # Create an appointment-type-scoped exception for it: move it to 10-11 AM.
    ...
    # Create the modified occurrence (appointment-type-scoped).
    ...
    # Fetch the appointment-type-scoped master with recurring_occurrences pre-annotated.
    # This simulates how slot_engine fetches appointment-type-scoped masters.
    master_with_occurrences = (
        ...
        .filter(appointment_type_slot_fk_id=surgery_slot.id, id=master.id)
        .first()
    )

    assert master_with_occurrences is not None
    # Expand occurrences using the pre-fetched master.
    # The exception-instance lookup should find the appointment-type-scoped exception
    # because _get_occurrences_in_range now uses _base_manager when master
    # is appointment-type-scoped.
    ...
    # This verifies that the exception lookup found the appointment-type-scoped modified occurrence.
...
def secondary_calendar(organization: Organization) -> Calendar:
    """A second calendar (non-primary in an appointment type) for testing multi-calendar
    scenarios."""
    cal = Calendar.objects.create(
        organization=organization,
        name="Dr. Chen",
...
@pytest.mark.django_db
def test_reschedule_appointment_type_event_with_non_primary_calendar_outside_window(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    ...
    """Reschedule an appointment-type event that includes a non-primary calendar to a time
    outside that non-primary calendar's appointment-type-scoped window is rejected.

    Verifies that reschedule_appointment_type_event enforces windows for ALL selected
...
@pytest.mark.django_db
def test_reschedule_appointment_type_event_with_non_primary_calendar_inside_windows(
    service: AppointmentTypeService,
    admin_user: User,
    calendar: Calendar,
    secondary_calendar: Calendar,
    ...
    Positive case verifying that reschedule_appointment_type_event does not over-reject
    ...
    # secondary 9-12). Should NOT raise AppointmentTypeScopedRuleViolationError
    # (the positive case for the enforcement check).
    # Note: the window check happens before the permission check, so if we don't
    # get AppointmentTypeScopedRuleViolationError, the window enforcement passed.
    try:
        service.reschedule_appointment_type_event(
            event_id=event.id,
    ...
    except AppointmentTypeScopedRuleViolationError:
        ...
        # passed; other parts of reschedule_appointment_type_event may fail.
        pass
