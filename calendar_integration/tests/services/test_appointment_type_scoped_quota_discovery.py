"""Tests for appointment-type-scoped quota rules in discovery and booking validation.
...
- Explicit booking past the cap is rejected with
  ``AppointmentTypeScopedRuleType.QUOTA_CONSUMED``.
- The headline risk appointment-type-scoped quota enforcement exists to guard against: the quota-counting
...
QUOTA_COUNTING_FUNCTION = "get_appointment_type_quota_period_counts_json"
...
def surgery_appointment_type(organization: Organization) -> AppointmentType:
    ...
    # appointment type below (``_book_via_service``'s default) -- a public appointment type with no
...
def reschedule_admin_user(organization: Organization) -> User:
    ...
    which must exercise the FULL ``reschedule_appointment_type_event`` write path
...
def reschedule_calendar_service(
    ...
    succeeds for the appointment-type booking's primary create (no permissions need to
    be attached to this token -- ``create_appointment_type_event`` sets
    ``appointment_type_authorized=True`` on the underlying create, which bypasses the
...
def _grant_reschedule_permission(
    ...
    passes for a subsequent ``reschedule_appointment_type_event`` call on ``event``."""
...
def _seed_booking(
    organization: Organization,
    surgery_appointment_type: AppointmentType,
    ...
    ``CalendarEvent`` plus its ``CalendarEventAppointmentTypeSelection`` link, exactly
    what ``AppointmentTypeService.create_appointment_type_event`` would persist for a
    single-calendar slot selection (mirrors the quota period-counting tests'
    ``_create_appointment_type_booking`` helper). Used to seed MULTIPLE pre-existing
    ...
    ``create_appointment_type_event`` more than once per test would collide on that
    constraint. The REJECTION/ACCEPTANCE tests below still exercise the real
    write path via ``create_appointment_type_event`` (only ONE successful write each).
    """
    ...
    event = CalendarEvent.objects.create(
        ...
        timezone="UTC",
        appointment_type=surgery_appointment_type,
    )
...
def _book_via_service(
    ...
    """Book through the real write path (``create_appointment_type_event``) -- used
...
def test_capped_calendar_hidden_for_period_and_offered_next_period(
    ...
    create_appointment_type_slot_quota_rule(
        organization=organization,
        appointment_type_slot=surgery_slot,
        calendar=calendar,
    ...
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 1, 9)
    )
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 2, 9)
    )
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 3, 9)
    )
...
@pytest.mark.django_db
def test_check_appointment_type_availability_excludes_capped_calendar(
    service: AppointmentTypeService,
    organization: Organization,
    calendar: Calendar,
    surgery_appointment_type: AppointmentType,
    ...
    [capped_result, free_result] = service.check_appointment_type_availability(
...
def test_cancelling_booking_frees_quota_with_no_further_action(
    ...
    # Cancelling an appointment-type booking deletes the CalendarEvent row (mirrors
    # AppointmentTypeService.cancel_appointment_type_event, which this test bypasses to
    ...
    # CalendarEventAppointmentTypeSelection.event_fk removes the appointment-type-selection link
    # too, so the counting function sees one fewer live booking -- exactly
    # what cancel_appointment_type_event's own deletion of the primary event achieves.
    third.delete()
...
def test_daily_and_weekly_rules_both_enforced(
    ...
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, WEEK1_MONDAY.replace(hour=9)
    )
...
@pytest.mark.django_db
def test_create_appointment_type_event_rejects_calendar_over_quota(
    service: AppointmentTypeService,
    organization: Organization,
    calendar: Calendar,
    surgery_appointment_type: AppointmentType,
    ...
    assert exc_info.value.rule_type == AppointmentTypeScopedRuleType.QUOTA_CONSUMED
...
@pytest.mark.django_db
def test_create_appointment_type_event_allows_calendar_under_quota(
    service: AppointmentTypeService,
    organization: Organization,
    calendar: Calendar,
    surgery_appointment_type: AppointmentType,
...
def test_reschedule_same_day_at_daily_cap_succeeds(
    reschedule_service: AppointmentTypeService,
    ...
    rescheduled = reschedule_service.reschedule_appointment_type_event(
        event_id=event.id,
...
def test_reschedule_across_boundary_into_full_period_rejected(
    ...
    with pytest.raises(AppointmentTypeScopedRuleViolationError) as exc_info:
        reschedule_service.reschedule_appointment_type_event(
            event_id=event.id,
...
def test_weekly_quota_with_sunday_week_start_hides_calendar_across_boundary(
    ...
    # week [Sep 7 - Sep 13].
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 7, 9)
    )
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 8, 9)
    )

    # Sep 9 (Tuesday) is still inside the now-capped [Sep 7 - Sep 13] week.
    still_in_capped_week = service.check_appointment_type_availability(
        surgery_slot.appointment_type_fk_id,  # type: ignore[arg-type]
    ...
    preceding_week = service.check_appointment_type_availability(
        surgery_slot.appointment_type_fk_id,  # type: ignore[arg-type]
...
def test_monthly_quota_hides_calendar_across_month_boundary(
    ...
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 5, 9)
    )
    _seed_booking(
        organization, surgery_appointment_type, surgery_slot, calendar, _utc(2025, 9, 20, 9)
    )

    # Still September -- at cap, hidden.
    september_result = service.check_appointment_type_availability(
        surgery_slot.appointment_type_fk_id,  # type: ignore[arg-type]
    ...
    october_result = service.check_appointment_type_availability(
        surgery_slot.appointment_type_fk_id,  # type: ignore[arg-type]
...
# and the SQL `get_appointment_type_quota_period_counts_json` function must
...
def test_python_and_sql_bucketing_agree_on_period_start(
    ...
    for instant in seed_instants:
        _seed_booking(organization, surgery_appointment_type, surgery_slot, calendar, instant)
    ...
    for period, week_start in (
        ...
        row = (
            ...
            .annotate(
                quota_counts=GetAppointmentTypeQuotaPeriodCountsJSON(
                    "id",
                    surgery_slot.id,
                    organization.id,
                    period,
...
# ---------------------------------------------------------------------------
# `check_appointment_type_availability` quota-counting query count is fixed per
# configured (slot, period) combination, not per range.
# ---------------------------------------------------------------------------


@pytest.mark.django_db
def test_check_appointment_type_availability_quota_query_count_independent_of_range_count(
    service: AppointmentTypeService,
    organization: Organization,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
    ...
    with CaptureQueriesContext(connection) as few:
        service.check_appointment_type_availability(
            surgery_slot.appointment_type_fk_id,  # type: ignore[arg-type]
            few_ranges,
        )
...
@pytest.mark.django_db
def test_unconfigured_appointment_type_issues_no_quota_counting_query(
    service: AppointmentTypeService,
    calendar: Calendar,
    surgery_slot: AppointmentTypeSlot,
) -> None:
