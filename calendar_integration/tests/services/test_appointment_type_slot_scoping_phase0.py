"""Integration tests for appointment-type-slot scoping on AvailableTime/BlockedTime.

Both models carry a nullable ``appointment_type_slot`` reference, and the default
...
that an appointment-type-scoped row, inserted directly (as a stand-in for what a future
write path will do), is invisible on every existing read path with zero call
site edits: the availability service (single-calendar), the appointment-type
service (appointment type discovery / availability), and the REST/public-API queryset
...
def appointment_type_slot(
    organization: Organization, managed_calendar: Calendar
) -> AppointmentTypeSlot:
...
def test_appointment_type_scoped_available_time_invisible_to_single_calendar_availability_service(
    organization: Organization,
    managed_calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
) -> None:
    """An appointment-type-scoped-only window must not make a managed calendar available on the
    single-calendar read path — that path has no appointment type context and must not gain one
    -- "no changes to single-calendar booking" is an explicit constraint.
    """
    start, end = _search_window()

    # Directly insert an appointment-type-scoped row covering the whole search window. No base
    # (appointment_type_slot IS NULL) row exists for this calendar.
...
@pytest.mark.django_db
def test_appointment_type_scoped_available_time_invisible_to_appointment_type_availability_check(
    organization: Organization,
    managed_calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
) -> None:
    """An appointment-type-scoped-only window must not make the calendar count toward its own
    slot's availability in appointment type discovery — appointment-type-scoped configuration is not
    wired into appointment type reads yet, so this must behave exactly as "nothing
    ...
    appointment_type_service.initialize(organization=organization)

    [availability] = appointment_type_service.check_appointment_type_availability(
        appointment_type_slot.appointment_type_fk_id, [(start, end)]
    )
    ...
    proposals = appointment_type_service.find_bookable_slots(
        appointment_type_id=appointment_type_slot.appointment_type_fk_id,
...
@pytest.mark.django_db
def test_appointment_type_scoped_available_time_invisible_to_appointment_type_availability_check_even_with_base_row(
    organization: Organization,
    managed_calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
) -> None:
    """Sanity check that the *base* row is what appointment type discovery actually reads —
...
@pytest.mark.django_db
def test_appointment_type_scoped_available_time_invisible_to_rest_and_public_api_queryset_pattern(
    organization: Organization,
    managed_calendar: Calendar,
    appointment_type_slot: AppointmentTypeSlot,
) -> None:
    ...
    so an appointment-type-scoped row must not appear in either.
    """
    start, end = _search_window()
    ...
    appointment_type_scoped_row = AvailableTime.objects.unscoped().create(
        organization=organization,
        calendar=managed_calendar,
        appointment_type_slot=appointment_type_slot,
    ...
    assert appointment_type_scoped_row.id not in visible_ids
