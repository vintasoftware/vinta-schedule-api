"""``AppointmentType.public_booking_slug``: generation, collision handling, and
...
class TestPublicBookingSlugBackfillMigrationChain:
    ...
    def _insert_appointment_type_without_slug(self, organization_id: int, name: str) -> int:
        """Insert an AppointmentType row straight through SQL, bypassing the ORM.

        Bypassing the ORM is required here, not a style choice: the LIVE
        ``AppointmentType`` model class still declares ``public_booking_slug``
    ...
    def test_backfill_is_distinct_idempotent_and_reverses_without_losing_data(self, organization):
        ...
        try:
            ...
            for appointment_type_id in ids[2:]:
                assert second_pass[appointment_type_id] == first_pass[appointment_type_id], (
                    "a row the prior run already filled must be untouched by a rerun"
                )
            for appointment_type_id in renulled:
                assert second_pass[appointment_type_id] is not None
                assert second_pass[appointment_type_id] != first_pass[appointment_type_id]
