"""Tests for AppointmentType management by organization admins.

Covers the use-case where a non-owner (e.g. a clinic administrator, scheduler,
or ops user) needs to manage an AppointmentType without being listed as a
`CalendarOwnership` on any of its pool calendars. Organization admins can
manage every appointment type in their own organization; org-admin privileges don't cross
...
def appointment_type(organization):
    ...
    g = AppointmentType.objects.create(organization=organization, name="Clinic Appointments")
    slot = AppointmentTypeSlot.objects.create(
        organization=organization, appointment_type=g, name="Physicians"
    )
...
# ---------------------------------------------------------------------------
# Membership capability, read from appointment types
# ---------------------------------------------------------------------------
...
@pytest.mark.django_db
def test_membership_in_the_admin_appointment_type_holds_manage_members(organization):
...
# ---------------------------------------------------------------------------
# can_manage_appointment_type admin override
# ---------------------------------------------------------------------------
@pytest.mark.django_db
def test_admin_can_manage_appointment_type_without_ownership(organization, appointment_type):
    ...
    assert svc.can_manage_appointment_type(user=admin, appointment_type=appointment_type) is True


@pytest.mark.django_db
def test_admin_of_other_org_cannot_manage_appointment_type(
    organization, other_org, appointment_type
):
    ...
    svc = CalendarPermissionService()
    assert (
        svc.can_manage_appointment_type(user=admin_elsewhere, appointment_type=appointment_type)
        is False
    )


@pytest.mark.django_db
def test_demoted_admin_cannot_manage_appointment_type(organization, appointment_type):
    ...
    svc = CalendarPermissionService()
    assert svc.can_manage_appointment_type(user=user, appointment_type=appointment_type) is True
    ...
    # routes the new appointment type set through ``assign_membership_groups``, and the
    # authorization decision reads those appointment types.
    ...
    assert svc.can_manage_appointment_type(user=user, appointment_type=appointment_type) is False


@pytest.mark.django_db
def test_appointment_type_permission_passes_for_admin_without_ownership(
    organization, appointment_type
):
    ...
    assert perm.has_object_permission(request, view=Mock(), obj=appointment_type) is True
