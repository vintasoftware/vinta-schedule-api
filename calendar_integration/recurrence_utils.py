...
class OccurrenceValidator:
    ...
    def get_previous_occurrence_date(
        recurring_object, before_date: datetime.datetime
    ) -> datetime.datetime | None:
        """Return the occurrence immediately before ``before_date`` or None."""
        rule = getattr(recurring_object, "recurrence_rule", None)
        if not rule:
            return None
        ...
        if before_date.tzinfo is None:
            before_date = timezone.make_aware(before_date, timezone.get_current_timezone())

        r = RecurrenceRuleSplitter._build_rrule(rule, dtstart)
        return r.before(before_date, inc=False)

    @staticmethod
    def normalize_modification_date(
        recurring_object, approximate_date: datetime.datetime
    ) -> datetime.datetime | None:
        """Return the closest actual occurrence to ``approximate_date`` (prefers earlier on ties)."""
        rule = getattr(recurring_object, "recurrence_rule", None)
        if not rule:
            return None

        dtstart = recurring_object.start_time
        if approximate_date.tzinfo is None:
            approximate_date = timezone.make_aware(
                approximate_date, timezone.get_current_timezone()
            )

        r = RecurrenceRuleSplitter._build_rrule(rule, dtstart)
        before = r.before(approximate_date, inc=True)
        after = r.after(approximate_date, inc=True)
        ...
        delta_before = abs((approximate_date - before).total_seconds())
        delta_after = abs((after - approximate_date).total_seconds())
