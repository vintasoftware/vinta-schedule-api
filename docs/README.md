...
| 6 | [concepts/appointment-types.md](concepts/appointment-types.md) | `AppointmentType`, slots, memberships, appointment type bookings. |
...
4. Most clinic appointments need **multiple** calendars at once — a doctor *and*
   a room. That's what **AppointmentTypes** model: a template that says "to make
...
5. **CalendarBundles** are the older one-shot mechanism for the same problem
   (single façade calendar that owns several child calendars). Appointment types
