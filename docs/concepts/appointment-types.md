...
("always Dr. Lee + this one room"); appointment types fix the **roles** and let
...
`AppointmentTypeSlot`s (its slots) and many `CalendarEvent`s (the
bookings made through it).

### `AppointmentTypeSlot`

A required role inside an appointment type. Carries:
...
- `calendars` — many-to-many to `Calendar` through
  `AppointmentTypeSlotMembership`. This is the **pool**.

### `AppointmentTypeSlotMembership`

Through table linking a `Calendar` to an `AppointmentTypeSlot`. One row
...
## Booking semantics — `AppointmentTypeService.create_appointment_type_event`

The service's `create_appointment_type_event` method:
...
5. Persists `CalendarEventAppointmentTypeSelection` rows for every (slot,
   calendar) pick.
...
- `AppointmentTypeQuerySet.only_appointment_types_bookable_in_ranges(ranges)` —
...
- `AppointmentTypeService.check_appointment_type_availability(appointment_type_id, ranges)` —
...
- `AppointmentTypeService.find_bookable_slots(appointment_type_id, search_window,
...
- Appointment type: `"Cardiology Outpatient"`.
...
- Appointment type: `"General Surgery"`.
...
- Appointment type: `"Overnight Hospitalist On-Call"`.
- Slot: `Hospitalist`, pool = the hospitalist personal calendars,
  `required_count=1`.

The "pick any free one from the pool" semantics make `AppointmentType`
...
## Updating and deleting appointment types
...
- `delete_appointment_type` refuses if there are any past or future events
  referencing the appointment type (the FK from `CalendarEvent` is `PROTECT`).
...
- An appointment-type event is still a **`CalendarEvent`** — recurrence,
...
- A bundled event is a different mechanism with a fixed membership and
  ...
  per-role pool picking should use appointment types; old flows that already work
  through bundles can keep doing so.
- Appointment type availability ultimately resolves to per-calendar availability,
