...
- **Gates:** `lint`, `types` (`check --deploy`), `typecheck` (`mypy .`) and `unit`, on every node. mypy is a zero-error CI gate, so it is a lane gate too; the reviewer should not be the first thing that notices a type error.
